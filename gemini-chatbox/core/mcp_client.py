"""
core/mcp_client.py — Comprehensive Model Context Protocol (MCP) Suite for Gemini Studio
...
LOCAL_MCP_CONFIG = os.path.join(STORAGE_DIR, "mcp_servers.json") if os.path.exists(os.path.join(STORAGE_DIR, "mcp_servers.json")) else (os.path.join(_BASE_DIR, "mcp_servers.json") if os.path.exists(os.path.join(_BASE_DIR, "mcp_servers.json")) else os.path.join(STORAGE_DIR, "mcp_servers.json"))
...
# ── 3b. Interactive Math, Science & Algorithm Visualizer MCP Server ──────
class MathScienceVisualizerMCPServer:
    def __init__(self):
        self.server_id = "mcp-math-visualizer"
        self.is_connected = True
        self.tools = [
            {
                "name": "render_math_visualizer",
                "description": "Generates a rich, interactive mathematical or physics simulation artifact (e.g. projectile motion, harmonic oscillators, Fourier series, geometry, linear algebra) with dynamic parameter sliders, live formula evaluation, and animated Canvas/SVG.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "title": {"type": "string", "description": "Title of the math/physics problem"},
                        "topic": {"type": "string", "description": "E.g. 'Calculus', 'Physics', 'Trigonometry', 'Fourier Series', 'Linear Algebra'"},
                        "formula": {"type": "string", "description": "Primary mathematical equation in LaTeX or plain text"},
                        "html_interactive_app": {"type": "string", "description": "Complete, standalone single-file HTML/CSS/JS code with interactive sliders, Canvas/SVG, and animated playback"}
                    },
                    "required": ["title", "formula", "html_interactive_app"]
                }
            },
            {
                "name": "render_function_plot",
                "description": "Generates an interactive 2D function plotting artifact with coordinate axes, tangent line / derivative slope indicator, and step-by-step calculus exploration.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "title": {"type": "string", "description": "Title of function plot"},
                        "function_expr": {"type": "string", "description": "Function expression e.g. 'f(x) = x^3 - 3x + 2'"},
                        "html_interactive_app": {"type": "string", "description": "Complete standalone HTML with Function Plot or D3 graph"}
                    },
                    "required": ["title", "function_expr", "html_interactive_app"]
                }
            }
        ]

    def call_tool(self, tool_name, arguments):
        title = arguments.get("title", "Interactive Mathematical Visualizer")
        app_html = arguments.get("html_interactive_app", "")
        if not app_html:
            return "Error: html_interactive_app code is required."

        return f'<antArtifact identifier="math-vis-{int(time.time())}" type="text/html" language="html" title="{title} (Interactive Math Simulation)">\n{app_html}\n</antArtifact>'
...
class MCPHub:
    def __init__(self):
        self.builtin_servers = [
            ...
            MathScienceVisualizerMCPServer(),
            NotebooksMCPServer(),
            DatabaseMCPServer()
        ]
        self.active_servers = {}
