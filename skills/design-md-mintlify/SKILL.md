...
- Tightly-controlled radius scale: marketing uses `{rounded.lg}` (12px), pill buttons use `{rounded.full}` â€” no in-between corner softening
...
### Brand & Accent
- **Mintlify Mint** ({colors.brand-green}): Signature accent â€” used on hero "Get started" pill button, green checkmarks in feature lists, featured pricing tier border accent, sidebar active indicator dots.
...
### Font Family
**Inter** (primary): Variable typeface optimized for UI legibility. Used across every UI surface â€” body, headings, navigation, button labels, captions. Fallbacks: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif.
...
The brand uses no italic variants of either face â€” emphasis comes from weight (500/600), color shift, or background highlighting (in code references).
...
- **Negative letter-spacing** progresses inversely with size â€” display sizes use -2px to -1.5px; smaller headings relax to 0
- **Documentation-grade body** (1.50 line-height on 14â€“16px) ensures comfortable long-form reading in dense docs surfaces
- **Inter / Geist Mono pairing** â€” Inter for everything else, Geist Mono surgically for code references; the contrast between the two is the brand's developer-respect signal
...
- **Tokens**: `{spacing.xxs}` (4px) Â· `{spacing.xs}` (8px) Â· `{spacing.sm}` (12px) Â· `{spacing.md}` (16px) Â· `{spacing.lg}` (20px) Â· `{spacing.xl}` (24px) Â· `{spacing.xxl}` (32px) Â· `{spacing.xxxl}` (40px) Â· `{spacing.section-sm}` (48px) Â· `{spacing.section}` (64px) Â· `{spacing.section-lg}` (96px) Â· `{spacing.hero}` (120px)
...
| 3 (mockup) | `rgba(0, 0, 0, 0.12) 0px 24px 48px -8px` | Hero product mockup framing â€” the deep diffuse drop on the homepage hero docs preview |
| 4 (brand-tinted) | `rgba(0, 212, 164, 0.08) 0px 8px 24px` | Featured pricing tier glow |

### Decorative Depth
- The homepage hero uses an atmospheric photographic backdrop (cloud illustration on sky-gradient) for depth â€” no shadow needed; the imagery does the work
...
The radius scale is tightly disciplined â€” the brand never uses a corner softening between `{rounded.md}` (8px) and `{rounded.lg}` (12px) for the same component family. Pill buttons (`{rounded.full}`) are used universally; rectangular cards use `{rounded.lg}` (12px) consistently.
...
**`button-primary`** â€” Black pill primary CTA, the dominant action across all surfaces.
...
**`button-ghost`** â€” Quieter rectangular ghost button (sidebar action, tertiary nav).
- Background transparent, text `{colors.ink}`, typography `{typography.button-md}`, padding `8px 12px`, rounded `{rounded.md}`.

**`button-link`** â€” Inline text link styled as a subtle button.
- Background transparent, text `{colors.ink}`, typography `{typography.body-sm-medium}`, padding `0`. Underline appears on activation.

**`button-icon-circular`** â€” 32Ã—32px circular utility button (close, copy, arrow).
- Background `{colors.canvas}`, text `{colors.ink}`, border `1px solid {colors.hairline}`, rounded `{rounded.full}`.

### Cards & Containers

**`card-base`** â€” Standard documentation/feature card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.

**`card-feature`** â€” Feature panel on light gray surface.
- Background `{colors.surface}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`.

**`card-help`** â€” "Need help?" CTA cards below the pricing comparison ("Quickstart guide", "Guide to technical writing", "Founder", "Sales").
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.

**`card-startup-perk`** â€” Startup-program perk grid item ("Discounts and credits", "Priority support", "Startup pack", "Founder community").
...
**`pricing-card`** â€” Standard pricing tier card.
...
**`pricing-card-featured`** â€” Highlighted pricing tier (Lift Off / featured plan).
...
**`testimonial-card-feature`** â€” Bright orange large testimonial card with photo + quote ("Cursor â€” Every YC batch we consistently see the top performing startups use Mintlify to build their docs.").
...
**`testimonial-card-quote`** â€” Smaller white testimonial card on the startups page.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.

**`founder-quote-card`** â€” Cursor founder testimonial card variant on the orange surface.
...
**`startup-program-card`** â€” Larger application/program card containing perks grid + apply CTA.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.

### Inputs & Forms

**`text-input`** â€” Standard text field.
...
- Border switches to `2px solid {colors.brand-green}` â€” focus uses the brand mint as the activation signal.

**`search-pill`** â€” Documentation top-bar search.
...
**`segmented-tab`** + **`segmented-tab-active`** â€” Underline-style tab navigation (used inside docs Tabs component for "First tab / Second tab / Third tab").
...
**`pill-tab`** + **`pill-tab-active`** â€” Pill-style tab nav (top of pricing page: "Pricing / Roadmap").
...
**`toggle-monthly-yearly`** â€” Two-state pill toggle (Monthly / Annual on pricing page).
...
**`badge-discount`** â€” Small green "Save 20%" badge attached to annual toggle.
...
**`badge-required`** â€” Red "REQUIRED" label on documentation property rows.
...
**`badge-type`** â€” Type signature chip in documentation (e.g. `string`, `number`, `boolean`).
- Background `{colors.surface}`, text `{colors.steel}`, typography `{typography.code-sm}`, rounded `{rounded.sm}`, padding `2px 6px`.

**`badge-tag`** â€” Documentation tag chip (e.g. `<Tabs>` reference highlighted in body text).
...
**`code-inline`** â€” Inline `<Tabs>` reference in body prose.
...
**`copy-code-button`** â€” "Copy code" button in code-block header.
...
**`property-row`** â€” API property documentation row (e.g. `defaultIndex` on the Tabs page).
...
**`feature-comparison-table`** â€” Detailed pricing-page feature comparison table.
- Background `{colors.canvas}`, text `{colors.ink}`, typography `{typography.body-sm}`, rounded `{rounded.md}`, border `1px solid {colors.hairline}`.

**`feature-comparison-row`** â€” Individual row inside the comparison table.
...
**`sidebar-nav-item`** + **`sidebar-nav-item-active`** â€” Documentation left rail link entries.
...
**`sidebar-section-header`** â€” Uppercase section header inside sidebar (e.g. "COMPONENTS", "PRIMITIVES").
- Background transparent, text `{colors.steel}`, typography `{typography.micro-uppercase}`, padding `{spacing.md} {spacing.md} {spacing.xs}`.

**`doc-toc-item`** + **`doc-toc-item-active`** â€” Right-rail table-of-contents links.
...
**Top Navigation (Marketing)** â€” Sticky white bar with logo, link list, and right-side CTAs.
...
**Top Navigation (Documentation)** â€” Compressed nav with center search-pill and right-side account/upgrade CTAs.
...
**`hero-band-sky`** â€” Homepage hero with atmospheric sky-blue to cream gradient and cloud illustrations.
...
**`hero-band-dark`** â€” Startups hero with dark teal-to-mint gradient and rocket launch illustration.
...
**`hero-product-mockup`** â€” Code-editor mockup framed inside the homepage hero.
...
**`logo-wall-item`** â€” Customer logo cell in 6-up trust-row grids ("Anthropic / Cognition / Mintlify / Vercel / react / Lovable", "Stripe / Block / PayPal / Compound / Auth").
- Background transparent, text `{colors.steel}`, typography `{typography.body-md-medium}`, padding `{spacing.lg}`.
- Logos rendered as wordmarks with consistent vertical centering.

**`faq-accordion-item`** â€” Frequently-asked-questions panel item (visible on pricing page).
...
**`footer-region`** â€” Multi-column site footer.
...
**`footer-link`** â€” Individual link entry in the footer.
- Background transparent, text `{colors.steel}`, typography `{typography.body-sm}`, padding `{spacing.xxs} 0`.

## Do's and Don'ts

### Do
- Reserve `{colors.brand-green}` (Mintlify mint) for accent CTAs and active state indicators only â€” even one accent button per viewport carries weight
...
- Pair Inter (UI prose) with Geist Mono (code) â€” never introduce a third typeface
...
- Keep documentation prose at `{typography.body-md}` (16px) with 1.50 line-height â€” never compress

### Don't
- Don't use `{colors.brand-green}` on body text or large surfaces â€” it loses signal
...
- Don't reduce documentation line-height below 1.50 â€” long-form readability suffers
- Don't combine atmospheric gradients with multiple competing color accents in the same hero â€” the sky/dark gradient is the brand mood; let it breathe
- Don't use Inter for code or Geist Mono for prose â€” the typeface assignment IS the brand voice
...
| Mobile (large) | 480 â€“ 767px | Same as small but feature tiles render 2-up. Hero scales to 44px. |
| Tablet | 768 â€“ 1023px | 2-column feature grids. Pill-tab nav returns. Documentation sidebar collapses to drawer. Hero scales to 56px. |
| Desktop | 1024 â€“ 1279px | Full 3-column docs grid (sidebar / body / TOC). 3-tier pricing card row. Hero at 72px. |
| Wide Desktop | â‰¥ 1280px | Wider hero gutters, larger product mockup, fixed 240px sidebar. |

### Touch Targets
- Pill buttons render at 36â€“40px effective height â€” bumps to 44px on mobile via padding override
- Circular icon buttons: 32Ã—32px desktop â†’ 44Ã—44px mobile
- Form inputs render at 40px height; bumps to 44px mobile
- Sidebar nav items render at ~32px tall â€” bump to 44px mobile drawers
...
- **Documentation grid**: 3-column desktop â†’ sidebar-drawer at < 1024px â†’ single-column at < 768px
- **Pricing comparison**: 3-column tiers â†’ 1-column stacked at < 768px; comparison table becomes horizontal-scroll
- **Hero typography**: `{typography.hero-display}` (72px) â†’ 56px tablet â†’ 44px mobile-large â†’ 36px mobile-small
- **Customer logo wall**: 6-up â†’ 3-up at tablet â†’ 2-up at mobile
- **Footer**: 5-column desktop â†’ 2-column tablet â†’ accordion at mobile
...
5. Default to `{typography.body-md}` for body and `{typography.subtitle}` for emphasis. Headlines step down `hero-display â†’ display-lg â†’ heading-1 â†’ heading-2 â†’ heading-3 â†’ heading-4 â†’ heading-5`.
...
8. Documentation prose belongs in `{typography.body-md}` 16px with 1.50 line-height â€” anything denser breaks the reading experience.
...
- Animation/transition timings are not extracted; recommend 150â€“200ms ease for hover/focus state transitions
- Form validation success state is not explicitly captured beyond defaults â€” implement following standard green-border + success badge patterns
