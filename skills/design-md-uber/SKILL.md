---
name: design-md-uber
description: Design system and styling guidelines based on uber. Use this skill when asked to create a design inspired by uber.
---
---
version: alpha
name: Uber-Inspired-design-analysis
description: An inspired interpretation of Uber's design language â€” a transportation-and-delivery super-app brand whose web surface is a black-and-white duet, framed by a custom geometric display sans, accented by a single signature pill shape (radius 999px) on every interactive element, and decorated only by editorial 4:3 illustrations of riders, drivers, and city objects.
...
components:
  ...
  ex-pricing-tier-featured:
    description: "Featured tier â€” polarity-flipped to ink with white text."
    backgroundColor: "{colors.ink}"
    textColor: "{colors.on-primary}"
    rounded: "{rounded.xl}"
    padding: "{spacing.2xl}"
  ex-product-selector:
    description: "Plan picker â€” re-purposed for the brand's Ride / Eats / Reserve tier picker. Uses category-button pills inside the frame."
    backgroundColor: "{colors.canvas-soft}"
    rounded: "{rounded.none}"
    padding: "{spacing.2xl}"
  ex-cart-drawer:
  ...
  ex-auth-form-card:
    ...
    padding: "{spacing.2xl}"
  ex-modal-card:
    description: "Modal dialog surface â€” same chrome as card-content with Level 2 drop shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.xl}"
    padding: "{spacing.2xl}"
  ex-empty-state-card:
  ...
  ex-toast:
    description: "Toast notification surface â€” flat-cornered card-content chrome with Level 2 drop shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.xl}"
    padding: "{spacing.md} {spacing.lg}"
    typography: "{typography.body-sm}"
...
Uber is a transportation-and-delivery super-app â€” ride, eats, freight, the whole urban logistics layer â€” and the brand's web surface signals that scale through restraint: no third colour, no accent palette, no illustration that fights the headline. The page is structurally a black-and-white duet, where black `{colors.primary}` is the conversion anchor (every CTA pill, every nav login button, the footer fill) and `{colors.canvas}` white carries everything else. The only consistent decoration is a body of editorial 4:3 illustrations â€” riders, drivers, parking lots, cars-on-highway â€” that ground the marketing without leaking accent colour into the system.

Type is the second decisive voice. Two custom faces carry every page: `UberMove` at weight 700 for headlines (32 â€“ 52 px display sizes with tight 1.22 â€“ 1.25 line-height, never letter-spaced), and `UberMoveText` at weights 400 / 500 for body, button, and link. The pairing reads as engineering-grade â€” no italic, no decorative weight, no tracking flourish. Headlines are sentence-case; eyebrows are uppercase only when used as the section eyebrow ("WHY BECOME"); buttons are sentence-case.

The single shape signature is the pill. Every interactive element rounds to `{rounded.pill}` 999 px â€” primary CTA, secondary CTA, subtle gray pill, white floating pill, category chip, app-download badge. Cards and surfaces round to `{rounded.xl}` 16 px; the larger "Go Get 2026" annual-showcase card uses the same 16 px shape, just at scale. The tab-toggle on the hero ride-request form uses an off-shape `{rounded.pill-tab}` 36 px â€” barely-pill, deliberately tighter than the canonical 999 px pill.
...
- The pill is the single signature shape â€” `{rounded.pill}` 999 px on every interactive element except the tab-toggle (`{rounded.pill-tab}` 36 px) and the larger product cards (`{rounded.xl}` 16 px).
...
- A signature alternating-band rhythm: white feature card â†’ black promo card (with white text and white CTA) â†’ white feature card â†’ black footer. The black bands are NOT hero-only; they appear mid-page as promo callouts.
...
### Brand & Accent
- **Ink Black** (`{colors.primary}` â€” `#000000`): The brand's only conversion colour. Every primary CTA pill, the footer fill, every dark promo band, every nav login button. The system has no secondary accent.
- **Surface Pressed** (`{colors.surface-pressed}` â€” `#e2e2e2`): The pressed-state fill for white pills â€” a soft grey that's used only in active / pressed states.
- **Black Elevated** (`{colors.black-elevated}` â€” `#282828`): A near-black used on hover for the translucent white tab-toggle pill. Documented as a system colour because it appears on a recurring brand control.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): The default page background.
- **Canvas Soft** (`{colors.canvas-soft}` â€” `#efefef`): The soft gray fill for category chips, form-input rows inside the ride-request card, and subtle pill buttons.
- **Canvas Softer** (`{colors.canvas-softer}` â€” `#f3f3f3`): A slightly lighter gray used as a nested-input fill on white surfaces.

### Text
- **Ink** (`{colors.ink}` â€” `#000000`): Every heading and body paragraph on light surfaces.
- **Body** (`{colors.body}` â€” `#5e5e5e`): Secondary text â€” captions, sub-headings, supporting copy.
- **Hairline Mid** (`{colors.hairline-mid}` â€” `#4b4b4b`): A mid-gray used for muted link text inside footer columns and breadcrumb-style nav.
- **Mute** (`{colors.mute}` â€” `#afafaf`): The lightest text role â€” placeholder text, fine print, low-priority metadata.
- **On Dark** (`{colors.on-dark}` â€” `#ffffff`): All text on `{colors.ink}` surfaces (footer, dark promo bands).

### Semantic
The brand does not maintain a separate error / success / warning palette in its public marketing surface. Validation cues come from the primary black or from the brand's editorial illustrations. The `#0000ee` link colour is the system's only chromatic â€” it's the browser-default link blue, appearing in body-copy inline links inside legal / footer text.
...
1. **A custom geometric display sans** (extracted as `UberMove`) for every headline. Weight 700 only; no italic; no tracking variation. Sizes range from `display-sm` 20 px up to `display-xxl` 52 px on the hero. Line-heights tighten to 1.22 â€“ 1.25 at display sizes for a poured-on-the-page look.
2. **A custom text sans** (extracted as `UberMoveText`) for body, button, link, and small headings. Weights 400 and 500 are the working pair. Used at 12 â€“ 18 px; 24 px maximum for ride-request form labels. Tracking is always neutral.

The two faces share a family DNA but never overlap roles â€” the display face never carries a body paragraph; the text face never carries a hero headline.
...
- **Display sans** â€” *Inter* weight 700 with `font-feature-settings: "ss01"` enabled comes closest. *Geist* weight 700 is the second-best option.
- **Text sans** â€” *Inter* weights 400 / 500 match the geometric width and x-height. *Plus Jakarta Sans* is a softer alternative if the brand wants a less neutral feel.

## Layout

### Spacing System
- **Base unit**: 4 px. Most captured values are multiples of 4 with a few 6-px sub-multiples (10, 14) inside button padding.
- **Tokens**: `{spacing.xxs}` 4 px Â· `{spacing.xs}` 6 px Â· `{spacing.sm}` 8 px Â· `{spacing.md}` 12 px Â· `{spacing.lg}` 16 px Â· `{spacing.xl}` 20 px Â· `{spacing.2xl}` 24 px Â· `{spacing.3xl}` 32 px.
...
Card-to-card spacing carries the rhythm â€” between two stacked promo cards there's roughly a full `{spacing.3xl}` 32 px gutter; inside a card the headline / paragraph / CTA stack is tight (`{spacing.sm}` 8 px between siblings). The black promo bands and the footer have no internal hairlines â€” content sits on flat ink with white text.
...
| Mobile-Large | 600â€“767px | Same as Mobile; chip rows enable horizontal scroll. |
| Tablet | 768â€“1119px | 2-up promo grid at upper widths; nav stays horizontal until â‰¥ 1120 px. |
| Desktop | 1120â€“1135px | Full nav row visible; promo cards 2-up. |
| Desktop-Large | â‰¥ 1136px | Container caps at ~1200 px; bands stay edge-to-edge while content centres. |

#### Touch Targets
The pill `button-primary` renders at ~44 px tall (10 px vertical padding + 24 px label line-height); the larger `button-large-rounded` at ~56 px. Both meet WCAG AAA at all breakpoints. Category chips inflate to â‰¥ 44 px tall through extra padding on touch viewports.
...
- **Photography**: same â€” square or landscape; framed inside `{rounded.xl}` card chrome.
...
| Level | Treatment | Use |
|---|---|---|
| Level 0 â€” Flat | No shadow, no border. | Default â€” most cards and surfaces lean on hairline-of-canvas contrast. |
| Level 1 â€” Subtle Drop | `rgba(0, 0, 0, 0.12) 0px 4px 16px 0px` | Card-elevated frames around promo cards on light bands. |
| Level 2 â€” Card Drop | `rgba(0, 0, 0, 0.16) 0px 4px 16px 0px` | The ride-request form card on the hero; large content cards with embedded forms. |
| Level 3 â€” Pill Float | `rgba(0, 0, 0, 0.16) 0px 2px 8px 0px` | The floating white pill button (the one that floats over hero photography). |
...
| `{rounded.xl}` | 16px | Canonical card radius â€” promo cards, content cards, ride-request form card, annual-showcase card, large rounded buttons. |
| `{rounded.pill}` | 999px | The brand's signature interactive shape â€” every pill button, category chip, app-download pill, icon button. |
...
**`button-primary`** â€” the canonical black pill, the brand's conversion target.
...
**`button-secondary`** â€” the white pill paired with the black primary.
- Background `{colors.canvas}`, text `{colors.ink}`, same label and padding as `button-primary`, shape `{rounded.pill}`.

**`button-subtle`** â€” the gray secondary pill used for tertiary actions inside cards (e.g., "Learn more" / "Use Reserve").
...
**`button-floating`** â€” the white pill with a subtle drop-shadow that floats over a dark or photographic surface.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.md}`, shape `{rounded.pill}`. Carries a Level 3 pill-float shadow.

**`button-large-rounded`** â€” the bigger black call-to-action used inside the ride-request flow ("Yes, help me").
- Background `{colors.primary}`, text `{colors.on-primary}`, label in `{typography.button-large}`, padding `{spacing.lg} {spacing.xl}`, shape `{rounded.xl}` 16 px (not pill â€” the only black CTA that breaks the pill rule, used in the larger form context).

**`button-tab-translucent`** â€” the tab-toggle on the hero ride-request form (Ride / Drive).
...
**`card-content`** â€” the canonical content card.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.2xl}`, shape `{rounded.xl}` 16 px. No shadow on the default state.

**`card-elevated`** â€” the content card with Level 1 subtle drop.
- Background `{colors.canvas}`, text `{colors.ink}`, same padding + shape as `card-content`. Shadow at Level 1.

**`card-soft-tinted`** â€” the gray-tinted card used as a sub-region inside the page (e.g., "Plan for later" callout).
- Background `{colors.canvas-soft}`, text `{colors.ink}`, padding `{spacing.2xl}`, shape `{rounded.xl}`.

**`promo-card-illustrated`** â€” the 2-column promo card with illustration on one side and copy on the other.
...
**`promo-card-on-dark`** â€” the polarity-flipped promo card in black.
...
**`nav-link`** â€” the link row inside `nav-bar`.
- Text `{colors.ink}`, set in `{typography.body-md-strong}` 500 weight.

**`footer`** â€” the deep-black footer band.
...
**`hero-band-light`** â€” the white hero with the ride-request card.
...
**`hero-band-dark`** â€” the rare black hero (used on Uber Eats and Drive landing).
...
**`category-button`** â€” the horizontal-scroll category row ("Reserve / Rentals / Teens / Group rides").
...
**`faq-row`** â€” the FAQ accordion item.
- Background `{colors.canvas}`, text `{colors.ink}`, question in `{typography.body-md-strong}`, padding `{spacing.lg}` 0. No card chrome â€” hairline dividers between rows.

**`app-download-pill`** â€” the "Download the Rider app" / "Download the Driver app" pill.
...
**`icon-button-circular`** â€” the round icon container used in the nav and inside the ride-request card.
- Background `{colors.canvas-soft}`, dark icon, shape `{rounded.full}`. No label.

### Links

**`link-blue`** â€” the system-default browser-blue link inside legal / footer fine print.
- Text `{colors.link}` (`#0000ee`), body in `{typography.body-md}`.

**`link-on-dark`** â€” the white link inside dark bands.
- Text `{colors.on-dark}`, body in `{typography.body-md}`.

**`link-mute`** â€” the muted gray link inside footer columns.
- Text `{colors.hairline-mid}`, body in `{typography.body-md}`.

**`link-mute-soft`** â€” the lightest gray link, used for low-priority secondary text on dark surfaces.
...
- Render cards in `{rounded.xl}` 16 px â€” promo cards, content cards, the ride-request form card, the annual-showcase card all share this radius.
...
- Don't use `{rounded.full}` 9999 px for square cards â€” the pill 999 px and full 9999 px effects are identical for interactive elements, but cards stay at `{rounded.xl}` 16 px.
