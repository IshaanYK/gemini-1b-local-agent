...
description: Design system and styling guidelines based on shopify. Use this skill when asked to create a design inspired by shopify.
---
---
version: alpha
name: Shopifi-Inspired-design-analysis
description: An inspired interpretation of Shopifi's design language â€” a cinematic commerce platform that runs two parallel design tracks. The marketing-hero and product-narrative pages live on near-black canvases with full-bleed photography of merchants, giant Neue Haas Grotesk display type at thin weights, and a single black-pill CTA stroked in white. The transactional pages (pricing, signup, dashboards) flip to a cream-mint canvas with pastel aloe and pistachio greens, the same pill button vocabulary, and Inter for UI body. The two tracks share typographic DNA but diverge sharply in canvas polarity â€” and that choice is the brand.
...
Shopifi runs two parallel design tracks that share typographic DNA and a single button vocabulary, but diverge in canvas polarity. The marketing track lives on `{colors.canvas-night}` (`#000000`) â€” full-bleed cinematic photography of merchants, giant `{typography.display-xxl}` headlines in Neue Haas Grotesk Display set at weight 330 (a thin, almost editorial cut), and a single CTA: a white-stroked black pill with the form `button-outline-on-dark`. The pages read like the spread of a high-end print magazine: lots of black, lots of negative space, photography that doesn't compete with text, and one and only one action per band.

The transactional track flips to `{colors.canvas-light}` and `{colors.canvas-cream}` (an off-white that's barely warmer than pure white). Pricing tiers, comparison tables, and signup flows sit on this lighter canvas, with the same pill button system but in inverse polarity (a solid black pill with white text, or a `{colors.aloe-10}` mint pill for the featured / "Start free trial" tier). The accents â€” `{colors.aloe-10}` mint and `{colors.pistachio-10}` pistachio â€” show up only on the light track, never on the cinematic dark hero pages.

Typography is split across three families. **Neue Haas Grotesk Display** at thin weights (330â€“500) handles every display, headline, and editorial moment â€” the brand's identity is that thin display cut. **Inter Variable** at 420â€“550 weights handles every UI body, button label, caption, and form field â€” utility text that doesn't fight the display. **ui-monospace** appears only in code blocks and rare technical eyebrows. Across all three families, the OpenType `ss03` stylistic set is enabled â€” it's the brand's character-level signature, applied universally.

**Key Characteristics:**
- Two-canvas system: `{colors.canvas-night}` for cinematic marketing, `{colors.canvas-light}` / `{colors.canvas-cream}` for transactional surfaces â€” never blended.
...
- Aloe and pistachio greens (`{colors.aloe-10}`, `{colors.pistachio-10}`) are reserved for the light track â€” they signal commerce, growth, transactional success.
...
- The OpenType `ss03` stylistic set is enabled across every text role â€” a character-level unifier that tracks across both tracks.
...
### Brand & Accent
- **Aloe** (`{colors.aloe-10}` â€” `#c1fbd4`): The featured-tier and "growth" accent. Used as a pill button background on light surfaces and as a feature-card fill in the pricing comparison band.
- **Pistachio** (`{colors.pistachio-10}` â€” `#d4f9e0`): Softer than aloe; used as a wide section band fill on the light track to signal a different category of feature without leaving the green family.
...
### Surface
- **Canvas Night** (`{colors.canvas-night}` â€” `#000000`): Pure black hero, cinematic feature pages, footer.
- **Canvas Night Elevated** (`{colors.canvas-night-elevated}` â€” `#0a0a0a`): Cards on cinematic surfaces, video frames.
- **Surface Elevated Dark** (`{colors.surface-elevated-dark}` â€” `#1e2c31`): Dark teal-shifted surface used on a small subset of dark cards to introduce subtle depth without breaking the black.
- **Canvas Light** (`{colors.canvas-light}` â€” `#ffffff`): Pricing, signup, comparison tables.
- **Canvas Cream** (`{colors.canvas-cream}` â€” `#fbfbf5`): Slightly warm off-white used on the pricing-page background canvas â€” invisibly different from `#ffffff` but adds editorial warmth.
- **Hairline Light** (`{colors.hairline-light}` â€” `#e4e4e7`): 1px borders on light cards, table dividers.
- **Hairline Dark** (`{colors.hairline-dark}` â€” `#1e2c31`): 1px borders on the rare dark cards that have visible chrome.

### Shade Ladder
- **Shade-30** (`{colors.shade-30}` â€” `#d4d4d8`): Tag / chip background on light, footer hairline on dark.
- **Shade-40** (`{colors.shade-40}` â€” `#a1a1aa`): Tertiary text on light, secondary text on dark.
- **Shade-50** (`{colors.shade-50}` â€” `#71717a`): Secondary text on light.
- **Shade-60** (`{colors.shade-60}` â€” `#52525b`): Tertiary text on light, deep on dark.
- **Shade-70** (`{colors.shade-70}` â€” `#3f3f46`): Pressed-state of the primary pill button; deep dark surface accent.

### Text
- **Ink** (`{colors.ink}` â€” `#000000`): All text on light canvas.
- **On Primary** (`{colors.on-primary}` â€” `#ffffff`): All text on dark canvas + filled-pill labels.

## Typography

### Font Family

The display tier is **Neue Haas Grotesk Display** at thin weights (330â€“500). When unavailable, fall back to **Helvetica** at light weight, then Arial. The thin-weight cut is the brand â€” no substitution should default to weight 400+.

The UI tier is **Inter Variable** at 420â€“550 â€” a variable font with sub-weight precision that lets the system span body (420), strong (550), and caption (500) without jumping to heavier tiers. Inter is open-source via Google Fonts.

The code tier is **ui-monospace**, the system mono â€” preferred over a webfont mono to avoid unnecessary downloads.
...
### Principles
- **Display thinness is the brand.** Always render display sizes at weight 330 â€” never 400+. The thinness is a deliberate editorial choice that makes the giant size feel quiet.
- **Display in NHGD, body in Inter.** Don't push body roles up to NHGD; don't push display roles down to Inter.
- **Tracking lifts on display.** The 96px hero gets +2.4px positive tracking â€” the thin glyphs need air. At 70px and below, tracking returns to 0.

### Note on Font Substitutes
Open substitutes for Neue Haas Grotesk Display: **Helvetica Now Display** (proprietary) or **Inter Display** at light weights (open-source) are the closest matches. Avoid Helvetica Neue at default weight â€” it's too heavy for the brand's thin tier. **Inter Variable** is open-source via Google Fonts and is the canonical body face â€” no substitute needed.
...
- **Section padding**: `{spacing.huge}` 64â€“128px on cinematic marketing pages (extreme negative space is the point); collapses to ~48px on transactional pages where density takes priority.
- **Card internal padding**: `{spacing.xxl}` 32px on pricing cards; `{spacing.xl}` 24px on compact tag rows.

### Grid & Container
- Cinematic hero pages use a wide max-width container (~1440â€“1600px) with edge-bleeding photography that escapes the container.
- Pricing collapses through 4-up â†’ 2-up â†’ 1-up tiers based on viewport.
- Body content centers in a ~720â€“840px reading column on long-form pages.

### Whitespace Philosophy
The cinematic track treats whitespace as the brand's most valuable asset â€” sections often have 128â€“192px of vertical air between content blocks, with photography filling the rest. The transactional track tightens to ~48â€“64px between bands because users are scanning, comparing, and acting. The contrast between the two whitespace philosophies is part of the brand voice.
...
### Decorative Depth
On the cinematic track, depth comes from photography â€” full-bleed merchant imagery layered behind cards, with subtle inset top-edge highlights creating the illusion of light hitting a glass surface. On the light track, the layered tiny-shadow stack (Level 3) produces a soft, paper-like halo around pricing cards â€” depth without harshness.
...
**`button-outline-on-dark`** â€” the cinematic hero CTA.
...
**`button-outline-on-light`** â€” the light-track equivalent.
- Background `{colors.canvas-light}`, 1px solid `{colors.ink}` border, text `{colors.ink}`, same pill geometry.

**`button-aloe-pill`** â€” the featured CTA on pricing pages.
...
**`card-pricing-featured`** â€” the highlighted pricing tier.
...
**`card-feature-cinematic`** â€” feature card on the cinematic track.
...
**`card-pistachio-band`** â€” wide horizontal band card used to highlight a category of features on the light track.
- Background `{colors.pistachio-10}`, text `{colors.ink}`, rounded `{rounded.lg}` 12px, padding `{spacing.xxl}`.

**`card-photo-frame`** â€” full-bleed photography container on cinematic pages.
...
**`text-input`** â€” standard text input on light surfaces.
...
**`nav-bar-light`** â€” top nav on light pages.
...
**`nav-bar-dark`** â€” top nav on cinematic pages.
...
**`pill-tag-mint`** â€” small tag on light surfaces, signaling a feature category.
- Background `{colors.aloe-10}`, text `{colors.ink}`, type `{typography.eyebrow-cap}`, padding `{spacing.xs} {spacing.md}`, rounded `{rounded.pill}`.

**`pill-tag-shade`** â€” neutral tag on light surfaces.
- Background `{colors.shade-30}`, text `{colors.ink}`, otherwise same shape as `pill-tag-mint`.

### Signature Components

**Cinematic Photography Layer** â€” full-bleed merchant photos on the hero. No overlay scrim, no text-on-image; instead, the type sits in clean negative space above or below the photo. The brand treats photography as an editorial spread, not as decoration.

**Stacked Tiny Shadows (Level 3 Elevation)** â€” pricing cards on the light track use 4 stacked tiny drop shadows (each 1â€“8px Y offset, 10% black) to produce a soft, layered paper halo. This is the brand's distinctive depth on light.

**`link-on-dark`** â€” inline link on cinematic pages.
...
**`footer-dark`** â€” full-page-width footer on the cinematic track.
- Background `{colors.canvas-night}`, text `{colors.on-primary}`, type `{typography.caption}`, padding `{spacing.huge} {spacing.xl}`. Contains 4â€“5 columns of muted-tone link groups, social icons, and a small legal row.

**`footer-light`** â€” equivalent on the transactional track.
- Background `{colors.canvas-light}`, text `{colors.ink}`, otherwise same structure.

## Do's and Don'ts

### Do
- Reserve `{colors.aloe-10}` and `{colors.pistachio-10}` for the light track only â€” they don't appear on cinematic black pages.
...
- Use full-bleed photography on cinematic pages â€” let it escape the container.
...
### Don't
- Don't introduce a third canvas color â€” stick to black or light/cream. Greys, beiges, and blues are not in the system.
...
- Don't put aloe / pistachio greens behind type â€” they're surface fills, not text colors.
...
|---|---|---|
| Wide | â‰¥ 1440px | Full cinematic hero with edge-bleeding photography; pricing 4-up |
| Desktop | 1024â€“1440px | Default content max-width; pricing 4-up tightens |
| Tablet | 768â€“1023px | Pricing 2-up; cinematic hero photography crops |
| Mobile | < 768px | Pricing 1-up; hamburger nav; display-xxl drops to ~56â€“64px |

### Touch Targets
- Pill buttons hit â‰¥ 44Ã—44px on mobile via 12px vertical padding Ã— 16px line-height. WCAG AAA compliant.
- Form fields stay at the 44px minimum height across all breakpoints.

### Collapsing Strategy
- Display sizes scale down through the breakpoint stair: 96 â†’ 70 â†’ 55 â†’ 48 â†’ 36px on mobile.
- Cinematic photography crops aggressively at smaller widths, prioritizing focal subject over edge-bleed.
- Pricing tiers stair-step 4-up â†’ 2-up â†’ 1-up; the featured aloe tier stays visually distinguished at every step.
...
6. Keep the two canvas tracks separated â€” when designing a new page, choose cinematic OR transactional, not both.
