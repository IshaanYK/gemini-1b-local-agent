...
description: Design system and styling guidelines based on nvidia. Use this skill when asked to create a design inspired by nvidia.
---
---
version: alpha
name: NVIDIA-design-analysis
description: |
  An engineering-grade marketing system organized around two surface modes â€” a deep black canvas for hero and footer chapters and a flat paper-white canvas for body content â€” connected by a single, almost violently saturated NVIDIA Green accent that carries every CTA, every active tab, and the small decorative corner squares that mark out cards. The system is unapologetically angular: 2px radius across every surface, tight bold sans-serif typography in NVIDIA's proprietary EMEA cut, and a hairline gray rule that separates dense multi-column technical content. There is no decorative gradient, no atmospheric mesh, no soft drop shadow â€” just black, white, gray, and green stacked into a structured editorial grid that scales from product cards to massive industry landing pages without bending its rules.
...
NVIDIA's marketing system is built like a piece of engineering documentation that learned graphic design â€” every page is a structured cascade of dense, factual information arranged on a paper-white grid, framed top and bottom by deep black hero/footer chapters. There is exactly one accent color in the entire system, and it is doing all the work: NVIDIA Green (`{colors.primary}` â€” `#76b900`), used for every primary CTA, every active tab, every link affordance on dark surfaces, and the small decorative corner squares that mark out card containers. Nothing else competes for attention.

The system's character comes from extreme typographic restraint and an almost punishingly angular geometry. Every container, button, and image uses `{rounded.sm}` (2px) â€” a token that's barely-there but never zero, giving the system the precise, technical feel of CAD output rather than warm consumer software. Cards sit on plain `{colors.canvas}` with a hairline `{colors.hairline}` border (no shadow, no elevation), separated by tight 8px-base spacing rhythm. Long-form pages stack 6â€“10 of these cards into multi-column technical grids without ever introducing decorative breaks.

The black-canvas hero and footer chapters are the system's "headline moments" â€” a single full-bleed photographic or 3D-rendered image with `{typography.display-xl}` headline copy laid in white, a single green CTA button, and a small green corner square as the only ornamentation. Everything else is subordinate.
...
- Two-mode surface architecture: `{colors.surface-dark}` for hero/footer chapters; `{colors.canvas}` for body â€” alternating in a predictable rhythm down the page
...
- Dense multi-column footer with 4â€“6 link columns on `{colors.surface-dark}` â€” every page closes with the same structured global navigation

## Colors

> **Source pages:** `/tr-tr/` (primary homepage), `/en-eu/industries/healthcare-life-sciences/`, `/en-eu/solutions/ai/`, `/en-eu/ai/foundry/`. The chrome palette is identical across all four â€” only photography and copy vary.

### Brand & Accent
- **NVIDIA Green** (`{colors.primary}` â€” `#76b900`): the brand. Every primary CTA, every active state, every link affordance on dark surfaces, every corner square, and the brand wordmark itself.
- **NVIDIA Green Dark** (`{colors.primary-dark}` â€” `#5a8d00`): pressed state for the primary button â€” a single notch deeper than the brand green.
- **Accent Green Pale** (`{colors.accent-green-pale}` â€” `#bff230`): rare highlight tint used in editorial callouts and decorative micro-blocks; never on chrome.

### Surface
- **Page Canvas** (`{colors.canvas}` â€” `#ffffff`): the body of every page. Cards sit directly on it with hairline rules.
- **Soft Surface** (`{colors.surface-soft}` â€” `#f7f7f7`): breadcrumb strip, sub-nav, side-by-side comparison panels, alternating row backgrounds.
- **Black Canvas** (`{colors.surface-dark}` â€” `#000000`): hero chapter, dark CTA strips, footer, primary nav. The system's "frame" color.
- **Surface Elevated** (`{colors.surface-elevated}` â€” `#1a1a1a`): nested dark panels inside the footer (column dividers, fine-print bar).
- **Hairline** (`{colors.hairline}` â€” `#cccccc`): 1px card border, table rule, divider between footer link sections.
- **Hairline Strong** (`{colors.hairline-strong}` â€” `#5e5e5e`): 1px divider on dark surfaces (footer column rules, dark-mode card edges).

### Text
- **Ink** (`{colors.ink}` â€” `#000000`): headlines and body text on `{colors.canvas}`.
- **Body** (`{colors.body}` â€” `#1a1a1a`): long-form paragraph text where pure black is too heavy.
- **Mute** (`{colors.mute}` â€” `#757575`): metadata, breadcrumb separators, footer copyright.
- **Stone** (`{colors.stone}` â€” `#898989`): least-emphasis text and disabled state.
- **Ash** (`{colors.ash}` â€” `#a7a7a7`): disabled icon color and faint utility text.
- **On Dark** (`{colors.on-dark}` â€” `#ffffff`): primary text on `{colors.surface-dark}`.
- **On Dark Mute** (`{colors.on-dark-mute}` â€” `rgba(255,255,255,0.7)`): secondary footer link text and dark-canvas body copy.

### Semantic
- **Error** (`{colors.error}` â€” `#e52020`): validation messages, destructive confirmation.
- **Error Deep** (`{colors.error-deep}` â€” `#650b0b`): pressed state for error buttons; hover-pressed validation icons.
- **Warning** (`{colors.warning}` â€” `#df6500`): caution callouts, deprecated documentation banners.
- **Warning Bright** (`{colors.warning-bright}` â€” `#ef9100`): inverse warning on dark canvas.
- **Success Deep** (`{colors.success-deep}` â€” `#3f8500`): positive confirmation where NVIDIA Green's saturation would clash.
- **Link Blue** (`{colors.link-blue}` â€” `#0046a4`): inline anchor link color on light canvas â€” the only blue in the system, reserved for prose-embedded hyperlinks.

### Editorial Accents (used sparingly inside long-form content)
- **Accent Purple** (`{colors.accent-purple}` â€” `#952fc6`): research / scientific computing editorial accent.
- **Accent Purple Deep** (`{colors.accent-purple-deep}` â€” `#4d1368`): paired dark for purple lockups.
- **Accent Purple Pale** (`{colors.accent-purple-pale}` â€” `#f9d4ff`): wash background for editorial callouts.
- **Accent Yellow Pale** (`{colors.accent-yellow-pale}` â€” `#feeeb2`): documentation tip / soft callout fill.

## Typography

### Font Family
- **NVIDIA-EMEA** is the proprietary brand sans-serif used across every text role on the site. It carries weights 400 (regular) and 700 (bold) and falls back to Arial â†’ Helvetica.
- **Font Awesome 6 Pro** and **Font Awesome 6 Sharp** are used exclusively for iconography (chevrons, social glyphs, breadcrumb separators, search/menu icons) at sizes 14â€“22px.

NVIDIA's type system is unusually flat: most chrome and body roles render at the same line-height (1.25â€“1.5) with the only meaningful variation coming from weight (400 vs 700) and size. The system relies on weight contrast â€” not size jumps and not color tinting â€” to establish hierarchy, which gives marketing copy and technical documentation an editorial newspaper feel.
...
- **Hero chapter padding:** 80px vertical / 48px horizontal â€” the largest spacing in the system, reserved for `{component.hero-card-dark}`.
...
- **Card aspect:** product cards lean to 1:1 or 4:3 with 16:9 imagery on top + 1â€“2 lines of metadata below. Resource cards are 3:2 imagery with a longer description block.

### Whitespace Philosophy
Whitespace is structural, not atmospheric. Sections butt against each other with `{spacing.section}` rhythm â€” there are no decorative dividers, no empty "breathing room" bands, no gradient transitions between sections. The sense of air comes from `{colors.canvas}` body sections sandwiched between `{colors.surface-dark}` chapter blocks, not from generous padding inside any one component.

## Elevation & Depth

| Level | Treatment | Use |
|---|---|---|
| 0 â€” Flat | No border, no shadow | Canvas-on-canvas blocks, hero chapter content, footer column body |
| 1 â€” Hairline border | 1px solid `{colors.hairline}` | All cards on `{colors.canvas}`, table cells, comparison panels |
| 2 â€” Hairline strong | 1px solid `{colors.hairline-strong}` | Dividers on `{colors.surface-dark}` (footer column rules, dark-card edges) |
| 3 â€” Soft shadow | `0 0 5px 0 rgba(0,0,0,0.3)` | Sticky nav bottom edge when scrolled, sticky CTA bar â€” used very sparingly |
...
- **Decorative corner squares:** the small `{component.corner-square}` (~12px solid `{colors.primary}` square) anchored to the top-left or bottom-right corner of resource and feature cards â€” the system's only consistent ornamental device.
...
| `{rounded.sm}` | 2px | Every interactive element â€” buttons, cards, inputs, pill tabs, badges |
...
- **Decorative corner squares:** 12Ã—12px on standard cards, scaled to 16Ã—16 on hero callouts.
- **Avatar/social icons:** 32â€“40px circles with 1px hairline.
...
**`button-primary`** â€” the universal NVIDIA CTA
...
- Pressed state lives in `button-primary-active` â€” background drops to `{colors.primary-dark}` (`#5a8d00`) with the same text color.

**`button-outline`** â€” secondary on light canvas
...
- The system's most distinctive secondary CTA: a clear pane bordered in NVIDIA Green. Used for "Read the Documentation", "Watch the Video", "Compare Products" â€” second-tier actions that still earn the brand color.

**`button-outline-on-dark`** â€” outline on `{colors.surface-dark}`
...
**`button-ghost-link`** â€” inline arrow link
- Text `{colors.primary}` with a small right-arrow icon, type `{typography.button-md}`, no background, no border, rounded `{rounded.none}`.
- "Learn More â†’" affordance sitting at the bottom of resource cards and long-form section blocks. The arrow is uppercase and bold per `{typography.caption-md}`-equivalent treatment.

**`button-disabled`**
- Background `{colors.surface-soft}`, text `{colors.ash}`, rounded `{rounded.sm}` â€” flat gray.
...
- Active: background `{colors.ink}`, text `{colors.on-dark}` â€” the tab flips inverted on selection. Used in the "Latest in AI Resources" filter strip and similar segmented controls.
...
- Focused: same surface, border becomes 2px solid `{colors.primary}` â€” the green border is the only focus signal in the system.

**`search-input`**
- Used in the global search overlay â€” same treatment as `text-input` but at 40px height with a magnifier glyph at left.

### Cards

**`product-card`**
- Container: background `{colors.canvas}`, 1px solid `{colors.hairline}`, padding `{spacing.xl}` (24px), rounded `{rounded.sm}`.
- Layout: 16:9 product image at top, `{typography.card-title}` title, `{typography.body-sm}` description, `{component.button-ghost-link}` "Learn More â†’" affordance at bottom.
...
- Layout: icon (Font Awesome at 22â€“24px) at top in `{colors.primary}` followed by `{typography.heading-md}` title and `{typography.body-md}` body.
...
- Footer: ghost-link "Read More â†’" with right-pointing chevron in `{colors.primary}`.

**`callout-stat`**
- Background `{colors.canvas}` with 1px hairline `{colors.hairline}`, padding `{spacing.xxl}`, rounded `{rounded.sm}`.
- Massive `{typography.display-lg}` (36px) numeric in `{colors.primary}` followed by `{typography.body-md}` caption underneath ("4Ã— faster training", "60% lower cost", etc.). Used inside long-form industry pages.
...
**`corner-square`**
- 12Ã—12px solid `{colors.primary}` square anchored to a card corner. The brand's signature ornamental motif.
...
- Section-specific nav anchored above content (e.g., "Healthcare â†’ Drug Discovery / Medical Imaging / Genomics / Patient Care").
...
**`link-inline`**
- Body-prose anchor link: `{colors.link-blue}` text with underline. The ONLY blue in the system â€” appears nowhere except inline links inside `{typography.body-md}` paragraphs.
...
- Stack hero/footer chapters in `{colors.surface-dark}` and body sections in `{colors.canvas}` â€” alternate them in a predictable rhythm down the page.
...
- Don't put `{component.button-primary}` on a `{colors.canvas}` background where green-on-white would clash with photo content â€” use `{component.button-outline}` instead and reserve fill for dark surfaces.
...
| desktop-large | 1440px | Default desktop layout â€” 4-up card grid, 6-col footer |
...
| mobile-narrow | 320px | Hero `{typography.display-xl}` scales from 48px â†’ 32px |

### Touch Targets
All interactive elements meet WCAG AA (â‰¥ 44Ã—44px). `{component.button-primary}` sits at 44px height with 24px horizontal padding. `{component.text-input}` sits at 44px. `{component.pill-tab}` sits at ~40px height with extended hit-target padding to 44px. `{component.button-outline}` matches the 44px standard. Footer links are 18â€“20px line-height with 8â€“12px vertical padding to keep tap targets at ~36â€“44px depending on link length.

### Collapsing Strategy
- **Primary nav:** desktop center cluster â†’ tablet hamburger drawer at 768px.
- **Card grid:** 4-up â†’ 3-up â†’ 2-up â†’ 1-up at 1024, 768, and 480px; gutters drop from 24px to 16px on mobile.
- **Footer:** 6-up link columns â†’ 2-up at tablet â†’ full accordion at mobile (each column header becomes a tap-to-expand row).
- **Hero copy:** desktop `{typography.display-xl}` (48px) â†’ tablet 36px â†’ mobile 32px; line-height holds at 1.25.
- **Sub-nav strip:** desktop horizontal anchor row â†’ tablet horizontal scroll â†’ mobile collapses into a select dropdown.
- **Section padding:** `{spacing.section}` (64px) desktop â†’ 48px tablet â†’ 32px mobile.
- **Long-form text:** desktop 60/40 body+sidebar â†’ tablet/mobile single-column with sidebar pushed to the bottom.
...
2. Reference component names and tokens directly (`{colors.primary}`, `{component.button-primary-active}`, `{rounded.sm}`) â€” do not paraphrase.
...
6. Keep `{colors.primary}` scarce per viewport â€” if more than one solid-green CTA appears in the same fold, neutralize one to `{component.button-outline}`.
...
- **Mobile screenshots not captured** â€” responsive behavior synthesizes NVIDIA's known mobile pattern (hamburger drawer, accordion footer, 1-up card grid, hero downscale) from desktop evidence and the documented breakpoint stack.
- **Hover states not documented** by system policy.
- **Dialog / modal styling** beyond the locale-selector overlay not visible in the captured surfaces.
- **Form field styling** for full sign-up / contact forms is not present in the captured surfaces â€” only inline search and basic text inputs are documented.
