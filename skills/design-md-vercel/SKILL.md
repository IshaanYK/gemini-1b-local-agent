...
components:
  ...
  ex-pricing-tier-featured:
    description: "Featured tier â€” polarity-flipped to ink primary with white text and white CTA."
    backgroundColor: "{colors.ink}"
    textColor: "{colors.on-primary}"
    rounded: "{rounded.lg}"
    padding: "{spacing.xl}"
  ex-product-selector:
    description: "What's Included summary card â€” repurposed for the brand's GPU / inference / Pro feature tiers."
    backgroundColor: "{colors.canvas-soft}"
    rounded: "{rounded.md}"
    padding: "{spacing.lg}"
  ex-cart-drawer:
    description: "Subscription summary â€” line items per add-on (NOT a literal e-commerce cart)."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.md}"
    padding: "{spacing.lg}"
  ...
  ex-auth-form-card:
    ...
    padding: "{spacing.xl}"
  ex-modal-card:
    description: "Modal dialog surface â€” same chrome as card-marketing-large with Level 5 modal shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.lg}"
    padding: "{spacing.xl}"
  ex-empty-state-card:
  ...
  ex-toast:
    description: "Toast notification surface â€” flat-cornered card-marketing chrome with Level 4 shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.md}"
...
- A multi-stop mesh gradient (cyan-blue-magenta-amber) is the only decorative chrome â€” used at hero scale and inside feature-band atmospheric backdrops. It is the brand.
...
- Subtle stacked-shadow elevation â€” three offsets layered with 4-12 % black opacity â€” never a single heavy drop-shadow.
- A complete 100â€“1000 gray + blue + red + amber + green + teal + purple + pink colour scale exists as a system token set, but the marketing surface uses only the `100`, `1000`, and `700`-level tones; the rest stay in the design-system tokens for in-product surfaces.
...
### Brand & Accent
- **Ink** (`{colors.primary}` â€” `#171717`): The single primary CTA color. Black-near-pure ink that carries every Sign Up pill, every footer CTA, the dark-band polarity-flip. Used as text color throughout the page on light surfaces. (Resolved from `--ds-gray-1000`.)
- **Cyan** (`{colors.cyan}` â€” `#50e3c2`): A signature mint-cyan used in the brand gradient and inside Geist-system spotlight tokens. Visible inside the hero gradient stops.
- **Highlight Pink** (`{colors.highlight-pink}` â€” `#ff0080`): The brand's highlight magenta, used as the high-saturation stop in the preview-gradient pair.
- **Violet** (`{colors.violet}` â€” `#7928ca`): The deep purple used as the start of the preview-gradient and inside developer-console highlights.
- **Link Blue** (`{colors.link}` â€” `#0070f3`): The brand's primary link color and the legacy `--geist-success` semantic.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): The pure-white card / dialog / modal surface.
- **Canvas Soft** (`{colors.canvas-soft}` â€” `#fafafa`): The default page background â€” 98 % white. Almost every section sits on this tone.
- **Canvas Soft 2** (`{colors.canvas-soft-2}` â€” `#f5f5f5`): A slightly deeper inset surface for "code editor inner background", template-card hover states, and dropdown menus.
- **Hairline** (`{colors.hairline}` â€” `#ebebeb`): 1 px dividers â€” table rows, card borders, input borders.
- **Hairline Strong** (`{colors.hairline-strong}` â€” `#a1a1a1`): The 500-level gray, used as the slightly-stronger divider on light bands and as the deemphasised text color.

### Text
- **Ink** (`{colors.ink}` â€” `#171717`): Every heading and body paragraph on light surfaces.
- **Body** (`{colors.body}` â€” `#4d4d4d`): Secondary text â€” sub-headings, body captions, nav-link inactive text, footer column body.
- **Mute** (`{colors.mute}` â€” `#888888`): Lowest-priority text â€” placeholder text, fine print, low-key labels.
- **On Primary** (`{colors.on-primary}` â€” `#ffffff`): All text on `{colors.primary}` surfaces.

### Semantic
- **Success / Link** (`{colors.success}` â€” `#0070f3`): The brand's legacy success indicator doubles as the primary link color. Visible underline-on-hover for inline body links.
- **Link Deep** (`{colors.link-deep}` â€” `#0761d1`): The pressed / visited tone for inline links.
- **Link Bg Soft** (`{colors.link-bg-soft}` â€” `#d3e5ff`): Soft pastel blue fill for "what's new" pill banners and informational badges.
- **Error** (`{colors.error}` â€” `#ee0000`): Validation red for destructive actions and form errors.
- **Error Soft** (`{colors.error-soft}` â€” `#f7d4d6`): Soft pastel red for destructive-state backgrounds.
- **Error Deep** (`{colors.error-deep}` â€” `#c50000`): Pressed / deep destructive state.
- **Warning** (`{colors.warning}` â€” `#f5a623`): Caution / pending status indicator.
- **Warning Soft** (`{colors.warning-soft}` â€” `#ffefcf`) / **Warning Deep** (`{colors.warning-deep}` â€” `#ab570a`): Background + pressed variants.

### Brand Gradient
The brand's signature decoration is a three-pair gradient stack:
- **Develop** (`{colors.gradient-develop-start}` `#007cf0` â†’ `{colors.gradient-develop-end}` `#00dfd8`) â€” the blue-to-teal pair used to mark the "deploy" / "develop" rhythm.
- **Preview** (`{colors.gradient-preview-start}` `#7928ca` â†’ `{colors.gradient-preview-end}` `#ff0080`) â€” the violet-to-pink pair used for "preview" surfaces.
- **Ship** (`{colors.gradient-ship-start}` `#ff4d4d` â†’ `{colors.gradient-ship-end}` `#f9cb28`) â€” the coral-to-amber pair used for "ship" surfaces.

The three pairs collapse into a single multi-color mesh gradient when used as the hero atmospheric backdrop. Treat the gradient as one unified object â€” do not crop down to a single colour, do not reorder the stops, and do not miniaturise. Used at hero scale only.
...
2. **A custom monospaced face** (extracted as `Geist Mono`) for terminal mockups, code blocks, and small mono-caption labels â€” anything that wants to signal "technical." Weight 400 only at 12 â€“ 13 px. Tracking neutral.
...
- **Sentence-case headlines, period-terminated.** Headlines like "Build and deploy on the AI Cloud." end with a deliberate period â€” that punctuation is part of the brand's voice.
...
- **Geometric sans** â€” *Inter* (400 / 500 / 600) is the closest stylistic match; `font-feature-settings: "ss01", "ss02"` enables the geometric alternates. *Satoshi* is a passable second choice.
- **Monospace** â€” *JetBrains Mono* (400) at 12 â€“ 13 px matches the technical voice. *IBM Plex Mono* is the second-best option.

## Layout

### Spacing System
- **Base unit**: 4 px. The brand's `--geist-space` token is exactly 4 px and every captured value is a multiple of 4.
- **Tokens**: `{spacing.xxs}` 4 px Â· `{spacing.xs}` 8 px Â· `{spacing.sm}` 12 px Â· `{spacing.md}` 16 px Â· `{spacing.lg}` 24 px Â· `{spacing.xl}` 32 px Â· `{spacing.2xl}` 40 px Â· `{spacing.3xl}` 48 px Â· `{spacing.4xl}` 64 px Â· `{spacing.5xl}` 96 px Â· `{spacing.6xl}` 128 px Â· `{spacing.section}` 192 px.
...
The mesh gradient does most of the heavy decorative lifting; whitespace separates the bands. Section spacing is generous â€” `{spacing.4xl}` to `{spacing.5xl}` between bands lets the gradient breathe. Inside a card, the headline/paragraph stack is tight (`{spacing.xs}` 8 px gap), then a wider gap before the CTA cluster. The page reads as engineered â€” large gaps + tight interior, never the other way around.
...
| Tablet | 600â€“959px | 3-up grids drop to 2-up; nav still horizontal. |
| Desktop | 960â€“1199px | Full 3-up grids; pricing 3-up. |
| Wide | 1200â€“1399px | Container caps at 1400 px content width. |
| Ultra-wide | â‰¥ 1400px | Content stays centred at 1400 px; bands stretch edge-to-edge in colour but content holds the max-width. |

#### Touch Targets
The `button-primary` pill renders at ~32 px tall in nav and ~48 px tall in marketing contexts. Marketing CTAs comfortably meet WCAG AAA at all breakpoints; nav buttons inflate touch area through `{spacing.xs}` padding on mobile to meet the 44 Ã— 44 px floor.
...
- **Three-feature row**: 3-up â†’ 2-up â†’ 1-up at the breakpoints above; cards keep their `{rounded.md}` 8 px shape across all viewports.
- **Pricing card grid**: 3-up at desktop, vertical stack at mobile with `pricing-card-featured` always sitting in the middle.
- **Template grid**: 5-up â†’ 3-up â†’ 2-up â†’ 1-up. Each `template-card` keeps its 16:9 aspect on the image.
...
| Level | Treatment | Use |
|---|---|---|
| Level 0 â€” Flat | No shadow, no border. | Full-bleed hero bands and the polarity-flipped dark sections. |
| Level 1 â€” Inset Hairline | `0 0 0 1px #00000014` inset 1 px border. | Default card chrome â€” the brand's universal "you can see this card" cue. |
| Level 2 â€” Subtle Drop | `0px 1px 1px #00000005, 0px 2px 2px #0000000a` plus inset hairline. | Slightly elevated cards (template-grid, marketing-card). |
| Level 3 â€” Soft Stack | `0px 2px 2px #0000000a, 0px 8px 8px -8px #0000000a` plus inset hairline. | The "medium" elevation â€” feature-grid cards. |
| Level 4 â€” Float Stack | `0px 2px 2px #0000000a, 0px 8px 16px -4px #0000000a` plus inset hairline. | "Large" elevation â€” pricing cards, callout panels. |
| Level 5 â€” Modal | `0px 1px 1px #00000005, 0px 8px 16px -4px #0000000a, 0px 24px 32px -8px #0000000f` plus inset hairline. | Modal / dialog surfaces and dropdown menus. |

The brand uses STACKED shadows â€” multiple small offsets layered to fake natural light â€” never a single 8-px-blur generic drop. Inset hairline rings are always added so the card edge stays crisp.

### Decorative Depth
- **Mesh gradient as atmospheric depth**: the hero's multi-stop gradient is the brand's only "atmospheric" effect â€” applied as a flat 2-D backdrop rather than a 3-D illustration.
...
| `{rounded.xs}` | 4px | Tightest inline pill â€” the `nav-cta-signup` 6-px-radius button (mapped to `xs/sm`). |
| `{rounded.sm}` | 6px | The brand's `--geist-radius` token â€” base UI radius for in-app buttons, form inputs, dropdown menus. |
| `{rounded.md}` | 8px | The brand's `--geist-marketing-radius` token â€” feature cards, template cards. |
| `{rounded.lg}` | 12px | Slightly larger card chrome (pricing-card variants). |
| `{rounded.xl}` | 16px | Largest card chrome â€” when a card hosts a hero image cap. |
| `{rounded.pill-sm}` | 64px | Tab-ghost pills inside the "AI Apps / Web Apps / Ecommerce / Marketing / Platforms" row. |
| `{rounded.pill}` | 100px | The marketing CTA pill â€” `button-primary`, `button-secondary`, "Start Deploying" pill. |
...
**`button-primary`** â€” the canonical 100-px-radius black pill, marketing scale.
...
**`button-secondary`** â€” the white pill paired with the black primary inside marketing bands.
- Background `{colors.canvas}`, text `{colors.ink}`, same typography + padding as `button-primary`, shape `{rounded.pill}`.

**`button-primary-sm`** â€” the smaller-scale primary pill used inside nav and pricing-card CTAs.
- Background `{colors.primary}`, text `{colors.on-primary}`, label set in `{typography.button-md}` (14 px / 500), shape `{rounded.pill}`.

**`button-secondary-sm`** â€” the smaller-scale white pill paired with `button-primary-sm`.
- Background `{colors.canvas}`, text `{colors.ink}`, same typography + shape as `button-primary-sm`.

**`tab-ghost`** â€” the centred-row tab pill ("AI Apps / Web Apps / Ecommerce / Marketing / Platforms").
...
**`icon-button-circular`** â€” the circular icon container (often a "?" or arrow inside).
- Background `{colors.canvas}`, dark icon, 1 px solid hairline border, shape `{rounded.full}`.

**Nav CTAs:**

**`nav-cta-signup`** â€” the small black "Sign Up" button in the nav row.
...
**`nav-cta-login`** â€” the white "Log In" button in the nav.
- Background `{colors.canvas}`, text `{colors.ink}`, same typography / height / shape as `nav-cta-signup`.

**`nav-cta-ask-ai`** â€” the small "Ask AI" button with a faint border.
...
**`card-marketing`** â€” the canonical marketing feature card (3-up section cards).
...
**`card-marketing-large`** â€” the larger marketing card used for "compute model" / "AI Gateway" callouts.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.xl}`, shape `{rounded.lg}` 12 px. Carries Level 4 float-stack shadow.

**`card-soft`** â€” the soft-tinted card used inside cluster groups (lighter than canvas-soft).
- Background `{colors.canvas-soft}`, text `{colors.ink}`, padding `{spacing.lg}`, shape `{rounded.md}`.

**`template-card`** â€” the deploy-template card in the "Deploy your first app" grid.
...
**`code-editor-mockup`** â€” the dark code-preview surface inside marketing bands.
...
**`pricing-card`** â€” the default pricing-tier card.
...
**`pricing-card-featured`** â€” the polarity-flipped "Pro" tier card.
...
**`form-input`** â€” the canonical text input.
...
**`nav-link`** â€” the centred link row inside `nav-bar`.
- Text `{colors.body}`, set in `{typography.body-sm}`, padding `{spacing.xs} {spacing.sm}`, shape `{rounded.full}` (ghost pill â€” visible only on hover or active, but the radius is documented).

**`footer`** â€” the bottom 4-column nav.
...
**`hero-band`** â€” the white hero with the mesh gradient backdrop.
...
**`feature-mesh-band`** â€” the secondary section that hosts a mesh-gradient atmospheric backdrop with feature copy on top.
...
**`showcase-band-light`** â€” a soft-canvas section ("Deploy your first app in seconds").
- Background `{colors.canvas-soft}`, text `{colors.ink}`, padding `{spacing.5xl} {spacing.lg}`.

**`showcase-band-dark`** â€” the polarity-flipped dark band ("A compute model for all workloads").
...
**`logo-strip`** â€” the customer-logo wrapping row near the top of the page.
...
**`badge-secondary`** â€” the small inline metadata pill ("New", "Beta", "Live").
- Background `{colors.canvas-soft}`, text `{colors.body}`, body in `{typography.caption}`, padding `0px {spacing.xs}`, shape `{rounded.full}`.

**`banner-marketing`** â€” the "Introducing X" announcement pill at the top of pages.
...
**`link-inline`** â€” body-copy inline links.
...
- Use the brand mesh gradient as atmospheric decoration at hero scale only â€” never miniaturise it to an icon, never reduce to a single colour.
...
- Cycle page surfaces in `{colors.canvas-soft}` â†’ `{colors.canvas}` â†’ `{colors.primary}` polarity-flipped bands; the dark band IS the depth cue.
...
- Don't pair the marketing 100-px pill CTA shape with the 6-px nav radius on the same screen â€” pick a scale and stay there.
