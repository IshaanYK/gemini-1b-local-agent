---
name: design-md-meta
description: Design system and styling guidelines based on meta. Use this skill when asked to create a design inspired by meta.
---
---
version: alpha
...
- Photographic feature cards with no card chrome (no border, no shadow) â€” the product imagery IS the surface treatment

## Colors

> Source pages: meta.com/ (homepage), /ai-glasses/ray-ban-meta-skyler-gen-2/ (PDP), /quest/quest-3s/buy-now/ (configurator), /ai-glasses/prescription/ (lens upsell). Token coverage was identical across all four pages â€” the design system is genuinely unified.
...
- **Oculus Purple** ({colors.oculus-purple}): VR product accent â€” used inside Quest-branded surfaces for category emphasis.
...
- **Warning** ({colors.warning}): Promotional banners ("Get 25% offâ€¦") and limited-time tags.
...
### Font Family
**Optimistic VF** is Meta's proprietary variable display face. Fallbacks: Montserrat, Helvetica, Arial, Noto Sans. The variable axes carry from 300 (light heading-md, used for editorial intro headlines like "Look forward") through 500 (display, hero, heading-sm) up to 700 (subtitle, body emphasis, button labels). Stylistic sets `ss01` and `ss02` are switched on across every heading role â€” they soften the geometry and give the type a slightly humanist breathing.
...
|---|---|---|---|---|---|---|
| `{typography.hero-display}` | 64px | 500 | 1.16 | 0 | ss01, ss02 | Homepage hero ("Get 25% offâ€¦", category opener) |
...
| `{typography.subtitle-lg}` | 18px | 700 | 1.44 | 0 | â€” | Bold callouts, FAQ question titles |
| `{typography.subtitle-md}` | 18px | 400 | 1.44 | 0 | â€” | Body lead and longer-line subtitles |
| `{typography.body-md}` | 16px | 400 | 1.50 | -0.16px | â€” | Primary body text |
| `{typography.body-md-bold}` | 16px | 700 | 1.50 | -0.16px | â€” | Body emphasis and link-md |
| `{typography.body-sm}` | 14px | 400 | 1.43 | -0.14px | â€” | Secondary body, helper text |
| `{typography.body-sm-bold}` | 14px | 700 | 1.43 | -0.14px | â€” | Pill tab labels, footer headings |
| `{typography.caption-bold}` | 12px | 700 | 1.33 | 0 | â€” | Badge labels, timestamps |
| `{typography.caption}` | 12px | 400 | 1.33 | 0 | â€” | Footer fine print, legal microcopy |
| `{typography.button-md}` | 14px | 700 | 1.43 | -0.14px | â€” | Pill button labels |
| `{typography.link-md}` | 16px | 700 | 1.50 | -0.16px | â€” | Inline navigation links |

### Principles
- Negative letter-spacing on body roles (`-0.14px` to `-0.16px`) tightens the type fractionally â€” Optimistic VF was designed for this snug-but-not-condensed setting.
...
- All headings carry `ss01, ss02` stylistic sets together â€” the design treats them as a paired alternates package, never one without the other.
...
- **Tokens**: `{spacing.xxs}` (4px) Â· `{spacing.xs}` (8px) Â· `{spacing.sm}` (10px) Â· `{spacing.md}` (12px) Â· `{spacing.base}` (16px) Â· `{spacing.lg}` (20px) Â· `{spacing.xl}` (24px) Â· `{spacing.xxl}` (32px) Â· `{spacing.xxxl}` (40px) Â· `{spacing.section-sm}` (48px) Â· `{spacing.section}` (64px) Â· `{spacing.section-lg}` (80px) Â· `{spacing.hero}` (120px).
...
### Grid & Container
- Marketing page max-width sits around 1280px with 32â€“48px gutters.
...
Whitespace is product-photography-first. Hero sections give product imagery 50â€“70% of the viewport height; copy is given oxygen to breathe in `{spacing.xxl}` to `{spacing.xxxl}` blocks above and below. Inside the configurator, whitespace tightens â€” the buy-now panel is information-dense, with `{spacing.base}` to `{spacing.lg}` rhythm between option groups.
...
- Decorative pastel tints inside accessory cards â€” soft pink, ice-blue, mint â€” appear briefly behind product cutouts but are NOT formalized as system tokens (treated as photographic content).
...
- Six-up "color & SKU" picker rows use 1:1 aspect tiles with `{rounded.lg}` (8px) corners â€” tighter than the hero photography frames to differentiate selection-grid context from showcase context.
...
**`button-primary`** â€” Black pill primary CTA for marketing surfaces ("Shop", "Pre-order").
...
**`button-buy-cta`** â€” Cobalt pill primary CTA for commerce flows ("Add to cart", "Configure", "Continue").
...
**`button-secondary`** â€” Outlined ghost CTA, often paired with primary in dual-CTA hero patterns.
...
**`button-ghost`** â€” Quieter outlined variant used for tertiary CTAs.
...
**`button-pill-tab`** + **`button-pill-tab-active`** â€” Top-of-page category navigation pills ("Glasses / Quest / Apps").
- Inactive: background `{colors.canvas}`, text `{colors.ink}`, border `1px solid {colors.hairline}`, padding `8px 16px`, rounded `{rounded.full}`.
- Active: background `{colors.ink-deep}`, text `{colors.canvas}`. No border in active state â€” the dark fill replaces it.

**`button-icon-circular`** â€” 40Ã—40px circular utility buttons (carousel arrows, share, favorite).
- Background `{colors.canvas}`, icon color `{colors.ink}`, rounded `{rounded.circle}`.

### Cards & Containers

**`card-product-feature`** â€” White feature card with product photography and copy (homepage "Designed for sport", "Advanced. Inside and out.").
- Background `{colors.canvas}`, rounded `{rounded.xxxl}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline-soft}`.

**`card-feature-photo`** â€” Edge-to-edge photographic showcase tile with NO chrome (homepage "Built for prescriptions" full-bleed glasses card).
...
**`card-promo-strip`** â€” Dark full-width promo card with embedded copy + CTAs (homepage "Meta Quest brings the magic of virtual reality" wide strip).
- Background `{colors.ink-deep}`, text `{colors.canvas}`, rounded `{rounded.xxxl}`, padding `{spacing.section}`.

**`card-icon-feature`** â€” Three-up feature tile with line icon, headline, and short copy ("Free 2-day delivery", "Free 30-day returns", "Worry-free warranty", "Buy now, pay later").
- Background `{colors.canvas}`, rounded `{rounded.xl}`, padding `{spacing.xl}`, border `1px solid {colors.hairline-soft}`.

**`card-checkout-summary`** â€” PDP right-rail sticky summary with title, price, color picker, "Add to cart" button.
...
**`product-thumbnail`** â€” Square product image cell used in color/SKU pickers and "People also bought" rows.
- Background `{colors.surface-soft}`, rounded `{rounded.xl}`, padding `{spacing.base}`, aspect-ratio `1 / 1`.

**`warranty-card`** â€” Promo callout for warranty + finance offers ("1y Warranty", "Meta Horizon+").
- Background `{colors.surface-soft}`, rounded `{rounded.xxl}`, padding `{spacing.xxl}`. Uses pastel-tinted variants for additional perks.

**`why-buy-tile`** â€” 4-up reassurance tile row in the lower marketing zone.
...
**`text-input`** â€” Standard form field (footer email subscribe, support form).
...
**Top Navigation (Desktop)** â€” Sticky white bar with category pill tabs, search, account, cart.
- Background `{colors.canvas}`, height ~64px with bottom `1px solid {colors.hairline-soft}`.
- Left: Meta wordmark logo (61Ã—14px). Center: pill-tab category nav. Right: search-pill + circular icon buttons (account, cart).

**Top Navigation (Mobile)** â€” Compressed to logo + hamburger + cart icon. Pill-tab nav slides into a full-screen drawer below 768px.

**Promo Banner** â€” Full-width strip ABOVE the nav for time-bound offers.
...
**Breadcrumb (PDP)** â€” Inline path above the product hero ("Glasses â€º Ray-Ban Meta â€º Skyler (Gen 2)").
- Typography `{typography.body-sm}`, separator dot in `{colors.stone}`, active leaf in `{colors.ink}`, parent links in `{colors.steel}`.

### Signature Components

**`hero-band-marketing`** â€” Full-bleed photographic hero with overlaid copy + dual-CTA pair.
...
**`product-gallery-pdp`** â€” Product detail page main hero: 4-up vertical thumbnail strip on the left, large product image center, sticky purchase rail right.
- Thumbnails: 80Ã—80px, `{rounded.lg}`, `{colors.surface-soft}` background, 1px `{colors.hairline-soft}` border (active border switches to `{colors.ink-deep}`).
- Main image area: ~720Ã—720px on desktop, `{rounded.xxxl}` rounding, photographic surface.
- Sticky rail uses `card-checkout-summary`.

**`color-sku-picker-row`** â€” Six-up grid of square product variants with name + price below each.
...
**`feature-icon-row`** â€” Four reassurance benefits ("Free 2-day delivery", "Free 30-day returns", "Worry-free warranty", "Buy now, pay later").
...
**`faq-accordion`** â€” Vertical stack of expandable Q&A items.
...
**`tech-specs-table`** â€” Two-column key/value table for product specifications.
...
**`testimonial-customer-card`** â€” Small editorial card with author + quote + photo.
...
**`footer-region`** â€” Dense multi-column site footer.
...
### Do
- Reserve `{colors.primary}` (cobalt) for buy-now CTAs only â€” its visual weight is meaningful precisely because it doesn't appear on marketing pages.
...
- Apply `{rounded.full}` to every button, every category pill, every badge, every chip â€” buttons are NEVER squared in Meta's system.
...
- Use the 300-weight `{typography.heading-md}` for editorial subheads â€” it creates the brand's signature visual rhythm against the 500-weight displays.

### Don't
- Don't use `{colors.primary}` (cobalt) for marketing-surface primary buttons â€” it conflicts with Meta's brand-history positioning of black-CTA-on-white-canvas marketing.
...
- Don't run feature cards without rounding â€” `{rounded.xxxl}` is the minimum for any photographic surface.
- Don't reduce `{typography.body-md}` line-height below 1.50 â€” the negative letter-spacing already tightens the metric and any further compression breaks legibility.
...
| Mobile (large) | 480 â€“ 767px | Same as small but feature tiles render two-up. |
| Tablet | 768 â€“ 1023px | Two-column feature grids. Pill-tab nav returns. PDP gallery + purchase rail render side-by-side at compressed proportions (~60/40). |
| Desktop | 1024 â€“ 1359px | Full three- and four-up feature grids; full pill-tab category nav; PDP at standard 58/42 split. |
| Wide Desktop | â‰¥ 1360px | Same as desktop with wider hero gutters and larger product photography. |

### Touch Targets
- Pill buttons render at 40â€“44px effective height (with the 14px button text + `14px 30px` padding). Above the WCAG AAA 44px floor.
- Circular icon buttons are 40Ã—40px â€” at the AA floor; bumps to 44Ã—44px on mobile via override.
- Color swatch circles are 32Ã—32px. To hit AAA, the swatch carries a 12px clear hit zone around it (effective hit target ~56px).
...
- **Feature grids** (3-up, 4-up) collapse to 2-up at 768â€“1023px and 1-up at < 768px. Card padding compresses from `{spacing.xxl}` to `{spacing.xl}` at the 1-up breakpoint.
...
- Product variant images preserve their `{rounded.lg}` thumbnails across all breakpoints â€” they never go full-width on mobile.

## Iteration Guide

1. Focus on ONE component at a time. The system has high internal consistency â€” small precision wins compound.
2. Reference component names and tokens directly (`{colors.primary}`, `{component-name}-pressed`, `{rounded.full}`) â€” do not paraphrase.
3. Run `npx @google/design.md lint DESIGN.md` after edits to catch broken refs, contrast issues, orphaned tokens.
4. Add new variants as separate `components:` entries (`-pressed`, `-disabled`, `-focused`) â€” do not bury them inside prose.
5. Default to `{typography.body-md}` for body and `{typography.subtitle-lg}` for emphasis. Headlines step down through `hero-display â†’ display-lg â†’ heading-lg â†’ heading-md â†’ heading-sm`.
...
- Selected/checked states for non-button form controls (toggle, multi-select) were not visible on the captured surfaces â€” implement following the cobalt-on-white pattern from `radio-option-selected`.
- Animation/transition timings are not extracted; recommend 150â€“250ms ease-out for primary surface transitions and 300ms ease-in-out for accordion expand/collapse.
...
- Pastel decorative tints inside accessory cards (soft pink, ice blue, mint) appear visually but are not formalized â€” treat them as photographic content, not as system colors.
