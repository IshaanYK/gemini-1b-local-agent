...
description: Design system and styling guidelines based on minimax. Use this skill when asked to create a design inspired by minimax.
---
---
version: alpha
name: MiniMax-design-analysis
description: MiniMax presents itself as a premium AI infrastructure brand through a striking duality â€” bold black-pill CTAs and stark white canvas for marketing, paired with vibrant gradient product cards (orange-red, magenta-pink, purple, blue) that turn each model release into a distinctive visual identity. The system uses DM Sans across all surfaces, employs an oversized 80px hero display, anchors major actions in deep near-black pills, and layers content density via a 3-column documentation grid with sidebar nav, prose body, and TOC. Coverage spans the marketing homepage, model showcase pages, developer documentation, and platform pricing surfaces.
...
MiniMax stages itself as a Chinese AI infrastructure brand with a sophisticated dual identity. Marketing surfaces and platform pages anchor in stark white canvas with deep-black typographic emphasis â€” the brand voice is confident, technical, almost editorial. But each model release gets its own vibrant gradient identity card: M2.7 in volcanic coral-red, Music 2.6 in magenta-pink, Hailuo in deep blue, Speech 2.8 in saturated orange-purple. Together these vibrant tiles read like album covers laid out on the homepage â€” each one declaring its own product personality.
...
**Key Characteristics:**
- Stark monochrome palette â€” black ({colors.primary}) and white ({colors.canvas}) â€” broken open by saturated brand-color gradient cards
...
### Text
- **Ink** ({colors.ink}): Primary headline and CTA text â€” the brand's near-black anchor.
...
- **Single typeface** strategy â€” never mix DM Sans with another sans-serif. Code samples (when shown) use a system monospace fallback, but no second typeface enters the brand canvas.

## Layout

### Spacing System
- **Base unit**: 4px (8px primary increment).
- **Tokens**: `{spacing.xxs}` (4px) Â· `{spacing.xs}` (8px) Â· `{spacing.sm}` (12px) Â· `{spacing.md}` (16px) Â· `{spacing.lg}` (20px) Â· `{spacing.xl}` (24px) Â· `{spacing.xxl}` (32px) Â· `{spacing.xxxl}` (40px) Â· `{spacing.section-sm}` (48px) Â· `{spacing.section}` (64px) Â· `{spacing.section-lg}` (80px) Â· `{spacing.hero}` (96px).
...
- **Card internal padding**: Vibrant product cards use `{spacing.xxl}` (32px); documentation cards use `{spacing.lg}â€“{spacing.xl}` (20â€“24px); promo strips expand to `{spacing.section}` (64px).

### Grid & Container
- Marketing pages use a 1280px max-width with 32px gutters.
- Homepage product matrix renders as a 4-column row of 32px-rounded gradient cards, each ~280â€“320px wide.
...
Marketing pages give product photography and color cards generous breathing room â€” `{spacing.hero}` (96px) above-the-fold creates visual oxygen for the 80px hero display. Inside documentation, whitespace tightens dramatically: section gaps drop to `{spacing.xxl}` (32px), table rows pack down to `{spacing.md}` (16px), and the sidebar nav uses `{spacing.xs}` (8px) vertical rhythm.
...
### Decorative Depth
- The vibrant gradient product cards carry their own atmospheric depth via internal radial gradients and silhouette imagery â€” no shadow needed; the color does the work.
...
- Vibrant product cards use 32px corner softening â€” distinct from the 16px used on quiet white cards. The doubled radius is the visual signature of "this is a featured product moment."
...
- Avatar circles (rare, in testimonials) are `{rounded.full}` â€” perfect circles.
...
**`button-secondary`** â€” Outlined pill secondary action, paired with primary in dual-CTA hero patterns.
...
**`button-tertiary`** â€” White-fill quieter pill, used for tertiary nav and informational CTAs.
...
**`button-icon-circular`** â€” 36Ã—36px circular utility button (carousel arrows, share, copy).
- Background `{colors.canvas}`, text `{colors.ink}`, border `1px solid {colors.hairline}`, rounded `{rounded.full}`.

### Vibrant Product Cards

**`product-card-coral`** â€” M2.7 / Token Plan signature card.
...
**`card-feature`** â€” Quieter feature panel on light gray.
- Background `{colors.surface}`, rounded `{rounded.xl}`, padding `{spacing.xxl}`.

**`card-recommendation`** â€” "Recommended Reading" tile in documentation footer.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.lg}`, border `1px solid {colors.hairline}`.

**`promo-cta-card`** â€” Bright orange "Refunds of 10%..." promo strip with embedded CTA pill.
...
**`ai-product-tile`** â€” White card in the AI Product Matrix grid (Agent, Hailuo Video, MiniMax Audio).
...
**`search-pill`** â€” Documentation top-bar search field.
...
**`segmented-tab`** + **`segmented-tab-active`** â€” Underline-style tab navigation (Benchmark / Self-Evaluation / Multi-Agent Collaboration on the M2.7 page).
...
**`pill-tab`** + **`pill-tab-active`** â€” Pricing-page tab nav (Token Plan / Audio Subscription / Video Package).
...
**`badge-success`** â€” Pale-green confirmation badge ("Available", "Active").
...
**`badge-new`** â€” Coral "NEW" / "Live" pill for fresh releases.
...
**`badge-beta`** â€” Pale-blue "BETA" / informational pill.
...
**`badge-code`** â€” Inline code-style chip ("Code", "API").
...
**`promo-banner`** â€” Sticky black promotional strip ABOVE the top nav ("Invite & Earn â€” Rewards for Both!").
...
**Top Navigation (Documentation/Platform)** â€” Compressed nav with center search-pill and right-side account/upgrade CTAs.
...
**`doc-toc-item`** â€” Right-rail table-of-contents links.
...
**`hero-band-marketing`** â€” Centered hero with massive 80px display + dual-CTA pair.
...
**`product-matrix-grid`** â€” 4-column horizontal scroll of vibrant gradient product cards (homepage "Full-Stack Model Matrix").
...
- Card heights are uniform (~360â€“400px); the row scrolls horizontally on mobile.

**`ai-product-matrix`** â€” 4-column grid of white product tiles below the vibrant matrix (Agent / Hailuo Video / Audio / Video).
...
**`docs-prose-block`** â€” Documentation main content area.
...
**`models-comparison-table`** â€” Documentation table comparing model sizes and features.
...
**`testimonial-stat-row`** â€” Stats strip ("214,000+ Enterprise Clients & Developers", "0+ Countries Served").
...
**`footer-region`** â€” Dense black-canvas multi-column footer.
...
**`footer-link`** â€” Individual link entry inside the footer column.
- Background transparent, text `{colors.muted}`, typography `{typography.body-sm}`, padding `{spacing.xxs} 0`. Active/visited states do not change color â€” only opacity shifts on activation.

## Do's and Don'ts

### Do
- Use `{colors.primary}` (black) as the dominant CTA â€” it's the brand's most recognizable interactive element.
- Reserve product brand colors (`{colors.brand-coral}`, `{colors.brand-magenta}`, `{colors.brand-blue}`, `{colors.brand-purple}`) ONLY for product-identity moments â€” never for general buttons or text.
- Pair `{rounded.hero}` (32px) gradient cards with `{rounded.xl}` (16px) white cards in the same viewport â€” the radius contrast is the visual signature.
- Apply `{rounded.full}` to every button, every pill tab, every badge.
- Use `{typography.hero-display}` (80px) with -2px letter-spacing for hero displays â€” never compromise the leading or letter-spacing.
...
### Don't
- Don't use brand-coral or brand-magenta on body text or large surfaces â€” they lose meaning when overused.
...
- Don't reduce hero leading below 1.10 â€” the brand needs that breathing room on the 80px display.
...
| Mobile (large) | 480 â€“ 767px | Same as small but AI product matrix renders 2-up. |
| Tablet | 768 â€“ 1023px | 2-column AI product matrix. Pill-tab nav returns. Documentation sidebar collapses to drawer. |
| Desktop | 1024 â€“ 1279px | Full 4-column product matrix; 3-column docs grid (sidebar / body / TOC). |
| Wide Desktop | â‰¥ 1280px | Wider hero gutters, larger product photography, fixed 220px sidebar. |

### Touch Targets
- Pill buttons render at 38â€“40px effective height â€” bumps to 44px on mobile via padding override.
- Circular icon buttons: 36Ã—36px desktop â†’ 44Ã—44px on mobile.
- Form inputs render at 40px height; bumps to 44px on mobile.
- Sidebar nav items render at ~32px tall â€” bumps to 44px on mobile drawers.
...
- **Documentation grid**: 3-column desktop â†’ sidebar-drawer at < 1024px â†’ single-column with collapsible sidebar at < 768px.
- **Product matrix**: 4-column desktop â†’ horizontal-scroll at < 1024px (carousel-style with snap points).
- **AI Product Matrix**: 4-column â†’ 2-column at tablet â†’ 1-column at mobile.
- **Hero typography**: `{typography.hero-display}` (80px) â†’ 56px at < 1024px â†’ 40px at < 768px â†’ 32px at < 480px.
- **Stats strip**: 4-column â†’ 2Ã—2 at < 768px â†’ 1-column at < 480px.
...
5. Default to `{typography.body-md}` for body and `{typography.subtitle}` for emphasis. Headlines step down `hero-display â†’ display-lg â†’ heading-lg â†’ heading-md â†’ heading-sm`.
