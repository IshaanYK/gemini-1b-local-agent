...
components:
  ...
  ex-pricing-tier-featured:
    description: "Featured tier â€” polarity-flipped to canvas-dark with white text."
    backgroundColor: "{colors.ink}"
    textColor: "{colors.on-primary}"
    rounded: "{rounded.sm}"
    padding: "{spacing.3xl}"
  ex-product-selector:
    description: "What's Included summary card â€” repurposed for the brand's GPU / inference packaging tiers."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.sm}"
    padding: "{spacing.2xl}"
  ex-cart-drawer:
  ...
  ex-auth-form-card:
    ...
    padding: "{spacing.3xl}"
  ex-modal-card:
    description: "Modal dialog surface â€” same chrome as article-card; relies on tinted scrim instead of card shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.sm}"
    padding: "{spacing.3xl}"
  ex-empty-state-card:
  ...
  ex-toast:
    description: "Toast notification surface â€” flat-cornered article-card chrome with a soft brand-tinted drop shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.sm}"
    padding: "{spacing.md} {spacing.lg}"
    typography: "{typography.body-md}"
...
- A three-color brand gradient (`{colors.accent-orange}` â†’ `{colors.accent-magenta}` â†’ `{colors.accent-periwinkle}`) is the entire decorative system â€” used as the hero ribbon graphic and never reduced to a swatch elsewhere.
- All-caps mono eyebrows and button labels in `{typography.mono-caps-eyebrow}` / `{typography.mono-caps-button}` everywhere â€” section titles, model row headers, "ON-DEMAND" labels in pricing tables.
...
- Dual surface mode â€” alternating `{colors.canvas-dark}` and `{colors.canvas}` bands; no in-between greys. The single soft surface `{colors.hairline}` exists only to mark table-header rows.
...
### Brand & Accent
- **Ink Black** (`{colors.primary}` â€” `#000000`): The single primary CTA color. Black pill carries "Sign in", "Contact sales", "Get started now", every footer CTA.
- **Brand Orange** (`{colors.accent-orange}` â€” `#fc4c02`): One leg of the three-color brand gradient. Appears in the hero ribbon graphic; never used as a UI fill on its own.
- **Brand Magenta** (`{colors.accent-magenta}` â€” `#ef2cc1`): The second leg of the gradient.
- **Brand Periwinkle** (`{colors.accent-periwinkle}` â€” `#bdbbff`): The third leg of the gradient; also used as a soft fill for some stat tiles.
- **Brand Mint** (`{colors.accent-mint}` â€” `#c8f6f9`): A pastel cyan that lives outside the gradient â€” used for hero secondary-CTA pills and `stats-card-tinted` tiles.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): The default product / pricing / docs background.
- **Hairline / Canvas Soft** (`{colors.hairline}` â€” `#ebebeb`): The brand's single soft surface tone â€” used for data-table header rows, toggle-pill rails, and 1 px dividers between table rows.
- **Canvas Dark** (`{colors.canvas-dark}` â€” `#010120`): The brand's dark hero surface; appears on `hero-band-dark` and `research-band-dark`.
- **Hairline** (`{colors.hairline}` â€” `#ebebeb`): 1 px dividers on light surfaces â€” table rows, card chrome, badge borders.
- **Hairline on Dark** (`{colors.surface-dark-soft}` â€” `#26263a`): 1 px dividers and badge backgrounds on `{colors.canvas-dark}` surfaces; pre-blended from the brand's translucent-white-on-dark hairline.
- **Surface Dark Soft** (`{colors.surface-dark-soft}` â€” `#313641`): A slightly lighter dark fill used inside dark-band cards.

### Text
- **Ink** (`{colors.ink}` â€” `#000000`): Every heading and body paragraph on light surfaces.
- **Body** (`{colors.body}` â€” `#999999`): Secondary text â€” captions, table cell secondary values, footer link text. Pre-blended from the brand's translucent-black 40 % body color.
- **Body Muted** (`{colors.body}` â€” `#999999`): The all-caps mono-eyebrow text color on light surfaces also rides on this token â€” there is no separate "mute" tone, the brand keeps secondary text consistent with caption text.
- **On Dark** (`{colors.on-dark}` â€” `#ffffff`): All text on `{colors.canvas-dark}` surfaces.

### Semantic
The brand does not maintain a separate error / success palette in its public surface; validation cues use the primary black or the brand gradient depending on context. No explicit error red, success green, or warning yellow is documented here â€” adopting framework defaults is appropriate.

### Brand Gradient
The brand's signature decoration is a three-stop gradient drawn from `{colors.accent-orange}` â†’ `{colors.accent-magenta}` â†’ `{colors.accent-periwinkle}`, applied as the only piece of decorative chrome (the hero ribbon graphic). Treat the gradient as one unified object â€” do not crop it down to a single colour, do not reorder the stops, and do not add a fourth stop. Used at large scale; never miniaturised to icon size.
...
2. **An uppercase mono caption face** (extracted as `PP Neue Montreal Mono`) for every eyebrow, button label, table-header cell, and pricing-table tab. Weight 500 at 11â€“16 px; always uppercase; positive letter-spacing (`0.05 â€“ 0.55 px`). The mono carries the brand's technical voice â€” every label that says "PRICING", "INFERENCE", "MODEL", "GPU", "GA-DEC '25" is set in this face.
...
| `{typography.mono-caps-button}` | 16px | 500 | 16px | 0.08px | Primary button labels â€” uppercase, mono. |
...
- **Display sans** â€” *Inter* (400 / 500) with `font-feature-settings: "ss01"` enabled comes closest; tighten letter-spacing by ~0.6 % at display sizes to land on the brand's compressed feel. *Geist* is the second-best option but reads slightly wider.
- **Uppercase mono eyebrow** â€” *JetBrains Mono* or *Geist Mono* (weight 500) at 11 px with `text-transform: uppercase` matches the brand's voice once tracking is bumped to `0.04em`.
...
- **Tokens**: `{spacing.xxs}` 2 px Â· `{spacing.xs}` 4 px Â· `{spacing.sm}` 8 px Â· `{spacing.md}` 12 px Â· `{spacing.lg}` 16 px Â· `{spacing.xl}` 20 px Â· `{spacing.2xl}` 24 px Â· `{spacing.3xl}` 32 px Â· `{spacing.4xl}` 44 px Â· `{spacing.5xl}` 48 px Â· `{spacing.6xl}` 55.2 px Â· `{spacing.section}` 80 px.
...
Surface contrast does most of the separation. A dark band ends â†’ 80 px of breathing room â†’ next light band begins. Inside a band, headline and lead paragraph hug close (`{spacing.lg}` 16 px between them), then a wider gap before the supporting visual or CTA cluster. Inside pricing data tables, the brand keeps rows tight (`{spacing.md}` 12 px vertical) â€” the table reads more like a sheet than a marketing component.
...
| Mobile-Large | 479â€“767px | Same as Mobile; some tables enable horizontal scroll. |
| Tablet | 768â€“991px | Article grid moves to 2-up; testimonial grid stays 3-up only if container > 900 px, otherwise 1-up. |
| Desktop | 992â€“1279px | Full 3-up research grid, 2-up article grid, hero 50/50 split. |
| Desktop-Large | â‰¥ 1280px | Container caps at 1280 px; bands stay edge-to-edge in colour while content centres. |

#### Touch Targets
The mono-cap button label is set at 16 px; combined with `{spacing.xs}` 4 px top / bottom and a 24 px horizontal padding, the primary pill renders at roughly 32 px tall. On mobile viewports, button height is inflated to â‰¥ 44 px through extra vertical padding inside the touch row â€” meeting WCAG AAA. The circular icon button (`button-icon-circular`) renders at 44 Ã— 44 px minimum at all viewports.
...
- **Hero**: at desktop, headline left + gradient ribbon right (50 / 50). At mobile, headline stacks above a smaller-scale ribbon â€” never below.
...
- **Footer wordmark banner**: scales fluidly â€” the giant `together.ai` wordmark stays edge-to-edge regardless of viewport.
...
| Level | Treatment | Use |
|---|---|---|
| Level 0 â€” Flat | No shadow, no border. | Most cards on light surfaces lean on hairline borders, not shadow. |
| Level 1 â€” Hairline | 1 px solid `{colors.hairline}` on `{colors.canvas}` cards. | Testimonial cards, article cards, data-table rows. |
| Level 2 â€” Hairline on Dark | 1 px solid `{colors.surface-dark-soft}` on `{colors.canvas-dark}` cards. | Research-band cards, on-dark badges. |
| Level 3 â€” Soft Drop | `rgba(1, 1, 32, 0.1) 0px 4px 10px 0px` â€” a barely-perceptible shadow tinted with the brand's dark-navy. | Floating elements (the chat-launcher orb, sticky-bottom nav row when one appears). |
...
| `{rounded.sm}` | 4px | The brand's canonical radius â€” buttons, badges, cards, data-table rows, stat tiles. |
...
- **Testimonial portraits**: 1:1 square crop with hard-edge corners â€” no avatar pill.
...
**`button-primary`** â€” the black pill that carries every primary CTA.
...
**`button-outline`** â€” the white-on-white outline button used inside pricing pages and feature toggles.
- Background `{colors.canvas}`, text `{colors.ink}`, 1 px solid `{colors.hairline}` border, shape `{rounded.xs}` 3.25 px.

**`button-icon-circular`** â€” the floating chat-launcher orb in the bottom-right of every page.
- Background `{colors.primary}`, white icon, shape `{rounded.full}`. The only fully-pill shape in the system.

### Cards & Containers

**`research-card`** â€” the 4-up grid card on the dark "Grounded in cutting-edge research" band.
...
**`testimonial-card`** â€” the 3-up "AI natives build on Together AI" card.
...
**`article-card`** â€” the 2-up "What's new at Together AI" article card.
...
**`code-editor-mockup`** â€” the dark code-preview surface inside the product band.
- Background `{colors.canvas-dark}`, text `{colors.on-dark}`, body in `{typography.mono-caption}`, padding `{spacing.2xl}`, shape `{rounded.sm}` 4 px. Window chrome stays minimal â€” no traffic-light dots, no title bar.

**`stats-card-tinted`** â€” the pastel-tinted stat tile (mint, peach, periwinkle) on the white middle band.
...
**`text-input`** â€” the form input on the startup-accelerator application form.
...
**`hero-band-dark`** â€” the dark navy hero that opens every product / marketing page.
...
**`research-band-dark`** â€” the dark navy band that hosts the "Grounded in cutting-edge research" 4-up card grid.
...
**`feature-tab-pill`** â€” the tab pill row inside the "Full-stack cloud" section.
...
**`pricing-sub-tab`** â€” the secondary tab row inside the pricing-page model table (TEXT / VISION / IMAGE / AUDIO / VIDEO).
...
**`data-table-row`** â€” the model row inside the pricing serverless-inference table.
...
**`data-table-header`** â€” the table header row.
...
**`toggle-pill-group`** â€” the "Standard Pricing / Wholesale Pricing" segmented control above the fine-tuning table.
...
**`badge-neutral`** â€” the inline tag pill on light surfaces.
...
**`badge-subtle-on-dark`** â€” the inline tag pill on dark hero / research surfaces.
...
**`footer-wordmark-banner`** â€” the massive `together.ai` wordmark at the bottom of every page.
...
### Do
- Reserve `{colors.primary}` (`#000000`) for every primary CTA. One black pill per visible viewport â€” that consistency is the brand's whole conversion story.
- Set every section eyebrow and button label in `{typography.mono-caps-button}` / `{typography.mono-caps-eyebrow}` â€” uppercase mono, positive tracking.
- Pair the brand gradient (`{colors.accent-orange}` â†’ `{colors.accent-magenta}` â†’ `{colors.accent-periwinkle}`) at hero scale only. The gradient is the brand chrome; never shrink to icon size.
- Cycle page surfaces in the `{colors.canvas-dark}` â†’ `{colors.canvas}` â†’ `{colors.canvas-dark}` rhythm; the dark-light contrast carries elevation more than any shadow.
...
- Render the giant `together.ai` wordmark banner at the bottom of every long page in `{typography.display-xxl}`, tinted toward `{colors.hairline}` so it reads as a stencil â€” not as a heavy footer title.
