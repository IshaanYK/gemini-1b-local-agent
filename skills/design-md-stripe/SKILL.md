...
Stripi's design language opens with the gradient mesh. A wide horizontal band of pastel cream, sherbet orange, lavender, electric indigo, and ruby pink occupies the upper third of nearly every marketing page â€” the brand's instantly-recognizable atmospheric backdrop. Type and product UI mockups float above it on `{colors.canvas}` (white), with the gradient acting as both decoration and visual anchor. The lower portion of the page returns to white, with feature explanations on `{colors.canvas-soft}` (a barely-tinted cool off-white) and dashboard product mockups composited as faux IDE/console panels in deep navy.

The color system has two primary roles. **Indigo** (`{colors.primary}` â€” `#533afd`) is the brand's signature CTA color, used sparingly: one filled pill per band. **Deep navy** (`{colors.ink}` â€” `#0d253d`) is the universal body text color and the fill of dashboard mockups, the featured pricing tier, and the dark-app surfaces on the dashboard track. Ruby (`{colors.ruby}`) and magenta (`{colors.magenta}`) appear inside the gradient mesh and as accent dots in product UI mockups; they are not used as button colors.

Typography is built around **Sohne** at weight 300 with negative letter-spacing â€” the brand's editorial-density display signature. Display sizes (32â€“56px) use -1.4px to -0.64px tracking; body sizes use 0; tabular caption sizes (where money and numerics matter) use the OpenType `tnum` feature plus a tightening -0.36 to -0.42px tracking. The `ss01` stylistic set is enabled across all roles.

**Key Characteristics:**
- Gradient-mesh backdrop on every marketing hero â€” cream/orange/lavender/indigo/ruby horizontally washed across the upper third of the page.
...
- Tabular-figure body type (`tnum`) for any cell containing money or numerics â€” the brand's quiet financial-data signal.
...
- Pill-shaped buttons (`{rounded.pill}` 9999px) with tight `8px 16px` padding â€” short, decisive, transactional.
...
### Brand & Accent
- **Indigo** (`{colors.primary}` â€” `#533afd`): The brand's signature CTA color. Filled-pill button, link emphasis, gradient anchor.
- **Indigo Deep** (`{colors.primary-deep}` â€” `#4434d4`): A deeper indigo used in gradient mid-stops and as the press-state warmer alternative.
- **Indigo Press** (`{colors.primary-press}` â€” `#2e2b8c`): Pressed-state lift of the primary.
- **Indigo Soft** (`{colors.primary-soft}` â€” `#665efd`): A lighter indigo used in product-UI accents and chart highlights.
- **Indigo Subdued** (`{colors.primary-bg-subdued-hover}` â€” `#b9b9f9`): Pale indigo fill used as soft tag background.
- **Brand Dark 900** (`{colors.brand-dark-900}` â€” `#1c1e54`): The deep navy used on the featured pricing tier and dashboard chrome.
- **Ruby** (`{colors.ruby}` â€” `#ea2261`): Gradient accent and chart highlight; never a button.
- **Magenta** (`{colors.magenta}` â€” `#f96bee`): Brighter pink stop in gradient meshes.
- **Lemon** (`{colors.lemon}` â€” `#9b6829`): Warm sherbet stop in gradient backdrops.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): Default page background.
- **Canvas Soft** (`{colors.canvas-soft}` â€” `#f6f9fc`): Cool-tinted off-white used on feature bands beneath the gradient hero.
- **Canvas Cream** (`{colors.canvas-cream}` â€” `#f5e9d4`): Warm cream used as a feature-band fill â€” the brand's chromatic interlude.
- **Hairline** (`{colors.hairline}` â€” `#e3e8ee`): 1px borders on cards and tables.
- **Hairline Input** (`{colors.hairline-input}` â€” `#a8c3de`): Slightly cooler hairline used on form inputs.

### Text
- **Ink** (`{colors.ink}` â€” `#0d253d`): Default body text color across the brand. Deep navy, never pure black.
- **Ink Secondary** (`{colors.ink-secondary}` â€” `#273951`): Secondary text on white.
- **Ink Mute** (`{colors.ink-mute}` â€” `#64748d`): Helper text, captions, table labels.
- **Ink Mute 2** (`{colors.ink-mute-2}` â€” `#61718a`): Near-equivalent to ink-mute used in nav.
- **On Primary** (`{colors.on-primary}` â€” `#ffffff`): Text on indigo / dark-navy surfaces.

### Semantic
The brand does not use a separate semantic color palette in the marketing system â€” error / success states live in dashboard-product UI specifically.

## Typography

### Font Family

The display and UI tier is **Sohne** (proprietary, licensed from Klim Type Foundry) at weights 300 (thin) and 400 (regular). The variable font (`sohne-var`) is loaded with `font-feature-settings: "ss01"` enabled globally â€” the stylistic set substitutes a single-story `a` and other character variants that are part of the brand's typographic signature.
...
Sohne is proprietary. Use **Inter** (open-source via Google Fonts) at weight 300 with `letter-spacing: -1.4px` and `font-feature-settings: "ss01"` for display tiers â€” Inter is the closest open-source analogue. For body sizes, Inter at 300 weight with `font-feature-settings: "tnum"` (where applicable) is the canonical substitute. Avoid Helvetica or system-ui defaults â€” they're heavier than the brand needs.
...
- **Section padding**: 64â€“96px on marketing surfaces; 32â€“48px on dashboard / product surfaces.
...
| 3 | Gradient mesh backdrop | The brand's primary depth medium â€” atmospheric color rather than literal shadow |
...
**`card-pricing-featured`** â€” the inverted dark featured tier.
...
**`card-cream-band`** â€” warm interlude card.
...
**`card-dashboard-mockup`** â€” composited dashboard / product UI screenshot.
...
**`nav-bar-on-mesh`** â€” top nav floating over the gradient hero.
...
**`pill-tag-soft`** â€” subdued indigo tag.
...
**Gradient Mesh Backdrop** â€” pastel cream â†’ sherbet orange â†’ lavender â†’ indigo â†’ ruby pink stops blurred horizontally across the upper third of the page. Implemented as SVG or a large background image â€” not a flat CSS gradient (the real mesh has organic blob shapes).

**Composited Dashboard Mockup** â€” multi-layer faux-product-UI compositions: an IDE panel on the left, a dashboard table center, a chart card on the right, all rendered at small scale inside `{rounded.lg}` containers with subtle Level 2 shadows. The composite is the brand's most-photographed feature.

**Tabular-Figure Money Type** â€” every number rendering money, count, or transaction value uses `font-feature-settings: "tnum"`. The brand's quiet signal that it's a financial-infrastructure platform.

**`link-on-light`** â€” inline links on light surfaces.
- Text `{colors.primary}` rendered in `{typography.body-md}`, no underline by default.

**`footer-light`** â€” site-wide footer.
- Background `{colors.canvas}`, text `{colors.ink-mute}`, type `{typography.caption}`, padding `{spacing.huge} {spacing.xl}` (64px 24px). Holds 4â€“6 columns of link groups, social icons, and a small legal row.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` for filled CTAs and inline link emphasis â€” it should appear sparingly, one filled button per band.
- Apply the gradient mesh to every marketing hero; bare-canvas heroes feel off-brand.
- Render display tiers at weight 300 with negative letter-spacing â€” the thin tracking is the typographic signature.
...
### Don't
- Don't bump display weight above 300 â€” at 400 the brand's editorial air collapses.
- Don't add new accent colors outside the documented gradient stops (cream / orange / lavender / indigo / ruby / magenta).
- Don't use the indigo `{colors.primary}` as a body-text color â€” it's a CTA and link color, not a type color at body size.
- Don't shrink button padding below `8px 16px` â€” the tight pill is part of the brand's transactional feel.
- Don't render money cells without `tnum` â€” it breaks the quiet financial-data signature.
...
|---|---|---|
| Wide | â‰¥ 1440px | Full gradient mesh edge-to-edge; dashboard composite at full scale |
| Desktop | 1024â€“1440px | Default content max-width; pricing 4-up |
| Tablet | 768â€“1023px | Pricing 2-up; dashboard composite simplifies to 2 panels |
| Mobile | < 768px | Pricing 1-up; hamburger nav; display drops 56 â†’ 36px |

### Touch Targets
- Pill buttons hit â‰¥ 40Ã—40px on mobile via padding scaling. On smaller screens, buttons size up to 44Ã—44px to maintain WCAG AAA.
- Form fields stay at 40px minimum height.

### Collapsing Strategy
- Display tiers stair-step 56 â†’ 48 â†’ 32 â†’ 26 â†’ 22px through the breakpoints.
...
- Pricing tiers stair-step 4-up â†’ 2-up â†’ 1-up.
...
7. The gradient mesh is non-negotiable on marketing heroes â€” bare-canvas heroes break the brand.
