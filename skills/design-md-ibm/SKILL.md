---
name: design-md-ibm
description: Design system and styling guidelines based on ibm. Use this skill when asked to create a design inspired by ibm.
---
---
version: alpha
name: IBM-design-analysis
description: "An enterprise-marketing canvas faithful to Carbon Design System: white surfaces, charcoal type, IBM Blue (#0f62fe) as the single confident accent, and a deliberately flat-square aesthetic where corners stay at 0â€“4px. Type runs IBM Plex Sans at light weight 300 for display sizes (a brand signature) and 400/600 for body and emphasis. Cards live as thin-bordered tiles with no shadow; sections separate via subtle gray rows. The chrome is square, the typography is light, and the only color in the system is one assertive blue â€” the result reads as old-world enterprise gravitas reframed for the cloud era."
...
**IBM Plex Sans** carries the entire type hierarchy. Display sizes (76 / 60 / 42px) run at weight **300** â€” IBM's signature light display treatment that makes 76px feel calmer than competing brands' 700-weight display. Body type sits at weight 400 with `letter-spacing: 0.16px` (a Carbon precision detail) and line-height 1.50. The voice reads as careful, technical, and trustworthy.

The system reaches for color rarely â€” IBM Blue marks links, primary CTAs, and the rare full-bleed CTA banner. Charcoal carries every other surface that isn't white. The result is enterprise gravitas without the enterprise stiffness: rigorous, light-weighted, and intentionally restrained.

**Key Characteristics:**
- **Carbon Design System** â€” IBM's marketing chrome IS Carbon. Buttons are square, inputs are square-with-bottom-rule, corners stay at 0px.
- **Light-weight display type**: Plex Sans at weight 300 for 42â€“76px headlines is the brand's typographic signature.
...
- Footer inverts to charcoal (`{colors.inverse-canvas}` #161616) â€” the only dark surface above the page break.
- Card hierarchy is carried by 1px hairlines and surface change, never by drop shadow.
- `letter-spacing: 0.16px` on body is a Carbon precision detail â€” the small positive tracking is part of the brand voice.
- Page rhythm: utility bar â†’ top nav â†’ hero with light-weight headline â†’ feature card grid â†’ customer logo marquee â†’ enterprise feature row â†’ training section â†’ newsletter / sign-in CTA â†’ dark footer.
...
- **Surface 1** ({colors.surface-1}): Light gray (#f4f4f4) â€” input fields, alternate-row stripes, subtle section bands.
- **Surface 2** ({colors.surface-2}): Slightly darker gray (#e0e0e0) â€” disabled fields, hairline-as-fill for separators.
...
- **Inverse Canvas** ({colors.inverse-canvas}): Charcoal #161616 â€” footer surface.
- **Inverse Surface 1** ({colors.inverse-surface-1}): One step lighter than inverse canvas â€” footer column dividers, hovered footer items.

### Text
- **Ink** ({colors.ink}): All headlines and emphasized body type â€” charcoal #161616.
- **Ink Muted** ({colors.ink-muted}): Secondary type at #525252 â€” meta, sub-headlines, footer body.
- **Ink Subtle** ({colors.ink-subtle}): Tertiary type at #8c8c8c â€” disabled, helper text, captions.
- **Inverse Ink** ({colors.inverse-ink}): White on charcoal â€” footer headings.
- **Inverse Ink Muted** ({colors.inverse-ink-muted}): Light gray on charcoal â€” footer body.

### Semantic
- **Success Green** ({colors.semantic-success}): Carbon green-50 â€” success states.
- **Warning Yellow** ({colors.semantic-warning}): Carbon yellow-30 â€” warning states.
- **Error Red** ({colors.semantic-error}): Carbon red-60 â€” error states; danger button background.
- **Info Blue** ({colors.semantic-info}): Identical to primary â€” informational badges.

## Typography

### Font Family

- **IBM Plex Sans** â€” IBM's open-source proprietary typeface (free for any use). Geometric, slightly humanist, designed specifically for enterprise UI. Fallback: `Helvetica Neue, Arial, sans-serif`.

The same family carries display, body, and caption â€” there is no display + body pairing. Hierarchy is carried by **size + weight** rather than by family change. Plex Sans is also free / open-source under the SIL Open Font License â€” making it the easiest custom face on this list to substitute for in implementation.
...
- **Light-weight display is the brand voice.** Plex Sans at weight 300 for 76px headlines reads as quietly authoritative â€” switching to 700 would make it look like every other enterprise site.
...
- **Eyebrow typography uses sentence case 14px** â€” Carbon resists the all-caps tracked eyebrow common to other enterprise brands.
- **Line-heights tighten on display, relax on body**: 1.17 at display-xl, 1.50 at body â€” proportional to size.
...
- Button padding: 12px vertical Â· 16px horizontal â€” Carbon spec.
- Form input padding: 11px vertical Â· 16px horizontal.
...
Carbon uses precise alignment to a 4-pixel grid as its whitespace system. Sections separate via thin gray rows (`{colors.surface-1}`) rather than via large vertical gaps. Content is dense by design â€” IBM's customers expect to see a lot on a page, not a lot of air.
...
Carbon resists drop shadows on marketing â€” depth is carried by surface change and 1px hairlines. The exception is product / app surfaces (Carbon documents shadow tokens for elevated panels), but the marketing site barely uses them.

### Decorative Depth

- **Soft blue gradient backdrops** appear behind some hero illustrations â€” a faint blue-to-white wash that warms the canvas without competing with the headline.
...
| Token | Value | Use |
|---|---|---|
| `{rounded.none}` | 0px | Default â€” every button, card, input, container |
...
- Image frames are flat â€” no rounded corners.
...
**`button-primary`** â€” Blue solid CTA. The default primary across all pages.
...
**`button-secondary`** â€” Charcoal solid button â€” Carbon's "secondary" treatment.
- Background `{colors.ink}`, text `{colors.inverse-ink}`, type `{typography.button}`, padding 12px 16px, rounded `{rounded.none}`.

**`button-tertiary`** â€” White button with blue 1px border + blue text. Used for tertiary CTAs.
...
**`text-input`** + **`text-input-focused`** + **`text-input-error`** â€” Carbon's input chrome.
...
**`newsletter-input`** â€” The "Stay connected" newsletter capture on the home page.
...
**`product-tab`** + **`product-tab-selected`** â€” The horizontal tab strip on product pages and the home "Recommended" carousel.
...
**`top-nav`** â€” Sticky white bar with the IBM logomark left, nav categories center, and search / sign-in icons right.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-sm}`, height 48px. 1px bottom hairline.

**`utility-bar`** â€” Slim gray ribbon above the top nav with location switch, contact, search shortcuts.
- Background `{colors.surface-1}`, text `{colors.ink-muted}`, type `{typography.caption}`, height 32px.

### Footer

**`footer`** â€” Charcoal footer (`{colors.inverse-canvas}`) with the IBM wordmark left and 5â€“6 columns of caption-sized links. The only inverted surface above the page break.
...
- Use surface change (`canvas` â†’ `surface-1`) and 1px hairlines for card hierarchy. Skip drop shadows.
- Stick to sentence case for eyebrows and section labels â€” Carbon resists all-caps tracking.
...
| Tablet | 672px | Card grid 4-up â†’ 2-up; nav becomes hamburger |
| Mobile | 320px | Single-column; display-xl scales 76px â†’ ~32px |
...
- **Card grid**: 4-up â†’ 2-up at 1056px â†’ 1-up below 672px.
- **Display type**: `{typography.display-xl}` 76px scales toward 42px on mobile while preserving the weight-300 treatment.
- **Footer**: 6-column link grid â†’ 3-column at tablet â†’ 1-column at mobile.
...
- IBM's product surfaces (cloud-pak, watson, datacap) have richer Carbon component usage (data tables, graph cells, breadcrumbs, contextual menus) that aren't present on the marketing pages inspected â€” those components live in Carbon's documentation rather than in the marketing extraction.
- Form-field error and validation styling is documented in Carbon docs; the inspected pages didn't render error states.
- Dark mode is documented in Carbon as Gray-100 theme but isn't exposed on these marketing pages â€” only the footer inverts. The full dark theme is a separate Carbon palette not extracted here.
- The community.ibm.com sub-domain uses a different chrome (community-platform white-label) that approximates Carbon but isn't strict â€” the documented system applies to ibm.com proper.
