...
MongoDB carries a strong dual-mode visual identity â€” dark deep-teal hero bands with the unmistakable bright MongoDB green ({colors.brand-green}) CTA pill paired with stark white documentation and pricing surfaces. The homepage opens with "One data platform. Unlimited AI potential." headline over a deep navy hero, the green pill sitting at the visual center as the primary CTA. Lower on the page, embedded code mockup cards (terminal-aesthetic) sit on the dark hero band, breaking out into white feature cards below. The pricing page renders a 3-tier comparison (Free / Flex / Dedicated) with a featured tier highlighted in soft mint background and bright green border. The MongoDB University page presents a course catalog grid where each tile carries a colored category tag (orange, purple, green, teal) â€” these are MongoDB's category-encoding accent colors and are the only place outside the brand green where saturated color appears.

The system uses Euclid Circular A as its display face. The face is contemporary geometric â€” confident but not overly playful â€” and pairs naturally with both the developer-tool aesthetic of the database product and the educational positioning of the learning surfaces. Cards use `{rounded.lg}` (12px) corners; buttons use `{rounded.full}` pills universally. The brand-teal palette ({colors.brand-teal-deep}) anchors hero bands, footer, code mockups, and the dark CTA banners.
...
### Brand & Accent
- **MongoDB Green** ({colors.brand-green}): The brand's most recognizable signal â€” bright pill-CTA color
...
Marketing surfaces give content generous breathing room â€” `{spacing.hero}` (120px) hero padding for deep teal bands. Pricing/learn surfaces tighten dramatically.
...
- Customer logos wall: wordmarks at consistent 60â€“80px height
...
**`button-primary`** â€” Bright MongoDB green pill primary CTA, the dominant action.
...
**`button-on-dark`** â€” Bright green pill on dark hero bands.
...
**`button-secondary-on-dark`** â€” Outlined pill on dark backgrounds.
...
**`button-link`** â€” Inline green text link.
...
**`card-product-deploy`** â€” Product deployment card ("MongoDB Atlas / Community").
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.

**`card-feature-dark`** â€” Dark teal feature card on hero band.
- Background `{colors.brand-teal-deep}`, text `{colors.on-dark}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`.

**`card-course`** â€” MongoDB University course tile.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.
- Top: colored category tag. Below: title `{typography.heading-5}`, description `{typography.body-sm}`, "Get Started â†’" link.

**`card-cert`** â€” Certification card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.

**`pricing-card`** â€” Standard pricing tier card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.

**`pricing-card-featured`** â€” Featured pricing tier (Flex tier, mint background + green border).
...
**`search-pill`** â€” Standard 44px search bar.
...
**`search-pill-large`** â€” Large 56px search bar (top of MongoDB University catalog).
...
**`pill-tab`** + **`pill-tab-active`** â€” Pill-style tab nav (top of pricing: "MongoDB Atlas / Enterprise Advanced").
...
**`badge-green`** â€” Bright green badge for new product highlights.
...
**`badge-green-soft`** â€” Pale-mint pill for success/free indicators.
...
**`badge-purple`** â€” Purple course category tag.
...
**`badge-orange`** â€” Orange course category tag.
...
**`badge-popular`** â€” "Most Popular" tier indicator (dark teal pill with green text).
...
**`service-tile`** â€” Tile in "Customize your deployment" 6-up grid.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.

**`why-card`** â€” "Loved by builders" feature card.
- Background `{colors.surface}`, rounded `{rounded.lg}`, padding `{spacing.xl}`.

**`customer-testimonial-card`** â€” Customer quote card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.
...
**`hero-band-dark`** â€” Deep teal hero band with embedded code mockup.
...
**`hero-platform-card`** â€” Lighter-teal platform showcase card on dark hero.
...
**`footer-region`** â€” Dark teal multi-column footer.
...
| Mobile (large) | 480 â€“ 767px | Course tiles 2-up. Hero 48px. |
| Tablet | 768 â€“ 1023px | 2-column feature grids. Hero 56px. |
| Desktop | 1024 â€“ 1279px | 3-tier pricing card row. 3-up course catalog. Hero 64px. |
| Wide Desktop | â‰¥ 1280px | Full 72px hero presentation. |

### Touch Targets
- Pill buttons render at 40â€“44px effective height
...
- **Pricing tiers**: 3-column â†’ 2-column tablet â†’ 1-column mobile
- **Course catalog**: 3-up â†’ 2-up tablet â†’ 1-up mobile
- **Hero typography**: 72px â†’ 56px â†’ 48px â†’ 36px
