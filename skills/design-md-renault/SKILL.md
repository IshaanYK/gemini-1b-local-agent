...
description: Design system and styling guidelines based on renault. Use this skill when asked to create a design inspired by renault.
---
---
version: alpha
...
description: |
  ...
  typeface. The system reads as confident, photography-first automotive â€” large
...
single Sunlight Yellow accent (`{colors.primary}` â€” `#ffed00`) reserved for
the most consequential actions. The brand's modernised flat diamond logo sets
the tone â€” geometric, confident, slightly industrial â€” and the system follows
...
is no secondary serif, no decorative italic, no script â€” the discipline is
the signature.
...
**Key Characteristics:**
- Two-tone canvas system â€” `{colors.canvas}` (white) for browsing, `{colors.surface-dark}` (black) for storytelling â€” switched in full-bleed bands rather than subtle gradations.
- A single brand accent â€” `{colors.primary}` Sunlight Yellow â€” used scarcely on primary CTAs, "NEW" badges, R5 hero photography, and configurator dot indicators.
...
- Photography-first product tiles â€” vehicle photos full-bleed inside otherwise neutral cards, with copy stacked beneath rather than overlaid.
- Page-level rhythm cycles white â†’ black â†’ yellow accent â†’ black, with the wordmark and footer always closing on `{colors.surface-dark}`.

## Colors

### Brand & Accent
- **Sunlight Yellow** (`{colors.primary}` â€” `#ffed00`): the brand accent. Reserved for primary CTAs, "NEW" / "yeni" badges, configurator dot indicators, and full-bleed promotional tiles. Never decorative.
- **Sunlight Yellow Pressed** (`{colors.primary-deep}` â€” `#e6d200`): the active/pressed state of `{colors.primary}` buttons and tiles.
- **On-Primary** (`{colors.on-primary}` â€” `#000000`): label colour on top of `{colors.primary}` surfaces. Yellow always pairs with black text â€” never white.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): the default page background and card surface.
- **Surface Soft** (`{colors.surface-soft}` â€” `#f7f7f7`): subtle elevation step for grouped configurator rows and inactive form fields.
- **Surface Dark** (`{colors.surface-dark}` â€” `#000000`): the alternate canvas, used for hero bands, footer, and full-bleed storytelling sections.
- **Surface Deep** (`{colors.surface-deep}` â€” `#111111`): a one-step-up elevation inside `{colors.surface-dark}` regions for inset cards and form panels.
- **Hairline** (`{colors.hairline}` â€” `#f2f2f2`): the soft 1px divider between rows on white surfaces.
- **Hairline Strong** (`{colors.hairline-strong}` â€” `#000000`): full-strength dividers on white, plus all card / button outlines.
- **Divider Dark** (`{colors.divider-dark}` â€” `rgba(255,255,255,0.16)`): the corresponding low-contrast divider used inside `{colors.surface-dark}` regions.

### Text
- **Ink** (`{colors.ink}` â€” `#000000`): primary text colour on white surfaces. The same value also drives logos, icons, and outline borders â€” black is structural, not decorative.
- **Body** (`{colors.body}` â€” `#222222`): secondary body text where pure black would feel too heavy in long paragraphs.
- **Charcoal** (`{colors.charcoal}` â€” `#333333`): captions, metadata, and small labels.
- **Mute** (`{colors.mute}` â€” `#666666`): supporting text and inactive nav labels.
- **Ash** (`{colors.ash}` â€” `#8a8a8a`): placeholder text, disabled labels.
- **Stone** (`{colors.stone}` â€” `#c4c4c4`): disabled-state foreground.
- **On-Dark** (`{colors.on-dark}` â€” `#ffffff`): primary text on `{colors.surface-dark}` surfaces.
- **On-Dark Mute** (`{colors.on-dark-mute}` â€” `rgba(255,255,255,0.72)`): secondary text in dark regions; preserves the brand's high-contrast feel without resorting to mid-grey.

### Semantic
- **Error** (`{colors.error}` â€” `#be6464`): muted desaturated red used for inline form errors. Notably warmer than typical pure-red error states.
- **Warning** (`{colors.warning}` â€” `#f0ad4e`): amber alert.
- **Success** (`{colors.success}` â€” `#8dc572`): muted green confirmation.
- **Info** (`{colors.info}` â€” `#337ab7`): a desaturated mid-blue used in informational chips.
- **Link** (`{colors.link}` â€” `#0000ee`): the unstyled-anchor default kept for fallback inline text links â€” production links inherit `{colors.ink}` and rely on underline/weight rather than colour.
...
**Inter Tight**, **Manrope**, or **HK Grotesk Semi Condensed** â€” all share
...
|---|---|---|---|---|---|
| `{typography.display-xl}` | 56px | 700 | 0.95 | 0 | Hero headlines, campaign titles ("E-TECH ELEKTRÄ°KLÄ°", "REVOLUTION"). |
...
- Body copy stays at weight 400 â€” never 500. The contrast between body and display is part of the system.
- Button labels carry a tiny positive letter-spacing (`0.144px` on `{typography.button-md}`) â€” almost imperceptible, but it adds the small bit of mechanical precision the brand wants on CTAs.
...
slightly looser line heights at display sizes â€” clamp display
...
- **Tokens**: `{spacing.xxs}` 4px Â· `{spacing.xs}` 8px Â· `{spacing.sm}` 12px Â· `{spacing.md}` 16px Â· `{spacing.lg}` 20px Â· `{spacing.xl}` 24px Â· `{spacing.xxl}` 32px Â· `{spacing.xxxl}` 40px Â· `{spacing.section}` 80px.
- Section padding (full-bleed band â†’ next band): `{spacing.section}` (80px) on desktop, collapsing to `{spacing.xxxl}` (40px) on mobile.
...
### Grid & Container
- **Max content width** â‰ˆ 1440px. Beyond that, content remains centred and the dark/light bands extend full-bleed.
...
- Whitespace is structural, not decorative. Sections are separated by colour-blocking (white â†’ black) rather than soft padding ramps.
- Inside cards and configurator rows, padding is generous but never airy â€” the brand is mass-market, so density is acceptable.
...
| Level | Treatment | Use |
|---|---|---|
| 0 â€” flat | No shadow, no border | Default page surface, full-bleed bands. |
| 1 â€” outline | 1px solid `{colors.hairline-strong}` or `{colors.hairline}` | Promo tiles on light, vehicle cards, configurator panels. |
| 2 â€” colour-blocked elevation | Surface colour shift (e.g. `{colors.canvas}` card sitting inside a `{colors.surface-soft}` band) | Configurator detail cards, related-content rows. |
| 3 â€” dark inversion | Card swaps to `{colors.surface-dark}` against a `{colors.canvas}` band | "Ticari araÃ§" hero promo tiles, lifestyle storytelling cards. |

Drop shadows are extracted from the system but rarely visible on the marketing
surfaces. When they appear, they are very subtle (~10% opacity, 2â€“4px blur)
and used on floating elements like the configurator's sticky summary bar.

### Decorative Depth
- The R5 hero band uses an atmospheric mesh-gradient backdrop â€” purple-to-pink-to-yellow glow behind the car silhouette â€” that acts as the only true atmospheric depth in the system. Everywhere else, depth is structural (colour-blocking + outlines), not atmospheric.
...
- Avatars and small profile cues â€” when present â€” use `{rounded.full}` circles to contrast with the otherwise square geometry.

## Components

### Buttons

**`button-primary`** â€” yellow CTA
- Background `{colors.primary}`, label `{colors.on-primary}`, type `{typography.button-md}`, padding `14px 24px`, `rounded: {rounded.xs}`.
- The single most important action on a page (e.g. "Hemen randevu al", "Hesapla", "KonfigÃ¼ratÃ¶rÃ¼ baÅŸlat").
- Pressed state lives in `button-primary-pressed` (background `{colors.primary-deep}`).

**`button-secondary-dark`** â€” solid black CTA
...
**`button-outline-dark`** â€” outlined CTA on light
- Background `{colors.canvas}`, label `{colors.ink}`, 1px solid `{colors.hairline-strong}`, type `{typography.button-md}`, `rounded: {rounded.xs}`.
- Tertiary action; appears alongside primary/secondary for "DetaylarÄ± gÃ¶r", "Modeller", etc.

**`button-outline-light`** â€” outlined CTA on dark
...
- The only place the system uses a pill â€” for top-level filter chips ("Servis & randevu", "Sahiplik dÃ¶nemi geÃ§iÅŸi", "Kampanyalar") and configurator tab switches.

**`button-icon-square`** â€” small icon button
- Background `{colors.canvas}`, 1px solid `{colors.hairline-strong}`, `rounded: {rounded.xs}`, 40Ã—40px square.
- Carousel arrows, share, language switcher.

### Cards & Containers

**`promo-tile-light`** â€” white promo tile
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.heading-lg}`, padding `{spacing.xxl}`, `rounded: {rounded.none}`.
- Used in the homepage 2-up grid for "Hybrid araÃ§ modelleri", "binek araÃ§ satÄ±ÅŸ kampanyalarÄ±" tiles.

**`promo-tile-dark`** â€” black promo tile
- Background `{colors.surface-dark}`, text `{colors.on-dark}`, type `{typography.heading-lg}`, padding `{spacing.xxl}`, `rounded: {rounded.none}`.
- Lifestyle / commercial-vehicle storytelling tiles ("ticari araÃ§ satÄ±ÅŸ kampanyalarÄ±").

**`promo-tile-yellow`** â€” accent promo tile
- Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.heading-lg}`, padding `{spacing.xxl}`, `rounded: {rounded.none}`.
- The single "PARLAK SARI" / "Sunlight Yellow" attention tile that anchors a campaign band. Reserved â€” usually one per page maximum.

**`vehicle-card`** â€” car listing card
...
- "SCENIC E-TECH ELEKTRÄ°KLÄ°" hero on the homepage.
...
- Inputs intentionally minimal â€” borderless on top and sides, single hairline at the bottom â€” keeping the catalogue feel.

### Configurator

**`configurator-row`** â€” option list row
...
- Right-side scrolling list on the configurator: "kasa tipi", "motor seÃ§imi", "versiyon seÃ§imi", "renk seÃ§enekleri", etc.

**`configurator-swatch`** â€” circular colour pick
- Background `{colors.surface-soft}` (or the actual car colour), `rounded: {rounded.full}`, 56Ã—56px.
...
- Left: diamond logomark. Centre: top-level nav ("Modeller", "Hizmetler", "Renault YaÅŸamÄ±", "MyRenault"). Right: language switcher + login icon.
...
- Pill chips set in a horizontal scroll bar between hero and content body (e.g. "Servis & randevu", "Sahiplik dÃ¶nemi geÃ§iÅŸi", "Kampanyalar"), `{component.button-pill}` styling.

### Signature Components

**`badge-new`** â€” "yeni" badge
...
### Do
- Reserve `{colors.primary}` exclusively for primary CTAs, "yeni"/"NEW" badges, and at most one accent promo tile per band â€” `{component.promo-tile-yellow}` is intentionally rare.
- Pair `{colors.primary}` only with `{colors.on-primary}` text. Yellow + white is forbidden.
- Set everything in **NouvelR** â€” no secondary serif, no script, no decorative italic.
...
- Use `{rounded.xs}` (2px) on every standard button â€” the near-flat corner is part of the brand.
...
- Use `{component.sub-nav-pill}` (`{rounded.pill}`) only for sub-nav and small filter rows â€” never for primary CTAs.
...
- Don't soften body weights to 500 or 600 â€” the system relies on the 400 / 700 contrast.
...
- Don't pair light grey text on white. Body text steps through `{colors.body}`, `{colors.charcoal}`, `{colors.mute}` â€” `{colors.ash}` and `{colors.stone}` are reserved for placeholders and disabled states.
- Don't add drop shadows to vehicle cards or promo tiles â€” the system is shadow-free at the catalogue level.
...
|---|---|---|
| Desktop XL | â‰¥ 1440px | Full max-width container, 3â€“4 column vehicle grid, 2-up promo tile grid. |
| Desktop | 1280â€“1439px | Same layout, container shrinks to viewport with `{spacing.xl}` side padding. |
| Tablet Large | 1024â€“1279px | Vehicle grid drops to 3-up, configurator left/right panes resize to 55/45. |
| Tablet | 768â€“1023px | Promo tile grid collapses to 2-up, sub-nav pills become horizontal scroll. |
| Mobile Large | 426â€“767px | Vehicle grid 2-up, configurator switches to stacked panes (visualisation on top, options below), nav collapses to hamburger. |
| Mobile | â‰¤ 425px | All grids 1-up, hero `{typography.display-xl}` clamps to ~40px, section padding `{spacing.section}` collapses to `{spacing.xxxl}`. |

### Touch Targets
- All buttons ship at minimum 44Ã—44px on mobile; default `{component.button-primary}` is 48px tall, comfortably exceeding WCAG AAA.
...
- Display headlines clamp: `{typography.display-xl}` 56px â†’ 40px â†’ 32px across the breakpoint ladder.
- Sub-nav pills convert from a wrap row to a horizontal scroll-rail at < 768px.

### Image Behavior
- Vehicle photography is served at 1.5Ã— and 2Ã— DPR; below 768px, the system swaps to a portrait-oriented composition where art direction allows.
...
1. Focus on ONE component at a time. Most components share `{rounded.xs}`, `{colors.canvas}` / `{colors.surface-dark}`, and NouvelR â€” only the role-specific tokens (`{colors.primary}`, `{component.promo-tile-yellow}`) shift between variants.
2. Reference component names and tokens directly (`{colors.primary}`, `{component.button-primary-pressed}`, `{rounded.pill}`) â€” do not paraphrase.
...
4. Add new variants as separate entries (`-pressed`, `-disabled`, `-outline`) â€” do not bury them in prose.
5. Default body type to `{typography.body-md}`; reach for `{typography.subtitle}` only on hero subtitles and lead paragraphs.
6. Keep `{colors.primary}` scarce â€” if more than one yellow element appears per viewport, ask whether one of them should drop to `{colors.surface-dark}` or `{colors.canvas}` instead.
...
- The MyRenault application surfaces (logged-in product) are out of scope for this extraction â€” only the public marketing canvas is documented.
