...
description: Design system and styling guidelines based on revolut. Use this skill when asked to create a design inspired by revolut.
---
---
version: alpha
...
description: |
  ...
  product colours â€” teal, light-blue, deep pink, light-green, warning orange.
  The system reads as fintech-meets-product-brochure: oversized 80pxâ€“136px
...
**near-black storytelling canvas** (`{colors.canvas-dark}` â€” `#000000`)
that hosts hero bands, product mockups, and the planning section, alternating
with **white catalogue bands** (`{colors.canvas-light}` â€” `#ffffff`) that
...
using Revolut") sits at 80â€“136px with `lineHeight: 1.0` and tight negative
letter-spacing. Body type is **Inter** at weight 400 â€” open-source,
no-nonsense, paired with positive tracking (`0.24px`) on UI labels for
slightly more mechanical precision.

The brand accent is `{colors.primary}` (`#494fdf`) â€” a saturated cobalt
violet â€” but it appears scarcely on marketing surfaces. The actual primary
...
inside product mockups and feature illustrations â€” never as button surfaces.

**Key Characteristics:**
- Two-mode canvas system â€” `{colors.canvas-dark}` (true black) for storytelling, `{colors.canvas-light}` (white) for browsing â€” switched in full-bleed bands.
- Display typography is **Aeonik Pro 500** at sizes 20â€“136px with tight `lineHeight: 1.0` and large negative letter-spacing on display sizes.
- The actual primary CTA is `{component.button-primary}` â€” a **white pill with black text**, sitting on the dark canvas as the brightest pixel. Cobalt-violet `{colors.primary}` is reserved for featured plan cards and secondary CTAs.
- Eight saturated accent colours live inside product mockups and illustrations only, never as button surfaces â€” teal, light-blue, deep-pink, light-green, warning orange, yellow, brown, danger red.
- All buttons are pill-shaped (`{rounded.full}`); content cards use `{rounded.lg}` (20px); inputs and small chips use `{rounded.md}` (12px).
- Photography is product-led â€” phone mockups, card mockups, terminal mockups â€” shown full-bleed inside dark sections with no caption overlay.

## Colors

### Brand & Accent
- **Cobalt Violet** (`{colors.primary}` â€” `#494fdf`): the brand accent. Reserved for featured plan cards (`{component.plan-card-featured}`), the brand wordmark icon, and secondary CTAs in white-canvas regions.
- **Cobalt Bright** (`{colors.primary-bright}` â€” `#4f55f1`): a one-step-up bright variant used in inline link colour and accent-photo headers.
- **Cobalt Deep** (`{colors.primary-deep}` â€” `#3a40c4`): the active/pressed state of cobalt elements.
- **On-Primary** (`{colors.on-primary}` â€” `#ffffff`): label colour on top of `{colors.primary}` surfaces.

### Surface
- **Canvas Light** (`{colors.canvas-light}` â€” `#ffffff`): the white catalogue mode for FAQ, download tiles, comparison tables.
- **Canvas Dark** (`{colors.canvas-dark}` â€” `#000000`): the storytelling canvas â€” true black, never near-black.
- **Surface Soft** (`{colors.surface-soft}` â€” `#f4f4f4`): a subtle off-white used on download tiles, soft buttons, and inset card groups inside white bands.
- **Surface Card** (`{colors.surface-card}` â€” `#ffffff`): pure white card surface, used for feature cards in white-canvas regions.
- **Surface Deep** (`{colors.surface-deep}` â€” `#0a0a0a`): a one-step-up dark surface for inset cards inside black-canvas regions.
- **Surface Elevated** (`{colors.surface-elevated}` â€” `#16181a`): the planning-section card background â€” slightly luminous, lifts plan cards off the black canvas.
- **Hairline Light** (`{colors.hairline-light}` â€” `#e2e2e7`): 1px dividers inside white bands.
- **Hairline Dark** (`{colors.hairline-dark}` â€” `rgba(255,255,255,0.12)`): the corresponding low-contrast divider in dark regions.
- **Hairline Strong** (`{colors.hairline-strong}` â€” `#191c1f`): structural full-strength dividers and the outline of light cards.

### Text
- **Ink** (`{colors.ink}` â€” `#191c1f`): primary text colour. Notably warmer than pure black, paired with the white canvas for body legibility.
- **Body** (`{colors.body}` â€” `#1f2226`): long-form body where `{colors.ink}` would feel slightly too sharp.
- **Charcoal** (`{colors.charcoal}` â€” `#3a3d40`): captions, secondary nav.
- **Mute** (`{colors.mute}` â€” `#505a63`): supporting text.
- **Ash** (`{colors.ash}` â€” `#5c5e60`): tertiary text, footer copy.
- **Stone** (`{colors.stone}` â€” `#8d969e`): metadata, subtle captions.
- **Faint** (`{colors.faint}` â€” `#c9c9cd`): disabled foreground, hairline replacements.
- **On-Dark** (`{colors.on-dark}` â€” `#ffffff`): primary text on `{colors.canvas-dark}`.
- **On-Dark Mute** (`{colors.on-dark-mute}` â€” `rgba(255,255,255,0.72)`): secondary text in dark regions.

### Semantic
- **Accent Teal** (`{colors.accent-teal}` â€” `#00a87e`): used in product mockup illustrations.
- **Accent Light Blue** (`{colors.accent-light-blue}` â€” `#007bc2`): inline link colour in dark photo headers.
- **Accent Blue Link** (`{colors.accent-blue-link}` â€” `#376cd5`): default inline link colour on white surfaces.
- **Accent Light Green** (`{colors.accent-light-green}` â€” `#428619`): success / positive product callouts.
- **Accent Green Text** (`{colors.accent-green-text}` â€” `#006400`): inline success text.
- **Accent Yellow** (`{colors.accent-yellow}` â€” `#b09000`): caution / pending state in product mockups.
- **Accent Warning** (`{colors.accent-warning}` â€” `#ec7e00`): full-saturation orange used in warning illustrations.
- **Accent Pink** (`{colors.accent-pink}` â€” `#e61e49`): deep pink â€” used inside product photography and category iconography.
- **Accent Danger** (`{colors.accent-danger}` â€” `#e23b4a`): destructive / error state.
- **Accent Deep Red** (`{colors.accent-deep-red}` â€” `#8b0000`): inline error text.
- **Accent Brown** (`{colors.accent-brown}` â€” `#936d62`): a single warm-neutral used in metals tier card chrome.
- **Link** (`{colors.link}` â€” `#376cd5`): default inline link colour. Same as `{colors.accent-blue-link}`.
...
- **Aeonik Pro** â€” proprietary humanist sans-serif used for all display sizes (20px+) at weight 500. Carries the brand's editorial confidence; tightens dramatically with negative letter-spacing at large sizes.
- **Inter** â€” open-source workhorse for body, button labels, captions, and metadata. Always at weight 400 or 600, with positive tracking (`0.16â€“0.24px`) on UI labels.

When Aeonik Pro cannot be licensed, **Inter Display**, **General Sans**, or **SÃ¶hne** are credible substitutes â€” all share the warm geometric character. Apply -1% letter-spacing on display sizes to match the original tightness.
...
### Principles
- Display sizes always run at weight 500 with `lineHeight: 1.0` (or 1.19â€“1.21 below 48px). The negative letter-spacing scales with size â€” bigger types tighten more.
- Body Inter sits at weight 400 with positive tracking (`0.24px`) â€” the small spacing nudge makes UI labels feel slightly mechanical, fitting fintech precision.
...
When Aeonik Pro is unavailable, clamp display `lineHeight` to 1.0 explicitly and apply -1% letter-spacing on display sizes. Inter Display, General Sans, or SÃ¶hne will read closest to the original. Inter is open-source and should be used directly.

## Layout

### Spacing System
- **Base unit**: 4px, with the working scale on multiples of 4 / 8 / 16.
- **Tokens**: `{spacing.xxs}` 4px Â· `{spacing.xs}` 6px Â· `{spacing.sm}` 8px Â· `{spacing.md}` 14px Â· `{spacing.lg}` 16px Â· `{spacing.xl}` 24px Â· `{spacing.xxl}` 32px Â· `{spacing.xxxl}` 48px Â· `{spacing.block}` 80px Â· `{spacing.section}` 88px Â· `{spacing.band}` 120px.
...
### Grid & Container
- **Max content width** â‰ˆ 1200px on body sections; hero bands run full-bleed.
...
- **Product mockup bands**: a single full-width hero photo of a phone or card mockup, no surrounding chrome â€” the asset itself is the section.

### Whitespace Philosophy
- Whitespace is generous and editorial â€” sections breathe at 88â€“120px so display headlines have room to register at 80â€“136px without feeling cramped.
...
| Level | Treatment | Use |
|---|---|---|
| 0 â€” flat | No shadow, no border | Default canvas bands (light or dark), full-bleed hero. |
| 1 â€” surface card | `{colors.surface-card}` (white) on `{colors.surface-soft}` band | Feature cards inside light bands. |
| 2 â€” surface elevated dark | `{colors.surface-elevated}` (`#16181a`) on `{colors.canvas-dark}` | Plan cards inside the planning section. |
| 3 â€” featured surface | `{colors.primary}` on `{colors.canvas-dark}` | Featured plan card (cobalt violet inversion). |
| 4 â€” product mockup | Full-bleed photo asset | Hero phone / card / terminal mockup bands. |

The system has **no traditional drop-shadow language**. Surfaces register depth via colour-blocking (light â†’ dark band switches) and surface-luminance shifts (`{colors.canvas-dark}` â†’ `{colors.surface-elevated}`). Photography mockups carry their own depth from the asset itself.

### Decorative Depth
- **Product mockup hero bands** â€” the home page features a phone mockup full-bleed against `{colors.canvas-dark}`, with the device's own glow providing the only atmospheric depth. No additional gradients, no shadows.
- **Featured plan card** â€” the cobalt-violet `{component.plan-card-featured}` sits inside the otherwise dark planning grid as a single saturated colour block, marking the recommended tier visually.
- **Card metals tier** â€” the brand uses `{colors.accent-brown}` and a deep gradient on metals card mockups to signal premium without resorting to gold-on-black metallic effects.
...
**`button-primary`** â€” white CTA on dark
...
**`button-dark`** â€” dark CTA on light
- Background `{colors.canvas-dark}`, label `{colors.on-dark}`, type `{typography.button-md}`, `rounded: {rounded.full}`.
- The reverse-canvas equivalent of `{component.button-primary}` â€” used inside white catalogue bands.

**`button-soft`** â€” soft surface CTA
...
**`button-outline-light`** â€” outlined CTA on light
...
**`button-outline-dark`** â€” outlined CTA on dark
...
**`button-pill-sm`** â€” small pill chip
...
**`hero-band-photo`** â€” photo-led hero
...
- Used on product pages â€” phone or card mockup as the full-band canvas.

**`feature-card-light`** â€” feature card on white
...
**`feature-card-dark`** â€” feature card on dark
...
**`plan-card`** â€” subscription plan card
...
**`plan-card-featured`** â€” featured plan card
- Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.body-md}`, `rounded: {rounded.lg}`, padding `{spacing.xxl}`.
- Cobalt-violet inversion of `{component.plan-card}` â€” used on the recommended tier.

**`product-mockup`** â€” full-bleed product asset
- Background `{colors.canvas-dark}`, the asset itself fills the band, `rounded: {rounded.xl}` on the device chrome.
- Phone, card, and terminal mockups â€” no caption overlay, no surrounding chrome.

**`download-tile`** â€” app store download tile
...
- Generous height for fintech accessibility â€” comfortably exceeds WCAG AAA touch target.
...
**`badge-feature`** â€” feature highlight badge
...
- Reserve `{colors.primary}` for the featured plan card and the brand wordmark â€” the cobalt should feel like a deliberate stamp, not a colour theme.
- Set hero headlines in **Aeonik Pro 500** at 80â€“136px with `lineHeight: 1.0` and large negative letter-spacing.
- Use **Inter** for body, button labels, captions â€” never substitute Aeonik Pro for body type.
- Apply `{rounded.full}` to every button and pill; `{rounded.lg}` (20px) to feature and plan cards; `{rounded.md}` (12px) to inputs.
- Show product mockups full-bleed inside dark sections â€” the asset IS the section.
...
- Don't pair white text with cobalt violet inside body content â€” `{colors.primary}` is for the featured plan card surface, not large prose.
...
- Don't bump body Inter to weight 500. Use 400 (default) or 600 (emphatic) â€” never the in-between.
...
|---|---|---|
| Desktop XL | â‰¥ 1440px | 4-up plan grid, full-bleed product mockup bands, max content 1200. |
| Desktop | 1280â€“1439px | Container shrinks; xl side padding. |
| Tablet Large | 1024â€“1279px | Plan grid 4-up; feature grid 3-up. |
| Tablet | 768â€“1023px | Plan grid 2-up; feature grid 2-up. |
| Mobile Large | 426â€“767px | Plan grid 1-up; feature grid 1-up; nav collapses to hamburger; hero `display-xxl` clamps to 64px. |
| Mobile | â‰¤ 425px | All grids 1-up; hero clamps to 48px; section padding `{spacing.section}` collapses to 64px. |

### Touch Targets
- All buttons ship at minimum 48px tall â€” comfortably exceeds WCAG AAA (44px). Default `{component.button-primary}` is 48px.
- `{component.text-input}` is 56px tall â€” fintech-grade accessibility.
...
- Hero `{typography.display-xxl}` clamps: 136px â†’ 80px â†’ 64px â†’ 48px across the breakpoint ladder.
...
### Image Behavior
- Phone and card mockups are served at 1.5Ã— and 2Ã— DPR; below 768px the system swaps to a smaller hero crop.
- Product photography retains its own atmospheric lighting at every breakpoint â€” no responsive variant assets.
...
2. Reference component names and tokens directly (`{colors.primary}`, `{component.plan-card-featured}`, `{rounded.lg}`) â€” do not paraphrase.
...
6. Keep `{colors.primary}` scarce â€” if more than one cobalt-violet element appears per viewport, ask whether one should drop to `{component.plan-card}` (`{colors.surface-elevated}`) instead.
...
- Logged-in app surfaces (transactions, transfers, account settings) are out of scope â€” only the public marketing canvas is documented.
