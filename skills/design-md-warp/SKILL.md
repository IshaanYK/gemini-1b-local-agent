---
name: design-md-warp
description: Design system and styling guidelines based on warp. Use this skill when asked to create a design inspired by warp.
---
---
version: alpha
name: Warp-Inspired-design-analysis
description: An inspired interpretation of Warp's design language â€” an agentic terminal-and-development-environment brand whose surface is a warm near-charcoal canvas (a tint warmer than pure black), broken only by clean Inter typography, the occasional Instrument Serif italic moment, and dense terminal-mockup imagery; CTAs are unusually understated, with shape geometry running tighter than most marketing sites.
...
- Extremely tight button radii â€” 3 / 4 px (1 / 2Ã— the brand's `{rounded.md}` 4 px base) â€” the brand never uses generous pill shapes for CTAs. Only icon containers use `{rounded.full}`.
...
- Terminal-mockup imagery is the brand's only consistent decorative system â€” no gradients, no atmospheric overlays.
...
### Brand & Accent
- **Off White Primary** (`{colors.primary}` â€” `#f7f5f0`): The brand's "primary" is a warm off-white. Used as button-primary fill, as default text on canvas, as the wordmark color. There is no chromatic brand accent â€” the off-white IS the brand's distinguishing tone.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#2b2622`): The warm dark page background. Resolved from `oklch(22.0% 0.004 84.6)`. Slightly browner than pure black, slightly warmer than a neutral gray â€” the warmth IS the brand's identity.
- **Canvas Soft** (`{colors.canvas-soft}` â€” `#383330`): A lighter warm-dark fill used for cards, mockup chrome, and partner-logo tiles.
- **Hairline** (`{colors.hairline}` â€” `#3f3a36`): 1 px solid divider on dark surfaces.

### Text
- **Ink** (`{colors.ink}` â€” `#f7f5f0`): Default text on canvas â€” same off-white as the primary, intentionally unified.
- **Body Strong** (`{colors.body-strong}` â€” `#dad2c1`): Mid-emphasis body text.
- **Body** (`{colors.body}` â€” `#c9c0ad`): Secondary body text â€” captions, supporting copy, press-coverage rows.
- **Mute** (`{colors.mute}` â€” `#aea69c`): Lowest-priority text â€” timestamps, fine print, footer secondary lines. Resolved from `oklch(71.5% 0.008 84.6)`.
...
3. **Instrument Serif** for occasional editorial italic moments â€” rare on the marketing surface, but documented as a third face for emphasised tagline-style phrases. **Abel** is also loaded as a fourth fallback for headline emphasis.
...
### Principles
- **Hero display at weight 400** â€” the brand reads as quietly confident, not as a billboard.
...
- **Inter** â€” load directly from Google Fonts or Vercel-hosted CDN.
- **DM Mono** â€” open-source on Google Fonts.
- **Instrument Serif** â€” open-source on Google Fonts.

## Layout

### Spacing System
- **Base unit**: 4 px (with occasional 10 px and 6 px values for button padding).
- **Tokens**: `{spacing.xxs}` 2 px Â· `{spacing.xs}` 4 px Â· `{spacing.sm}` 8 px Â· `{spacing.md}` 10 px Â· `{spacing.lg}` 16 px Â· `{spacing.xl}` 24 px Â· `{spacing.2xl}` 32 px Â· `{spacing.3xl}` 48 px Â· `{spacing.4xl}` 64 px Â· `{spacing.5xl}` 96 px.
...
| Desktop | â‰¥ 1024px | Full hero split; 3-up download tiles. |

#### Touch Targets
Buttons render at ~36 px tall (8 px vertical padding + 20 px line-height). Mobile inflates touch area through additional padding to meet WCAG 44 Ã— 44 px floor.
...
| Level | Treatment | Use |
|---|---|---|
| Level 0 â€” Flat | No shadow, no border. | Default for hero band. |
| Level 1 â€” Hairline | 1 px solid `{colors.hairline}` border on `{colors.canvas-soft}`. | Default card chrome. |
| Level 2 â€” Inset Card | Canvas-soft fill against canvas background with 1 px hairline. | Mockup cards, download tiles, testimonial cards. |
...
| `{rounded.sm}` | 3px | Default button radius â€” extremely tight. |
...
**`button-primary`** â€” the off-white CTA on dark canvas.
...
**`partner-logo-tile`** â€” the canvas-soft tile hosting a partner logo.
- Background `{colors.canvas-soft}`, monochrome logo SVG inside, padding `{spacing.lg}`, shape `{rounded.md}`.

**`testimonial-card`** â€” the single quote-style card with a portrait.
...
**`press-row`** â€” the press-coverage list item.
- Background `{colors.canvas}` (no fill â€” sits on the canvas band), 1 px solid bottom border `{colors.hairline}`, body in `{typography.body-md}`, padding `{spacing.lg}` 0.

**`job-row`** â€” the "Join our team" list item (single row per open role).
- Background `{colors.canvas}`, 1 px solid bottom border, body in `{typography.body-md-strong}`, padding `{spacing.lg}` 0.

### Inputs & Forms

**`text-input`** â€” the dark-canvas text input.
...
**`hero-band`** â€” the dark hero band hosting the 64-px Inter headline.
...
**`content-band`** â€” the standard content band.
...
- Keep the warm-dark canvas tone â€” pure black breaks the brand's identity.
