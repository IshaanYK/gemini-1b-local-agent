...
description: Design system and styling guidelines based on hashicorp. Use this skill when asked to create a design inspired by hashicorp.
---
---
version: alpha
name: HashiCorp-design-analysis
description: "An enterprise-infrastructure marketing canvas built around a near-black ground (#000000) and a system of per-product accent colors â€” Terraform purple, Vault yellow, Consul pink, Waypoint cyan, Vagrant blue â€” that act as identity tokens rather than decorative palette. Display type is hashicorpSans set in 600/700 with tight 1.17â€“1.21 line-heights; body type runs the same family at 500 weight with relaxed 1.50â€“1.71 line-heights. Cards live as charcoal surfaces with 1px translucent gray borders; product showcase cards lift into per-product chromatic gradients. The system reads as confident, technical, and intentionally multi-product â€” every section quietly signals which HashiCorp tool it represents."
...
HashiCorp's marketing canvas is a near-black ground that serves a multi-product portfolio without ever feeling generic. The dominant surface is `{colors.canvas}` (pure black) layered with `{colors.surface-1}` charcoal cards and 1px translucent gray hairlines. The chrome is monochrome â€” white pill-rounded buttons (`{components.button-primary}`), white type, gray secondary type â€” but the system is held together by a **palette of per-product accent colors** that signal which HashiCorp tool a given section belongs to: Terraform purple, Vault yellow, Consul red, Waypoint cyan, Vagrant blue, Nomad green, Boundary coral.

Display type is **hashicorpSans** at weights 600/700 with tight line-heights (1.17â€“1.21); body type is the same family at 500 weight with deliberately relaxed line-heights (1.50â€“1.71) â€” the contrast feels editorial, not enterprise-templated. CTAs use small `{rounded.md}` 8px corners rather than pills, which keeps the system reading as developer-facing rather than consumer-y.

The signature device is the **product-card** family â€” each HashiCorp product gets its own colored card variant on the home and infrastructure pages, lifting Terraform into a violet ground, Vault into yellow, Waypoint into cyan. These aren't decorative gradients â€” they're identity surfaces. A reader scrolling the page can tell which product a section is about from the corner of their eye.

**Key Characteristics:**
- Black-canvas marketing system: `{colors.canvas}` is the surface for hero, body, pricing, comparison tables, and footer alike.
- **Per-product color identity**: Terraform `{colors.product-terraform}`, Vault `{colors.product-vault}`, Waypoint `{colors.product-waypoint}`, Vagrant `{colors.product-vagrant}`, Consul `{colors.product-consul}`, Nomad `{colors.product-nomad}`, Boundary `{colors.product-boundary}` â€” each with its own button + card variant.
- Display headlines run hashicorpSans 600/700 with line-height 1.17â€“1.21 (tight); body runs the same family at 500 with 1.50â€“1.71 (relaxed) â€” the proportional gap is the brand's voice.
- CTA shape is `{rounded.md}` 8px â€” not a pill â€” keeping the system reading as developer-tool rather than consumer-app.
- Charcoal surface lift (canvas â†’ surface-1 â†’ surface-2) instead of shadow-driven elevation.
- 1px translucent gray hairlines (`rgba(178,182,189,0.1)`) define cards and dividers â€” the borders are felt more than seen.
- Eyebrow typography (12â€“13px, 600 weight, 0.6px positive tracking, uppercase) marks every section as a category label.
...
### Brand & Accent
- **Black** ({colors.primary}): The system primary surface. Canvas, footer, comparison tables, hero â€” all black.
...
- **Surface 1** ({colors.surface-1}): Charcoal one step above canvas â€” feature cards, pricing cards, resource tiles.
- **Surface 2** ({colors.surface-2}): Two steps above â€” featured pricing card, secondary buttons, hovered product chrome.
- **Surface 3** ({colors.surface-3}): Three steps above â€” small chips, badges, sub-nav backgrounds.
- **Hairline** ({colors.hairline}): 1px borders on cards and dividers.
- **Hairline Soft** ({colors.hairline-soft}): Subtler dividers â€” comparison-table rows.
- **Inverse Canvas** ({colors.inverse-canvas}): Pure white â€” used as the surface of `button-primary` only.

### Text
- **Ink** ({colors.ink}): All headline and emphasized body type â€” pure white.
- **Ink Muted** ({colors.ink-muted}): Secondary type at #b2b6bd â€” meta info, footer columns.
- **Ink Subtle** ({colors.ink-subtle}): Tertiary type at #656a76 â€” form helper text, timestamps, footnotes.

### Per-Product Identity (signature)
HashiCorp's marketing isn't held together by a single accent color â€” it's held together by a system of product-specific accents, each used to mark which tool a section represents.

- **Terraform Purple** ({colors.product-terraform}): Terraform sections, terraform CTAs, the violet 3D cube on the home hero.
- **Terraform Bright** ({colors.product-terraform-bright}): Saturated highlight â€” link emphasis on Terraform pages.
...
- **Amber 100** ({colors.amber-100}): Soft warm highlight â€” extracted but used sparingly.
...
- **hashicorpSans** â€” HashiCorp's proprietary marketing typeface. Geometric, clean, slightly humanist. Fallback stack `__hashicorpSans_Fallback_96f0ca` (system font), then `-apple-system, BlinkMacSystemFont, Segoe UI, helvetica, arial`.

The same family carries display, body, button, and caption â€” no separate display + body pairing. Hierarchy is carried by weight (500 body / 600 emphasis / 700 display) and by a deliberate line-height contrast (tight on display, relaxed on body).
...
- **Tight on display, relaxed on body.** Display sits at line-height 1.17â€“1.21; body lifts to 1.50â€“1.71. The size + line-height contrast carries hierarchy.
- **Weight hierarchy is small.** 500 body / 600 emphasis / 700 display. No light / black extremes â€” the brand reads as engineered.
- **Eyebrow positive-tracked uppercase 12px is the section header.** Every meaningful section has one above the headline.
- **No mono.** Despite being a developer-tools brand, the marketing surface uses no monospace face â€” code voice is reserved for in-product surfaces.
...
- Button padding: 10px vertical Â· 18px horizontal on `{components.button-primary}`.
...
The dark canvas IS the whitespace. Sections separate by surface lift (canvas â†’ surface-1) rather than by gaps in white. Within a section, generous `{spacing.xl}` 32px gaps separate cards; `{spacing.lg}` 24px separates rows.
...
| 3 (product chromatic) | Per-product accent color background â€” Terraform purple, Vault yellow, Waypoint cyan | Product showcase cards |

The product chromatic level isn't a "modal lift" â€” it's an identity device. A Terraform card sits at the same z-plane as a feature-card; the difference is meaning, not depth.

### Decorative Depth

- **3D product visuals** â€” isometric purple cubes (Terraform), translucent yellow safes (Vault), and similar product-tinted illustrations sit in the right column of hero sections.
- **1px translucent gray hairlines** are the dominant edge â€” borders are visible without competing.
...
- Product 3D illustrations use full-bleed within their container â€” no rounded inner mask.
...
**`button-primary`** â€” White rounded-rect CTA. Used as primary CTA on all pages.
...
**`button-product-waypoint`** â€” Waypoint-cyan CTA.
...
**`product-card`** â€” Default product showcase card â€” surface-1 charcoal.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 24px.

**`product-card-terraform`** â€” Product card with Terraform purple ground (used as identity surface, not modal elevation).
- Background `{colors.product-terraform}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 24px.

**`product-card-vault`** â€” Vault-yellow ground.
- Background `{colors.product-vault}`, text `{colors.inverse-ink}`, otherwise identical structure.

**`product-card-waypoint`** â€” Waypoint-cyan ground.
...
**`feature-card`** â€” Generic feature highlight on surface-1.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 24px.

**`pricing-card`** â€” Pricing tier on `/en/pricing`.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 32px.

**`pricing-card-featured`** â€” Recommended tier (visually emphasized via surface lift).
- Background `{colors.surface-2}`, otherwise identical structure.

**`resource-card`** â€” PDF / whitepaper / guide tile in the resources directory.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body-sm}`, rounded `{rounded.lg}`, padding 16px.

### Inputs & Forms

**`text-input`** + **`text-input-focused`** â€” Form fields on pricing seat-count and contact forms.
...
**`product-pill`** â€” Small product-name chip used above hero headlines and on resource cards to label which product a piece of content belongs to.
- Background `{colors.surface-1}`, text `{colors.ink-muted}`, type `{typography.caption}`, rounded `{rounded.pill}`, padding 4px 10px.

### Comparison Table

**`comparison-row`** â€” Single row inside the pricing comparison table.
- Background `{colors.canvas}`, text `{colors.ink-muted}`, type `{typography.body-sm}`. Row separator is `{colors.hairline-soft}`.

### CTA Banner

**`cta-banner`** â€” Large rounded panel used at the bottom of long-form pages with a closing CTA.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.subhead}`, rounded `{rounded.xxl}`, padding 48px.

### Navigation

**`top-nav`** â€” Sticky black bar with HashiCorp logomark left, primary nav links centered, and a `button-primary` ("Sign up") + `button-secondary` ("Sign in") pair right.
...
**`footer`** â€” Dense link grid on `{colors.canvas}` with the wordmark at left and 5â€“6 columns of caption-sized links.
...
- When introducing a section about a specific HashiCorp product, use that product's `{colors.product-*}` token consistently â€” for the section pill, the CTA button, and (where appropriate) the showcase card background.
- Use `{rounded.md}` 8px on CTA buttons; HashiCorp's brand reads as engineered, not consumer.
- Pair tight display line-heights (1.17â€“1.21) with relaxed body line-heights (1.50â€“1.71). The contrast IS the brand voice.
- Use the eyebrow typography (`{typography.eyebrow}`, uppercase, 0.6px tracking) above every meaningful section.
- Use surface lift (canvas â†’ surface-1 â†’ surface-2) to express hierarchy on dark.
...
- Don't square off CTA corners â€” use `{rounded.md}` 8px, not 0px.
...
- Don't combine multiple product accents in the same viewport â€” the system says "this section is about THIS tool", and mixing accents breaks that signal.
...
| Tablet | 1024px | Product card grid 3-up â†’ 2-up |
| Mobile-Lg | 768px | Pricing comparison becomes per-tier accordion; nav becomes hamburger |
| Mobile | 480px | Single-column everything; display-xl scales 80px â†’ ~36px |

### Touch Targets

- CTA buttons (`button-primary`, `button-secondary`) maintain â‰¥40px tap height across viewports.
- Product pills are 24px tall on desktop and grow to 28px on touch viewports.
- Form inputs hold â‰¥44px tap target on touch viewports.
...
- **Product card grid**: 3-up â†’ 2-up at 1024px â†’ 1-up below 768px.
...
7. Eyebrow type is mandatory above every section â€” skipping it makes sections read as floating.
...
- Dark mode is the only marketing mode â€” light-mode adaptation is not documented.
