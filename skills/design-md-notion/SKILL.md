...
The system uses a Notion-Sans typeface (Inter-based) across every UI surface â€” humanist-geometric character that pairs naturally with the colorful illustrations. Buttons are `{rounded.md}` (8px) rectangles, NOT pills â€” distinguishing Notion's sober rectangular geometry from competitors that use pills universally. Cards use `{rounded.lg}` (12px) consistently.

**Key Characteristics:**
- Deep navy hero band ({colors.brand-navy}) with scattered sticky-note dots + mesh wire decorative illustrations
- **Signature purple pill** ({colors.primary}) primary CTA â€” Notion's recognizable "Get Notion free" button color
...
- 8px-rounded buttons (NOT pills), 12px-rounded cards â€” sober editorial geometry
...
### Brand & Primary
- **Notion Purple** ({colors.primary}): Signature primary CTA color â€” the unmistakable "Get Notion free" pill button. Reserved for the dominant CTA only.
- **Purple Pressed** ({colors.primary-pressed}): Pressed-state variant
- **Purple Deep** ({colors.primary-deep}): Deeper variant for emphasis
- **Brand Navy** ({colors.brand-navy}): Hero band background â€” deep navy
...
### Decorative Depth
- Hero workspace mockup card uses deep diffuse drop shadow (Level 3) â€” significant elevation against the navy band
...
Notion's geometry is sober-editorial â€” `{rounded.md}` (8px) buttons distinguish it from pill-button-everywhere brands.
...
**`button-primary`** â€” Signature purple rectangular primary CTA, the dominant action.
...
**`button-dark`** â€” Black rectangular CTA on light backgrounds.
- Background `{colors.ink-deep}`, text `{colors.on-dark}`, typography `{typography.button-md}`, padding `10px 18px`, rounded `{rounded.md}`.

**`button-secondary`** â€” Outlined rectangular for secondary actions ("Request a demo").
...
**`card-feature`** â€” Feature card with larger padding.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.

**`card-feature-yellow-bold`** â€” Bold yellow feature banner for high-emphasis content ("Ask your on-demand assistants").
- Background `{colors.card-tint-yellow-bold}`, text `{colors.charcoal}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`.

**`card-feature-peach`** + **`card-feature-rose`** + **`card-feature-mint`** + **`card-feature-sky`** + **`card-feature-lavender`** + **`card-feature-yellow`** + **`card-feature-cream`** â€” Pastel-tinted feature cards.
...
**`card-agent-tile`** â€” Agent assistant tile.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.

**`card-template`** â€” Template thumbnail card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.lg}`, border `1px solid {colors.hairline}`.

**`card-startup-perk`** â€” Startup-program perk grid item.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline}`.

**`pricing-card`** â€” Standard pricing tier card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.

**`pricing-card-featured`** â€” Featured pricing tier (Plus or Business â€” purple-bordered).
...
**`pill-tab`** + **`pill-tab-active`** â€” Pill-style tab nav for top-level switching.
...
**`badge-purple`** â€” Purple status badge (matches primary CTA).
...
**`badge-pink`** â€” Pink accent badge.
...
**`badge-orange`** â€” Orange accent badge.
...
**`badge-tag-purple`** â€” Soft-purple feature tag chip.
...
**`badge-tag-orange`** â€” Soft-orange feature tag.
...
**`badge-tag-green`** â€” Soft-mint feature tag.
...
**`badge-popular`** â€” "Most Popular" tier indicator.
- Background `{colors.primary}`, text `{colors.on-primary}`, typography `{typography.caption-bold}`, rounded `{rounded.full}`, padding `4px 10px`.

**`promo-banner`** â€” Light surface promo strip ABOVE the top nav.
...
**`workspace-mockup-card`** â€” Embedded Notion workspace UI mockup on hero band ("Ramp HQ" kanban board).
...
**`testimonial-card`** â€” Customer testimonial card.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `1px solid {colors.hairline}`.
...
**`stat-row`** â€” Stats strip with bar chart visualization ("More productivity. Fewer tools.").
- Background `{colors.surface}`, text `{colors.ink}`, rounded `{rounded.lg}`, padding `{spacing.section-sm}`.

**`cta-banner-light`** â€” Light surface CTA banner.
...
**`hero-band-dark`** â€” Deep navy hero band with embedded workspace mockup and decorative dots/wires.
...
- Atmospheric decoration: scattered colorful sticky-note dots and mesh wire illustrations around the hero content (NOT a literal pattern fill â€” handled per-page via SVG/illustration).

**`footer-region`** â€” Multi-column light footer.
...
### Do
- Use `{colors.primary}` (purple) as the dominant CTA across all surfaces â€” it's the brand's recognizable signal
...
- Apply `{rounded.md}` (8px) to buttons consistently â€” Notion uses rectangles, not pills
...
- Don't mix link-blue ({colors.link-blue}) with primary-purple ({colors.primary}) â€” they have distinct roles
...
| Mobile (large) | 480 â€“ 767px | Feature cards 2-up. Hero 48px. |
| Tablet | 768 â€“ 1023px | 2-column feature grids. Hero 56px. |
| Desktop | 1024 â€“ 1279px | 4-tier pricing card row. Hero 72px. |
| Wide Desktop | â‰¥ 1280px | Full 80px hero presentation. |

### Touch Targets
- Buttons render at 40â€“44px effective height
- Form inputs render at 44px height
- Pill tabs ~32px â†’ 44px on mobile

### Collapsing Strategy
- **Promo banner** stays full-width; truncates at < 480px
- **Top nav** below 1024px collapses to hamburger
- **Hero band**: workspace mockup card moves below text/buttons on mobile
- **Pricing tiers**: 4-column â†’ 2-column tablet â†’ 1-column mobile
- **Feature cards**: 3-up desktop â†’ 2-up tablet â†’ 1-up mobile
- **Hero typography**: 80px â†’ 56px â†’ 48px â†’ 36px
- **Footer**: 6-column desktop â†’ 3-column tablet â†’ accordion mobile

### Image Behavior
- Workspace mockup card maintains aspect ratio
- Pastel illustrations inside feature cards scale proportionally
- Customer logo wall: wordmarks at consistent 60â€“80px height
...
6. Keep `{colors.primary}` (purple) as the primary CTA â€” distinct from `{colors.link-blue}` for inline links
...
- Pastel-tint mapping (which feature uses which tint) is observation-based â€” the actual brand library may have more entries
