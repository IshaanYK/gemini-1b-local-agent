...
description: Design system and styling guidelines based on figma. Use this skill when asked to create a design inspired by figma.
---
---
version: alpha
name: Figma-design-analysis
description: "A confident black-and-white editorial frame interrupted by oversized, hand-cut pastel color blocks. The marketing canvas is rigorously monochrome â€” figmaSans variable type, pure white surfaces, pure black ink, pill-shaped CTAs â€” while each story section drops the page into a saturated lime, lavender, cream, mint, or pink panel that reads like a sticky note placed on a clean desk. The result is a design system that feels both technical and joyful â€” a tool for serious work, made by people who like color."
...
Figma's marketing canvas is, at the system level, an editor-clean black-and-white frame. The chrome â€” top nav, body type, footer, primary CTA â€” is monochrome. Headlines are oversized `{typography.display-xl}` set in `figmaSans` with aggressive negative tracking, body copy hovers around weight 320â€“340 of the same variable family, and small mono `{typography.eyebrow}` and `{typography.caption}` labels (figmaMono, all-caps, positive tracking) act as section markers. Every CTA is a pill â€” `{rounded.pill}` â€” and the primary action across the entire site is the same black `{components.button-primary}` paired with the same white `{components.button-secondary}`.

What makes the design unique is what happens **between** those monochrome bookends: the page repeatedly drops into oversized pastel **color-block sections** â€” lime, lavender, cream, mint, pink, coral, and a deep navy â€” that span the full content width with `{rounded.lg}` corners and `{spacing.xxl}` interior padding. These blocks are where the storytelling lives. They aren't accents tucked into a card; they take over a whole viewport's worth of vertical space, like a designer arranging giant sticky notes on a clean wall. FigJam is the most pastel-saturated, the home page rotates through the full set, and the pricing page ends with a lime FAQ panel â€” same vocabulary, different rhythm per route.
...
- Pill is the only button shape â€” `{rounded.pill}` for text CTAs, `{rounded.full}` for icon buttons. No square buttons anywhere.
- `figmaSans` variable typeface used at unusually fine weight increments (320, 330, 340, 450, 480, 540) â€” the type system reads as a single voice that flexes rather than a multi-weight family.
- Tight negative letter-spacing on display sizes (-1.72px at 86px, -0.96px at 64px) creates a confident editorial cadence.
- `figmaMono` reserved for category labels, eyebrows, and captions â€” always uppercase, positive tracking â€” to flag taxonomy without competing with display type.
- Color-block page rhythm (home): white hero â†’ marquee strip â†’ white feature â†’ lime systems block â†’ navy ship-products block â†’ coral developer block â†’ white template grid â†’ white footer.
...
- **Magenta Promo** ({colors.accent-magenta}): A single saturated CTA pink reserved for promotional inline buttons â€” appears, for example, on the lilac "Save your spot" Release Notes banner. Use scarcely; it is not a section color.
...
- **Hairline Soft** ({colors.hairline-soft}): Even subtler dividers â€” comparison-table row separators and footer column rules.
...
- **Block Cream** ({colors.block-cream}): Soft warm background â€” FigJam hero strip, template-grid section.
...
- **Block Navy** ({colors.block-navy}): Deep indigo story block â€” only place dark surfaces appear above the footer.

### Text
- **Ink** ({colors.ink}): All headline, body, and caption type on light surfaces. There is no softer mid-gray text role on marketing â€” body copy is always black at weight 320â€“340, and weight (not opacity) carries the hierarchy.
...
- **figmaSans** â€” Figma's proprietary variable typeface; fallback stack `figmaSans Fallback, SF Pro Display, system-ui, helvetica`. Variable weight axis is exercised at unusually fine increments (320, 330, 340, 450, 480, 540, 700) â€” the design system reads as a single voice modulating rather than a stepped weight family.
- **figmaMono** â€” Proprietary monospace; fallback `figmaMono Fallback, SF Mono, menlo`. Used exclusively for eyebrow labels and captions, always uppercase with positive letter-spacing.
...
- **Weight, not size, carries hierarchy on body copy.** A 20px paragraph at weight 330 sits next to a 20px link at weight 480 â€” the eye reads emphasis without scale change.
...
- **Mono is taxonomy, not body.** figmaMono is reserved for eyebrows and captions â€” never used to set a paragraph.
- **Tight line-heights on display, generous on body.** Display sizes run 1.00â€“1.10; body runs 1.40â€“1.45. The contrast reinforces that headlines are graphics and body copy is for reading.
...
- **Base unit**: 8px.
- **Tokens (front matter)**: `{spacing.hair}` 1px Â· `{spacing.xxs}` 4px Â· `{spacing.xs}` 8px Â· `{spacing.sm}` 12px Â· `{spacing.md}` 16px Â· `{spacing.lg}` 24px Â· `{spacing.xl}` 32px Â· `{spacing.xxl}` 48px Â· `{spacing.section}` 96px.
...
- Form input padding: `{spacing.sm}` 12px vertical Â· 14px horizontal.
- Button padding: `{spacing.xs}` 8px vertical Â· `{spacing.lg}` 24px horizontal for pill buttons (the asymmetric `8px 18px 10px` extracted on `button-secondary` nudges the type optically inside the pill).
- Universal rhythm constant: `{spacing.section}` (96px) â€” the vertical gap between major content sections holds across home, pricing, and FigJam pages.
...
- Color-block sections break the column grid â€” they span content width with full bleed inside the rounded `{rounded.lg}` corners, then place a single editorial column of headline + body inside.
...
- **Sticky-note style component thumbnails** in FigJam â€” slightly off-axis pastel rectangles arranged like notes on a board â€” read as collage, not card-stack.
...
- Image frames use `{rounded.md}` (8px) â€” generous enough to feel friendly, conservative enough to read as editorial.
...
- No avatar circles appear in marketing surfaces â€” Figma's marketing avoids personification.

## Components

### Buttons

**`button-primary`** â€” The black "Get started for free" pill that appears in the top nav, every hero, and every closing CTA.
...
**`button-secondary`** â€” White pill with black text. Used for tertiary navigation actions ("Contact sales") and as the visual counterpart to the primary pill.
...
**`button-tertiary-text`** â€” Plain text link styled as a button hit target inside top nav and footer.
...
**`button-icon-circular`** â€” 40px circular icon button used for carousel controls, social links, and inline actions on light surfaces.
- Background `{colors.surface-soft}`, text `{colors.ink}`, rounded `{rounded.full}`, size 40px.

**`button-icon-circular-inverse`** â€” Same shape, used on inverse-canvas / dark color blocks.
- Background `{colors.on-inverse-soft}` (translucent white), text `{colors.inverse-ink}`, rounded `{rounded.full}`, size 40px.

**`button-magenta-promo`** â€” Saturated pink pill used only inside promotional surfaces such as the lilac "Save your spot" Release Notes banner. Reserved for moments where Figma's product team wants the CTA to pop against an already-colored panel.
- Background `{colors.accent-magenta}`, text `{colors.on-primary}`, type `{typography.button}`, rounded `{rounded.pill}`, padding 10px 18px.

### Pricing Tabs

**`pricing-tab-default`** + **`pricing-tab-selected`** â€” The pill-toggle that switches between Starter / Professional / Organization / Enterprise on `/pricing/`.
- Default: `{colors.canvas}` background, `{colors.ink}` text, rounded `{rounded.pill}`.
- Selected: `{colors.primary}` background, `{colors.on-primary}` text â€” exactly the same surface as `button-primary`, which makes the selected tab feel like an active CTA, not a passive state.

### Inputs & Forms

**`text-input`** + **`text-input-focused`** â€” Form fields on `/contact/` and pricing seat-count steppers.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.md}`, padding 12px 14px.
- Focused state retains the same surface â€” focus is communicated via ring, not via fill change.
...
**`pricing-card-feature-row`** â€” Single row inside the comparison table.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-sm}`. Row separator is `{colors.hairline-soft}`.

**`template-card`** â€” Thumbnail tile in the home "Explore what people are making" grid and the FigJam template gallery.
- Background `{colors.surface-soft}`, text `{colors.ink}`, type `{typography.body-sm}`, rounded `{rounded.md}`, padding `{spacing.md}`.

**`feature-illustration-tile`** â€” Larger composition tile that holds a product UI mock or pastel illustration.
...
**`color-block-section`** â€” lime ground for "systems" stories (home), pricing FAQ, and the contact form.
- Background `{colors.block-lime}`, text `{colors.ink}`, type `{typography.subhead}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`.

**`color-block-section-lilac`** â€” lavender ground for `/design/` hero and FigJam highlight sections.
- Background `{colors.block-lilac}`, otherwise identical structure.

**`color-block-section-navy`** â€” deep indigo ground for the home "Ship products" story block. The only inverse color-block surface above the footer.
...
**`promo-banner-lilac`** â€” The Release Notes / "Save your spot" inline banner that floats above the contact form.
...
**`top-nav`** â€” Sticky white bar with logo, primary nav links, sign-in link, and the right-anchored `button-secondary` ("Contact sales") + `button-primary` ("Get started for free") pair.
...
**`marquee-strip`** â€” Thin black ribbon directly under the nav that scrolls through customer logos in white.
- Background `{colors.inverse-canvas}`, text `{colors.inverse-ink}`, type `{typography.body-sm}`, height 36px.

### Comparison Glyphs

**`comparison-checkmark`** â€” Green check used in the pricing comparison matrix.
- Background `{colors.canvas}`, glyph color `{colors.semantic-success}`, rounded `{rounded.full}`, size 16px.

### Footer

**`footer`** â€” Dense link grid on white canvas with the wordmark "Figma" set in display weight at the top-left.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.caption}` for column headings and small links, padding `{spacing.section}` top/bottom Â· `{spacing.xl}` sides.
...
- Keep type in `figmaSans` at variable weights â€” pick from 320, 330, 340, 480, 540, 700 to express hierarchy. Avoid intermediate weights outside this set.
...
- Pair `button-primary` and `button-secondary` whenever a section needs both a primary action and a sales / secondary action â€” the black-and-white pair is the brand signature.

### Don't

- Don't introduce mid-gray text. Body hierarchy comes from `figmaSans` weight, not from opacity.
- Don't add drop shadows to color-block sections â€” the color is the depth device.
...
- Don't combine more than one color block visible inside a single viewport â€” Figma's pacing always lets the white canvas separate them.
- Don't square off CTAs. Square buttons read as a different brand.
- Don't put `figmaMono` in body copy â€” it's a taxonomy tool, not a reading typeface.
...
| Tablet | 960px | Pricing collapses 4-up â†’ 2-up; nav becomes hamburger |
...
- Pill buttons (`button-primary`, `button-secondary`) maintain a minimum 44px tap height across all viewports â€” achieved by combining `{typography.button}` 20px line-height with the documented vertical padding.
...
- **Pricing tier grid**: 4-up â†’ 2-up at 960px â†’ 1-up below 768px. The pill toggle stays horizontal and scrolls horizontally if needed below 560px.
...
- Sticky-note style FigJam thumbnails maintain their slight off-axis rotation across breakpoints â€” the rotation is a brand signal, not a desktop-only flourish.
...
5. Add new variants as separate component entries (`-pressed`, `-selected`) â€” do not bury them in prose.
6. Keep `{colors.primary}` scarce. If two `button-primary` instances appear in the same viewport, the section is doing too much â€” neutralize one to `button-secondary`.
...
- Dark mode is not documented because the marketing site does not ship a dark theme â€” the closest analog is the navy color-block (`color-block-section-navy`) and the inverse-canvas footer.
