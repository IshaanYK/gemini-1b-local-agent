...
description: Design system and styling guidelines based on mistral.ai. Use this skill when asked to create a design inspired by mistral.ai.
---
---
version: alpha
name: Mistral-AI-design-analysis
description: Mistral AI brands itself with a singular signature â€” atmospheric sunset gradients (mustard, orange, deep red) layered over photography of mountains, plus a horizontal "sunset stripe" bar that closes every page. The system pairs warm cream-yellow surfaces ({colors.cream}) with a saturated orange primary CTA ({colors.primary}) and uses an elegant near-serif voice for hero displays. Coverage spans homepage (Frontier AI hero), Le Studio product page, Coding solutions, news article surfaces, contact form, and services tier page â€” all anchored by the signature gradient closing band.
...
- `{rounded.md}` (8px) buttons and `{rounded.lg}` (12px) cards â€” less playful, more editorial geometry
...
- **Block 5/6/7** ({colors.block-5}, {colors.block-6}, {colors.block-7}): Spectrum stops along the sunset gradient (light-yellow â†’ mid-yellow â†’ deep-orange)
...
### Principles
- **Editorial / sans pairing** â€” PP Editorial Old (near-serif, classical) anchors hero displays; Inter (geometric sans) carries everything else. The contrast IS the brand voice.
...
- **Negative letter-spacing** progresses with size â€” display sizes use -1.5px to -0.5px; smaller heads relax to 0
...
Marketing surfaces give content generous breathing room â€” `{spacing.hero}` (120px) hero padding lets the mountain-sunset photography fill the frame. Form pages tighten dramatically: contact form panel uses `{spacing.xxl}` (32px) internal padding, fields stack on `{spacing.md}` (16px) gap.
...
### Decorative Depth
- The atmospheric depth on Mistral's hero comes from the photographic mountain-sunset imagery â€” natural light gradient does the work
- The "sunset stripe" closing band carries depth via its multi-stop gradient (red â†’ orange â†’ yellow â†’ cream)
...
| `{rounded.full}` | 9999px | Status badges, pill tabs (used sparingly â€” most buttons are NOT pills) |

The radius scale is sober and editorial â€” Mistral does NOT use pill buttons. `{rounded.md}` (8px) for buttons, `{rounded.lg}` (12px) for cards, `{rounded.full}` reserved for badges and the rare pill tab.
...
- Customer logos wall presents wordmarks inline at consistent 60â€“80px height
...
**`button-primary`** â€” Saturated-orange primary CTA, the dominant action.
...
**`button-cream`** â€” Warm cream-yellow secondary action, common on cream-surface sections.
...
**`button-on-cream`** â€” White button on cream-tinted backgrounds.
...
**`card-photographic`** â€” Photographic product card with dark background.
...
**`pricing-card-featured`** â€” Featured pricing tier (cream background + orange border).
...
**`pill-tab`** + **`pill-tab-active`** â€” Pill-style tab nav (used sparingly on product pages).
...
**`segmented-tab`** + **`segmented-tab-active`** â€” Underline-style tab navigation.
...
**`badge-orange`** â€” Saturated orange badge.
...
**`code-block`** â€” Syntax-highlighted IDE-style code block (Le Studio page mockup, agent demos).
- Background `{colors.surface-code}`, text `{colors.on-dark}`, typography `{typography.code-md}`, rounded `{rounded.md}`, padding `{spacing.md}`.

**`code-block-header`** â€” Header bar above the code block.
...
**`feature-icon-tile`** â€” Cream-yellow feature icon callout.
- Background `{colors.cream}`, rounded `{rounded.md}`, padding `{spacing.md}`, border `1px solid {colors.beige-deep}`.

**`industry-tile`** â€” Industry-vertical tile in solutions page grid.
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xl}`, border `1px solid {colors.hairline-soft}`.

**`stat-cell`** â€” Stat-row cell ("75% more / 80% better").
- Background transparent, text `{colors.ink}`, typography `{typography.stat-display}`, padding `{spacing.lg}`.

**`customer-testimonial-card`** â€” Customer quote card (used inside Le Studio and Solutions pages).
...
**`logo-wall-item`** â€” Customer logo wordmark cell.
- Background transparent, text `{colors.steel}`, typography `{typography.body-md-medium}`, padding `{spacing.lg}`.

**`faq-accordion-item`** â€” FAQ panel.
- Background `{colors.canvas}`, rounded `{rounded.md}`, padding `{spacing.xl}`, bottom border `1px solid {colors.hairline}`.

**`app-store-badge`** â€” App Store / Google Play download badge.
...
**Top Navigation (Marketing)** â€” Sticky white bar.
...
**`hero-band-sunset`** â€” Atmospheric sunset hero band.
...
**`sunset-stripe-band`** â€” Horizontal closing band at the foot of every page.
- Multi-stop gradient: `{colors.primary}` â†’ `{colors.sunshine-700}` â†’ `{colors.sunshine-500}` â†’ `{colors.yellow-saturated}` â†’ `{colors.cream}`.
- Padding `{spacing.lg} 0`. Spans full width, sits above the footer. THIS IS THE BRAND'S MOST RECOGNIZABLE SIGNATURE ELEMENT.

**`cta-banner-cream`** â€” Page-bottom CTA band on cream surface.
...
**`footer-region`** â€” Cream-tinted multi-column footer.
...
**`footer-link`** â€” Individual footer link.
...
- Use the **sunset stripe band** at the foot of every page â€” it's the brand's most recognizable signature
- Pair PP Editorial Old (display) with Inter (UI) â€” never substitute either with a generic alternative
...
- Anchor heroes with photographic mountain-sunset imagery (or its visual equivalent â€” atmospheric gradient sky)
- Use stat-display token (PP Editorial 56px) for stat callouts to maintain editorial character

### Don't
- Don't use pill-shaped buttons (`{rounded.full}`) â€” Mistral's geometry is sober and editorial, not playful
- Don't introduce additional accent colors beyond the orange/yellow/cream sunset palette
- Don't reduce hero leading below 1.05 â€” the editorial display needs that magazine-grade tightness
- Don't replace PP Editorial Old hero displays with Inter â€” the editorial / sans contrast IS the brand
- Don't apply heavy shadows on flat documentation cards; reserve elevation for IDE mockups
- Don't drop the sunset stripe band from any page bottom â€” it's the brand's continuity element
...
| Mobile (large) | 480 â€“ 767px | Feature tiles 2-up. Hero scales to 52px. |
| Tablet | 768 â€“ 1023px | 2-column feature grids. Pill-tab nav returns. Hero 64px. |
| Desktop | 1024 â€“ 1279px | Multi-column layouts. Hero 76px. Stat row at full width. |
| Wide Desktop | â‰¥ 1280px | Full 84px hero presentation. |

### Touch Targets
- Buttons render at 40â€“44px effective height â€” at WCAG AAA floor with `10px 20px` padding
- Form inputs render at 44px height
- Pill tabs render at ~32px tall â€” bumps to 44px on mobile

### Collapsing Strategy
- **Promo banner** stays full-width; truncates at < 480px
- **Top nav** below 1024px collapses to hamburger
- **Hero band**: 2-column hero (text + photography) collapses to stacked at < 1024px
- **Pricing tiers**: 4-column desktop â†’ 2-column tablet â†’ 1-column mobile
- **Stat row**: 3-column â†’ stacked at < 768px
- **Hero typography**: 84px â†’ 64px â†’ 52px â†’ 40px
- **Footer**: 5-column desktop â†’ 3-column tablet â†’ 1-column accordion mobile
...
- Customer logo wall presents wordmarks at consistent 60â€“80px height
...
- Animation/transition timings not extracted; recommend 150â€“200ms ease for hover/focus state transitions
- Form validation success state not explicitly captured beyond defaults
- Sunset stripe band gradient stops are approximations â€” the actual values may vary slightly across pages but the visual rhythm is consistent
