...
description: Design system and styling guidelines based on intercom. Use this skill when asked to create a design inspired by intercom.
---
---
version: alpha
name: Intercom-design-analysis
description: "An editorial customer-service marketing canvas built around a soft cream-white ground, charcoal type set in Saans (Intercom's proprietary geometric sans), and a single confident Fin Orange (#ff5600) reserved for the Fin AI brand. Cards live as floating white tiles with thin hairline borders and minimal radii (8â€“16px). Display headlines run Saans at weight 500 with measured negative tracking. The system reads as a careful, product-led publication: product screenshots dominate, ornament is rare, and the only place chromatic energy enters is the Fin Orange CTA."
...
Intercom's marketing canvas is a soft cream-white ground (`{colors.canvas}` â‰ˆ #f5f1ec) â€” not pure white. The warmth is the brand's signal: this is editorial, calm, and product-focused, not bright SaaS. On top of the cream canvas sit white floating cards (`{colors.surface-1}`), thin hairline dividers (`{colors.hairline}`), and charcoal type (`{colors.ink}` #111111).

Display type is **Saans** â€” Intercom's proprietary geometric sans â€” set at weight 500 with measured negative letter-spacing (-2.0px on 72px display). Body type is the same family at weight 400. The single proprietary mono is **SaansMono**, used sparingly for code snippets and product UI screenshots embedded in the marketing surface.

The single chromatic accent is **Fin Orange** (`{colors.fin-orange}` #ff5600) â€” Intercom's AI-product brand color. It surfaces on the Fin product CTA, the Fin badge in pricing, and a few inline emphasis moments. It is NOT a system primary; the system primary is charcoal `{colors.ink}`. Intercom also maintains a small **report palette** (`{colors.report-blue}`, `{colors.report-green}`, `{colors.report-pink}`, `{colors.report-lime}`) used inside in-product analytics surfaces shown in mockups.
...
**Key Characteristics:**
- **Cream canvas** (`{colors.canvas}` #f5f1ec) is the brand's defining surface â€” neither white nor gray, deliberately warm.
...
- **Charcoal** `{colors.ink}` (#111111) is the system primary â€” buttons, headlines, body type all sit on charcoal.
- **Fin Orange** (`{colors.fin-orange}` #ff5600) is the AI product color â€” used on the Fin CTA and Fin badge, never decoratively.
- Display tracking pulls aggressively negative (-2.0px on 72px); body stays at 0.
- Card corners stay modest at `{rounded.lg}` 12px and `{rounded.xl}` 16px â€” never pill-rounded; never square.
...
### Brand & Accent
- **Charcoal** ({colors.ink}): The system primary surface. Headlines, body type, primary CTA pill background â€” all charcoal.
...
- **Brand Blue** ({colors.brand-blue}): Saturated brand blue (#0007cb) â€” used on a small set of marketing illustrations.

### Surface
- **Canvas** ({colors.canvas}): Default page background â€” soft cream-white #f5f1ec.
- **Surface 1** ({colors.surface-1}): Pure white â€” used for floating cards (pricing, feature, product-mockup).
- **Surface 2** ({colors.surface-2}): Slightly darker cream â€” startup-discount banner, alt-row stripes.
- **Hairline** ({colors.hairline}): 1px borders on cards â€” soft warm gray (#d3cec6).
- **Hairline Soft** ({colors.hairline-soft}): Even softer dividers between FAQ rows and footer columns.
- **Inverse Canvas** ({colors.inverse-canvas}): Pure black â€” only on the testimonial / quote callout strip.
- **Inverse Surface 1** ({colors.inverse-surface-1}): One step lighter â€” hovered footer items in dark contexts.

### Text
- **Ink** ({colors.ink}): All headlines, body type, button labels â€” charcoal #111111.
- **Ink Muted** ({colors.ink-muted}): Secondary type at #626260 â€” meta info, deselected pricing tabs.
- **Ink Subtle** ({colors.ink-subtle}): Tertiary type at #7b7b78 â€” footer columns, helper text.
- **Ink Tertiary** ({colors.ink-tertiary}): Quaternary type at #9c9fa5 â€” disabled, footnotes.
- **Inverse Ink** ({colors.inverse-ink}): White on black â€” quote-strip type.
- **Inverse Ink Muted** ({colors.inverse-ink-muted}): Light gray on black â€” quote-strip meta.
...
The report palette appears INSIDE product UI mockups â€” these are Intercom's in-product chart colors, not marketing surface colors.

## Typography

### Font Family

- **Saans** â€” Intercom's proprietary geometric sans, fallback `Saans Fallback, ui-sans-serif, system-ui`. Carries display, body, eyebrow, and button.
- **SaansMono** â€” Proprietary mono, fallback `SaansMono Fallback, ui-monospace`. Used inside code snippets shown in product mockups.
...
- **Negative letter-spacing scales with size.** -2.0px at 72px (â‰ˆ3% of size), down to 0 on body.
...
- **Eyebrow uses sentence case** at 14px / 500 weight â€” no all-caps tracking.

### Note on Font Substitutes

If implementing without Saans, suitable substitutes include **SÃ¶hne** (paid), **Inter** (free, weight 500), or **Geist Sans** (free). Inter at weight 500 is the closest free substitute; SaansMono can be approximated with **JetBrains Mono** at weight 400.
...
- Pill button padding: 10px vertical Â· 18px horizontal.
...
- Product mockup cards span full content width â€” they're the protagonist of every section.
...
- **Product UI mockups** dominate every section's right column or center band â€” these are screenshots, not illustrations.
...
- Customer logo tiles render at small sizes (~24â€“32px logo height) on `{colors.canvas}` cream with no border.
- Avatar circles in testimonial cards use `{rounded.full}` at 40â€“48px sizes.

## Components

### Buttons

**`button-primary`** â€” Charcoal CTA. The default primary CTA across all pages.
...
**`button-secondary`** â€” White button on cream. Used for secondary CTAs.
...
**`button-fin`** â€” Fin Orange CTA â€” reserved for Fin AI product CTAs.
...
- Selected: `{colors.surface-1}` white background, `{colors.ink}` text â€” selected = lift onto white.
...
**`pricing-card-featured`** â€” Featured / recommended tier â€” inverts to charcoal.
- Background `{colors.ink}`, text `{colors.on-primary}`, otherwise identical structure.

**`feature-card`** â€” Generic feature highlight.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.lg}`, padding 24px.

**`product-mockup-card`** â€” The dominant card type â€” frames a high-fidelity product UI screenshot.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.xl}`, padding 24px.

**`testimonial-card`** â€” Customer quote with avatar + name + company.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body-lg}`, rounded `{rounded.lg}`, padding 32px.

**`startup-discount-card`** â€” The "Startups get 90% off" tinted card.
...
**`text-input`** + **`text-input-focused`** â€” Form fields on contact and search overlays.
- Background `{colors.surface-1}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.md}`, padding 10px 14px.

### FAQ

**`faq-row`** â€” Expandable accordion row in the pricing FAQ.
...
**`top-nav`** â€” Sticky cream bar with the Intercom wordmark left, nav links centered, log-in + sign-up pair right.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-sm}`, height 56px.

### Footer

**`footer`** â€” Dense link grid on `{colors.canvas}` cream with the Intercom wordmark left.
...
- Reserve `{colors.canvas}` cream as the system's anchor surface â€” never replace with pure white.
...
| Mobile | 480px | Single-column; display-xl scales 72px â†’ ~32px |

### Touch Targets

- CTAs hold â‰¥40px tap height across viewports.
- Pricing tab pills hold â‰¥40px tap height.
...
- Saans and SaansMono are proprietary; an open-source substitute (Inter, SÃ¶hne, Geist) is acceptable.
