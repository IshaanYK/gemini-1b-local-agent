...
description: Design system and styling guidelines based on wired. Use this skill when asked to create a design inspired by wired.
---
---
version: alpha
name: Wired-Inspired-design-analysis
description: An inspired interpretation of Wired's design language â€” a flagship technology-magazine brand whose surface is a strict editorial duet of stark black wordmark on white canvas, anchored by a tall narrow custom display serif for hero headlines, a humanist serif body face for long-form reading, and a clean sans face for metadata; layout reads like a printed magazine ported to the web with very little marketing chrome.
...
Wired is the flagship technology-magazine brand under CondÃ© Nast â€” and the web surface refuses to dress itself as a SaaS marketing site. The page is unmistakably an editorial product: a white canvas, a strict black wordmark in the brand's proprietary `WiredDisplay` (a tall, narrow, high-contrast serif used at 64 px), and stacked story cards that read as a printed magazine grid ported to the screen. There is no atmospheric gradient, no decorative chrome, no chromatic accent â€” the brand's only colour beyond the black-and-white duet is the small `{colors.link}` (`#057dbc`) used for inline body links inside long-form articles.
...
Buttons are square. The brand uses `{rounded.none}` 0 px corners across the entire UI â€” newsletter sign-ups, login forms, "Read more" CTAs all render as sharp rectangles. The only circular shape is the `button-icon-circular` used for social-share affordances. There are no soft drop-shadows; the brand uses hairline borders for elevation when needed.
...
- Three-face typographic system â€” `WiredDisplay` serif for display, `BreveText` serif for body, `Apercu` sans for metadata and buttons.
- Square buttons (`{rounded.none}`) â€” the brand never softens corners on interactive elements.
...
- The brand's only signature decorative move is the **masthead band** â€” a thin black strip with the wordmark centred, no other decoration.
- A near-black `{colors.ink}` (`#000000`) footer band, no graphics, just text columns and the wordmark repeating.

## Colors

### Brand & Accent
- **Ink Black** (`{colors.primary}` â€” `#000000`): The brand's only "accent." Used for wordmark, headlines, CTAs, footer fill. Pure black, never softened.

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): The default page background.
- **Canvas Soft** (`{colors.canvas-soft}` â€” `#f5f5f5`): Rare tint used for the comment-section background and search-result row hover states (not in the main page rhythm).
- **Hairline** (`{colors.hairline}` â€” `#e0e0e0`): 1 px dividers between story rows. The brand's only "line."

### Text
- **Ink** (`{colors.ink}` â€” `#000000`): Every headline, every body paragraph in BreveText.
- **Ink Soft** (`{colors.ink-soft}` â€” `#1a1a1a`): A near-black variant used for caption-strong / footer link emphasis.
- **Body** (`{colors.body}` â€” `#757575`): Secondary metadata â€” bylines, timestamps, supporting body lines.
...
- **Link** (`{colors.link}` â€” `#057dbc`): The inline body-link blue. Used only inside long-form article body copy, never on UI buttons or navigation.

## Typography

### Font Family
Three families ladder the system:
1. **WiredDisplay** â€” the proprietary tall-narrow high-contrast serif used exclusively for display headlines (64 px hero, scaling down to 26 px sub-display). The brand's most-recognisable typographic signature.
2. **BreveText** â€” the proprietary humanist serif used for long-form body, bylines, and editorial captions. Used at 16 â€“ 19 px line-height 1.45 â€“ 1.50 for comfortable reading density.
3. **Apercu** â€” a humanist sans used for nav, button labels, category eyebrows, metadata, and captions. Weights 400 / 700.
...
- **Display weight 400** â€” the proprietary WiredDisplay reads as elegant by virtue of its thin-tall-narrow design at default weight, not via weight 700+.
...
- **WiredDisplay** â€” *Playfair Display* weight 400 at large display sizes captures the high-contrast didone feel, though wider than the brand's tall-narrow proportions.
- **BreveText** â€” *Lora* or *Source Serif Pro* at 16 â€“ 19 px.
- **Apercu** â€” *Inter* or *Manrope* weights 400 / 700.

## Layout

### Spacing System
- **Base unit**: 4 px.
- **Tokens**: `{spacing.xxs}` 2 px Â· `{spacing.xs}` 4 px Â· `{spacing.sm}` 8 px Â· `{spacing.md}` 12 px Â· `{spacing.lg}` 16 px Â· `{spacing.xl}` 20 px Â· `{spacing.2xl}` 24 px Â· `{spacing.3xl}` 32 px Â· `{spacing.4xl}` 48 px.
...
|---|---|---|
| Mobile | < 768px | Cover hero 64â†’40 px; all grids 1-up; hamburger nav. |
| Tablet | 768â€“1023px | 2-up secondary story grid. |
| Desktop | â‰¥ 1024px | Full magazine grid. |
...
| Level | Treatment | Use |
|---|---|---|
| Level 0 â€” Flat | No shadow, no border. | Default â€” almost every surface lives at this level. |
| Level 1 â€” Hairline | 1 px solid `{colors.hairline}` border. | Story-row dividers, input borders. |
| Level 2 â€” Heavy Black Border | 2 px solid `{colors.ink}` border. | Subscribe CTA on certain campaign moments. |
...
| Token | Value | Use |
|---|---|---|
| `{rounded.none}` | 0px | Every interactive shape â€” buttons, inputs, cards. The brand's signature square geometry. |
...
**`button-primary`** â€” the square black CTA.
...
**`button-outline`** â€” the white outline CTA.
- Background `{colors.canvas}`, text `{colors.ink}`, 1 px solid `{colors.ink}` border, same typography / padding / shape.

**`button-icon-circular`** â€” the circular share-icon button.
- Background `{colors.canvas}`, ink icon, shape `{rounded.full}`.

### Cards & Containers

**`story-card-large`** â€” the cover-story card with `{typography.display-md}` headline.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.lg}`. No border â€” the card lives on the canvas with the photo doing the work.

**`story-card`** â€” the secondary story card.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.md}`. Photo at top, sans display heading + body lead below.

**`story-row`** â€” the bylined story list row.
...
**`text-input`** â€” the standard text input.
...
**`nav-bar`** â€” the top nav, light by default.
...
**`footer`** â€” the black footer band.
...
**`hero-band`** â€” the white hero band hosting the cover-story.
...
**`masthead-band`** â€” the thin top band with the wordmark.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.md} {spacing.xl}`. The wordmark sits centred; flanked by section nav.

**`category-eyebrow`** â€” the small uppercase category label above story headlines.
- Text `{colors.ink}`, set in `{typography.body-sm-strong}` (Apercu 14 / 700 â€” though some campaigns use ALL CAPS via CSS).

**`byline-row`** â€” the article byline strip.
...
**`hairline-divider`** â€” the 1 px line between story rows.
...
- Use `{rounded.none}` 0 px on every button and form input. The brand reads as a printed magazine â€” square corners are non-negotiable.
- Pair WiredDisplay (serif display) with BreveText (serif body) and Apercu (sans labels). Three faces, three roles.
- Render story rows with `{colors.hairline}` 1 px dividers â€” the brand's only elevation cue.
