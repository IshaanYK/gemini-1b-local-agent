---
name: design-md-miro
description: Design system and styling guidelines based on miro. Use this skill when asked to create a design inspired by miro.
---
---
version: alpha
name: Miro-design-analysis
description: Miro presents itself as the AI-powered visual workspace through a confident, almost playful brand voice â€” anchored by its signature canary yellow ({colors.brand-yellow}) wordmark over white canvas, broken open by colorful pastel feature tints (rose, teal, coral, orange, mint) that echo the actual sticky-note color palette used on the live whiteboard. Black-pill primary buttons dominate marketing, real Miro-board mockups serve as feature illustrations, and a 4-tier pricing grid leads into a dense comparison table. Roobert PRO carries display headlines; the system supports homepage, pricing, AI Workflows product page, agile vertical, and customer stories surfaces.
...
### Brand & Accent
- **Miro Yellow** ({colors.brand-yellow}): The brand's recognizable canary yellow â€” wordmark color, top promo banner, "yellow tag" pills
...
- **Negative letter-spacing progression** â€” display sizes use -2px to -1.5px; smaller headings relax to 0
- **Stat-display token** (64px / 500) for marketing stat callouts
- **Single weight scale** â€” 400 (body), 500 (medium emphasis + headings), 600 (badges and uppercase). Roobert PRO does not use 700 in this system.
...
Marketing surfaces give content generous breathing room â€” `{spacing.hero}` (120px) hero padding gives the small wordmark room to breathe. Pricing surfaces tighten dramatically.
...
### Decorative Depth
- The atmospheric depth on Miro's hero comes from the live-product-board mockup illustrations â€” sticky notes layered at z-offsets, color-block tints behind whiteboard frames
...
**`button-primary`** â€” Black pill primary CTA, the dominant action ("Get started free").
...
**`button-yellow`** â€” Brand-yellow pill for moments of brand emphasis.
...
**`button-blue`** â€” Brand-blue pill for inline action callouts.
...
**`button-secondary`** â€” Outlined pill for secondary actions ("Book a demo").
...
**`pricing-card-featured`** â€” Featured pricing tier (Business â€” lavender background + blue border).
- Background `{colors.surface-pricing-featured}`, rounded `{rounded.xl}`, padding `{spacing.xxl}`, border `2px solid {colors.brand-blue}`.

**`pricing-card-enterprise`** â€” Dark-canvas enterprise tier card.
...
**`filter-dropdown`** â€” Pill-shaped filter dropdown ("Company use" / "Industry" / "Use case").
...
**`pill-tab`** + **`pill-tab-active`** â€” Pill-style tab nav.
...
**`toggle-monthly-yearly`** â€” Two-state pill toggle (Monthly / Annual on pricing).
- Background `{colors.surface}`, rounded `{rounded.full}`, padding `4px`.

### Badges & Status

**`badge-promo`** â€” Yellow promo banner badge.
...
**`badge-tag-yellow`** â€” Soft-yellow feature tag chip ("Yellow" tag on AI Workflows page).
...
**`badge-tag-purple`** â€” Lavender feature tag chip ("AI agent" tag).
...
**`badge-tag-coral`** â€” Coral feature tag chip variant.
...
**`badge-success`** â€” Green success indicator.
...
**`badge-discount`** â€” Yellow rectangular discount pill ("Save 15%").
...
**`comparison-table`** â€” Pricing feature comparison table.
- Background `{colors.canvas}`, text `{colors.ink}`, typography `{typography.body-sm}`, rounded `{rounded.md}`, border `1px solid {colors.hairline}`.

**`comparison-row`** â€” Individual feature row.
- Background `{colors.canvas}`, text `{colors.ink}`, padding `{spacing.md} {spacing.lg}`, bottom border `1px solid {colors.hairline-soft}`.

### Documentation Components

**`whiteboard-mockup`** â€” Real Miro-board UI rendered as feature illustration.
...
**`template-card`** â€” Template thumbnail card.
- Background `{colors.canvas}`, rounded `{rounded.xl}`, padding `{spacing.md}`, border `1px solid {colors.hairline}`.

**`industry-tile`** â€” Industry-vertical tile.
- Background `{colors.canvas}`, rounded `{rounded.xl}`, padding `{spacing.xl}`, border `1px solid {colors.hairline-soft}`.

**`faq-accordion-item`** â€” FAQ panel item.
...
**`capterra-badge`** â€” Review/rating badge in the footer.
...
**`app-store-badge`** â€” App store / Google Play download pill.
...
**Top Navigation (Marketing)** â€” Sticky white bar with yellow Miro wordmark + horizontal links + right-side CTAs.
...
**`hero-band-marketing`** â€” Marketing hero band.
...
**`cta-banner-dark`** â€” Dark CTA banner at the bottom of feature pages.
...
**`footer-region`** â€” Massive multi-column dark footer.
...
**`footer-link`** â€” Individual link in the footer.
...
- Don't use stock photography â€” show the live product board UI
...
| Mobile (large) | 480 â€“ 767px | Feature tiles 2-up. Hero scales to 48px. |
| Tablet | 768 â€“ 1023px | 2-column feature grids. Pill-tab nav returns. |
| Desktop | 1024 â€“ 1279px | 4-tier pricing card row. Customer story grid 2-up. Hero at 64px. |
| Wide Desktop | â‰¥ 1280px | Full hero presentation, 80px hero display. |

### Touch Targets
- Pill buttons render at 40â€“44px effective height â€” at WCAG AAA floor
- Circular icon buttons: 36Ã—36px desktop â†’ 44Ã—44px mobile
- Form inputs render at 44px height
- Filter dropdowns render at ~36px tall â€” bumps to 44px on mobile

### Collapsing Strategy
- **Promo banner** stays full-width; truncates at < 480px
- **Top nav** below 1024px collapses to hamburger
- **Hero band**: 2-column hero collapses to stacked at < 1024px
- **Pricing comparison**: 4-column tiers â†’ 2-column tablet â†’ 1-column mobile; comparison table becomes horizontal-scroll
- **Customer story grid**: 2-up â†’ 1-up at < 768px
- **Hero typography**: 80px â†’ 60px tablet â†’ 48px mobile-large â†’ 36px mobile-small
- **Footer**: 6-column desktop â†’ 3-column tablet â†’ 2-column mobile â†’ accordion at small mobile
...
- Animation/transition timings not extracted; recommend 150â€“200ms ease
