...
description: Design system and styling guidelines based on vodafone. Use this skill when asked to create a design inspired by vodafone.
---
---
version: alpha
name: Vodafone-Inspired-design-analysis
description: An inspired interpretation of Vodafone's design language â€” a telecom super-brand whose web surface alternates between editorial photography hero bands with massive uppercase display headlines and clean white content bands, anchored by the company's signature scarlet red CTA and the proprietary Vodafone display sans set at impossibly heavy 800 weight.
...
components:
  ...
  ex-pricing-tier-featured:
    description: "Featured tier â€” polarity-flipped to ink with white text and white pill CTA inside."
    backgroundColor: "{colors.ink}"
    textColor: "{colors.on-primary}"
    rounded: "{rounded.card}"
    padding: "{spacing.lg}"
  ex-product-selector:
    description: "Tariff-tier picker â€” repurposed as the brand's plan selector with badge-chip chips inside the frame."
    backgroundColor: "{colors.canvas-soft}"
    rounded: "{rounded.card}"
    padding: "{spacing.lg}"
  ex-cart-drawer:
    description: "Subscription summary â€” line items per tariff add-on, light hairline dividers."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.card}"
    padding: "{spacing.lg}"
  ...
  ex-auth-form-card:
    ...
    padding: "{spacing.lg}"
  ex-modal-card:
    description: "Modal dialog surface â€” same chrome as card-content; brand uses scrim, not card shadow."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.card}"
    padding: "{spacing.lg}"
  ex-empty-state-card:
  ...
  ex-toast:
    description: "Toast notification surface â€” card-content shape with caption-strong body."
    backgroundColor: "{colors.canvas}"
    rounded: "{rounded.sm}"
    padding: "{spacing.md} {spacing.lg}"
    typography: "{typography.body-sm}"
...
Every interactive CTA renders as a generously rounded pill (`{rounded.pill-lg}` 60 px) â€” Vodafone has never used a square button on its marketing surface in years, and the brand's pill scale ladder runs from 32 px (badge pills) through 60 px (CTA pills) up to 9999 px (icon circular containers). Cards stay gentler at `{rounded.card}` 6 px.

**Key Characteristics:**
- A single primary CTA color `{colors.primary}` (`#e60000`) â€” Vodafone Red. Pill-filled for primary, pill-outline for secondary. No third button variant.
...
- The `speechmark-logo-orb` â€” a red square hosting Vodafone's quotation-mark icon â€” is the only piece of decorative chrome that's not a CTA. It anchors the brand's centre of every page.
- Pill geometry on every interactive shape â€” `{rounded.pill-lg}` 60 px for buttons, `{rounded.pill-md}` 32 px for inline badges. Card chrome stays at `{rounded.card}` 6 px.
- A two-band page rhythm â€” `{colors.ink}` dark hero / `{colors.canvas}` light content. No mid-band greys; the brand uses surface contrast, not soft neutrals, for elevation.
- Editorial photography (real portraits, real cities, real cabling) is the only consistent decorative system â€” no illustration, no atmospheric gradients.

## Colors

### Brand & Accent
- **Vodafone Red** (`{colors.primary}` â€” `#e60000`): The single brand accent. Every primary CTA pill, every speechmark logo, every conversion target. The most iconic red in telecom â€” never desaturated, never used at scale for body fills (reserved for high-attention surfaces).

### Surface
- **Canvas** (`{colors.canvas}` â€” `#ffffff`): The default light content background.
- **Canvas Soft** (`{colors.canvas-soft}` â€” `#f2f2f2`): A near-white tint used as the badge-chip background.
- **Ink** (`{colors.ink}` â€” `#25282b`): The brand's near-black surface â€” used as the dark hero band, the nav background, and the footer fill. Doubles as the primary text color on light surfaces.

### Text
- **Ink** (`{colors.ink}` â€” `#25282b`): Every heading and body paragraph on light surfaces.
- **Body** (`{colors.body}` â€” `#7e7e7e`): Secondary body text on light surfaces â€” captions, metadata, supporting copy.
- **Mute** (`{colors.mute}` â€” `#bebebe`): The lowest-priority text color â€” placeholder text, low-key footer links.
- **On Dark** (`{colors.on-dark}` â€” `#ffffff`): All text on `{colors.ink}` surfaces (hero, footer, nav).
...
### Font Family
A single custom face carries the entire system: **Vodafone**, the brand's proprietary display sans. The face spans weights 300 (light), 400 (regular), 600, 700, and 800 â€” every role in the system pulls from this one family. There is no mono companion; technical labels (rare on the marketing surface) borrow the same face at smaller sizes.
...
|---|---|---|---|---|---|
| `{typography.display-hero}` | 144px | 800 | 114px | -1px | The hero stencil (`"STAY CONNECTED"`) â€” uppercase, ultra-tight tracking, brand's signature size. |
...
- **Weight 300 = the calmer secondary voice.** Used at 40 â€“ 48 px for sub-displays; never below 24 px to keep legibility.
...
- **Display sans** â€” *Inter* weight 800 at hero scale with `letter-spacing: -1px` is the closest free match. *Geist* weight 700â€“800 is the second-best.
- **Lighter display weight (300)** â€” *Inter* weight 300 holds its line-height well at 48 px display sizes.
...
| Tablet | 600â€“1023px | Story-card grid 2-up; display headlines drop to 90 â€“ 110 px. |
| Desktop | 1024â€“1399px | Full display headline at 126 â€“ 144 px; 2-up story grid. |
| Ultra-wide | â‰¥ 1400px | Container caps at ~1400 px; bands stay edge-to-edge in colour. |
...
- **Story-card grid**: 2-up â†’ 1-up at the breakpoint above.
...
| Level | Treatment | Use |
|---|---|---|
| Level 0 â€” Flat | No shadow, no border. | Default for most cards and panels â€” surface contrast does the elevation work. |
| Level 1 â€” Hairline | 1 px solid `{colors.ink}` border. | Form inputs, the `divider-on-dark` between footer columns. |
| Level 2 â€” Border on Dark | 1 px solid `{colors.on-dark}` border on `{colors.ink}` surfaces. | Outline buttons sitting on the dark hero band. |
...
| `{rounded.sm}` | 6px | The brand's canonical content radius â€” applied to images and inputs. |
| `{rounded.card}` | 6px | Card chrome and image frames (alias for `sm`). |
| `{rounded.pill-md}` | 32px | Badge / chip pills. |
| `{rounded.pill-lg}` | 60px | The brand's signature CTA pill â€” every primary and secondary button. |
...
**`button-primary`** â€” the red pill CTA, brand's primary conversion target.
...
**`card-hero`** â€” the slightly larger card variant used as the lead story.
...
**`nav-bar`** â€” the dark top nav, fixed to the page top.
...
**`nav-link`** â€” the link items inside `nav-bar`.
...
**`hero-band-dark`** â€” the dark navy/ink hero band hosting the massive display headline.
...
**`hero-band-red`** â€” the rare full-bleed red hero used on signature campaigns.
- Background `{colors.primary}`, text `{colors.on-primary}`, padding `{spacing.3xl} {spacing.3xl}`. Headline in `{typography.display-xl}`.

**`content-band-light`** â€” the white content band that follows every hero.
...
**`speechmark-logo-orb`** â€” the red square hosting Vodafone's quotation-mark icon. The brand's visual anchor.
...
**`badge-chip`** â€” the inline metadata pill used for category tags inside story cards.
...
**`divider-on-dark`** â€” the 1 px hairline used between sections / columns on dark surfaces.
...
- Cycle page surfaces in `{colors.ink}` dark hero â†’ `{colors.canvas}` light content â†’ `{colors.ink}` footer. Surface contrast is the depth cue.
- Pair editorial portrait photography with the massive display headline overlay â€” that combination IS the brand's signature.
- Render the speechmark logo orb at consistent size relative to surrounding content â€” it's the brand's centre of gravity on every page.
...
- Don't pair the weight 800 display face with letter-spacing 0 at 144 px â€” the `-1px` tracking is part of the brand's calibration.
